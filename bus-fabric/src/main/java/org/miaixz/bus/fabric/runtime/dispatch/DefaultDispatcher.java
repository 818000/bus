...
final class DefaultDispatcher implements Dispatcher {
     ...
     */
    private static final int MAX_BACKGROUND = Normal._1024;
                ...
                if (background.size() >= MAX_BACKGROUND) {
                    if (Logger.isWarnEnabled()) {
                        Logger.warn(
                                false,
                                "Fabric",
                                "Background dispatch rejected: active={}, limit={}",
                                background.size(),
                                MAX_BACKGROUND);
                    }
            ...
            try {
                ...
                if (Logger.isErrorEnabled()) {
                    Logger.error(
                            ...
                            "Fabric",
                            e,
                            "Background dispatch submission failed: key={}, exception={}",
                            handle.key(),
    ...
    public List<Activity> queued() {
        return activities(State.QUEUED);
    }
    ...
    public List<Activity> running() {
        return activities(State.RUNNING);
    }
    ...
    public void close() {
        ...
        if (failure != null) {
            if (Logger.isErrorEnabled()) {
                Logger.error(false, "Fabric", failure, "Dispatcher close failed");
            }
            throw new InternalException("Unable to close dispatcher", failure);
        }
        if (Logger.isInfoEnabled()) {
            Logger.info(false, "Fabric", "Dispatcher closed");
        }
    }
    ...
    private void runDelayed(final DelayedDispatch task) {
        synchronized (submissionLock) {
            ...
            synchronized (registry) {
                if (registry.get(task.handle) != Channel.DELAYED || scope.state() != State.RUNNING) {
                    task.handle.cancel();
                    return;
                }
    ...
    private void runBackground(final BackgroundDispatch task) {
        ...
        try {
            ...
            handle.activity().run();
            if (handle.activity().state() == State.CANCELLED || handle.activity().cancellation().cancelled()) {
                handle.cancel();
            ...
            if (handle.state() == State.CANCELLED || handle.activity().state() == State.CANCELLED
                    || original instanceof CancellationException) {
                handle.cancel();
                if (Logger.isDebugEnabled()) {
                    Logger.debug(false, "Fabric", "Background dispatch cancelled: key={}", handle.key());
                }
            } else {
                ...
                if (Logger.isErrorEnabled()) {
                    Logger.error(
                            ...
                            "Fabric",
                            original,
                            "Background dispatch failed: key={}, exception={}",
                            handle.key(),
    ...
    private RuntimeException closeBackground(final RuntimeException failure) {
        ...
        for (final BackgroundDispatch task : snapshot) {
            if (task.future != null && task.handle.state() != State.CANCELLED) {
        ...
        try {
            ...
            synchronized (background) {
                retained = background.values().stream()
                        .map(
                                task -> task.handle.key() + Symbol.C_BRACKET_LEFT + task.handle.state().name() + "]@"
                                        + topFrame(task.runner))
     ...
     * @return activities
     */
    private List<Activity> activities(final State state) {
