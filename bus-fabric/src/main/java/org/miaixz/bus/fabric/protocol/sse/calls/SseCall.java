...
public final class SseCall implements Call<SseSession> {
     ...
     * Result future.
     */
    private final CompletableFuture<SseSession> future;
    ...
    private SseCall(final SseX exchange, final Dispatcher dispatcher) {
        if (exchange == null) {
            throw new ValidateException("SSE exchange must not be null");
        }
     ...
     * @return this call
     */
    public Call<SseSession> enqueue(final Dispatcher dispatcher) {
        if (dispatcher == null) {
        ...
        final Activity activity = Activity.of("sse-open", () -> {
            try {
                open();
            } catch (final RuntimeException e) {
                future.completeExceptionally(e);
     ...
     */
    @Override
    public SseSession await(final Duration timeout) {
     ...
     */
    private static SseSession awaitFuture(final CompletableFuture<SseSession> future) {
        try {
            return future.get();
        } catch (final InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new InternalException("Interrupted while waiting for SSE call", e);
        } catch (final ExecutionException e) {
            throw new InternalException("SSE call failed", e.getCause());
        } catch (final CancellationException e) {
            throw new InternalException("SSE call was cancelled", e);
        }
    }
     ...
     * @return completed value
     */
    private SseSession awaitFuture(final CompletableFuture<SseSession> future, final Duration timeout) {
        if (timeout.isZero()) {
            if (!future.isDone()) {
                cancel();
                throw new TimeoutException("SSE call timed out");
            }
            return awaitFuture(future);
        }
        try {
            ...
            cancel();
            throw new TimeoutException("SSE call timed out", e);
