...
public class ImageBanner {
    ...
    /**
     * Applies the piecewise CIE transfer function used while converting XYZ coordinates to L*a*b*.
     *
     * @param t normalized XYZ coordinate
     * @return cube-root or linearized coordinate required by the CIE conversion
     */
    ...
    public String printBanner(Integer maxWidth, Double aspectRatio, boolean invert, boolean cie94) {
        if (this.image == null || !this.image.isFile() || maxWidth == null || maxWidth <= 0 || aspectRatio == null
                || !Double.isFinite(aspectRatio) || aspectRatio <= 0) {
            return Normal.EMPTY;
        }
        try {
            BufferedImage sourceImage = ImageIO.read(this.image);
            if (sourceImage == null) {
                return Normal.EMPTY;
            }
            BufferedImage resizedImage = resizeImage(sourceImage, maxWidth, aspectRatio);
            return imageToBanner(resizedImage, invert, cie94);
        } catch (Exception ex) {
            Logger.warn(
                    false,
                    "Starter",
