...
import org.miaixz.bus.spring.web.resolver.BindingDefaults;
...
class JsonTypeMatcher {
     ...
     */
    private static final String DEFAULT_RULE = BindingDefaults.BUS_TYPE_RULE;
     ...
     */
    private JsonTypeMatcher(List<String> rules) {
        this.rules = rules;
    }
     ...
     */
    static JsonTypeMatcher of(String autoType) {
        ...
        return new JsonTypeMatcher(List.copyOf(rules));
    }
    ...
    private boolean matchesArrayDescriptor(String className) {
        ...
        int endIndex = className.indexOf(Symbol.C_SEMICOLON, dimensionIndex);
        if (endIndex < 0) {
            return false;
        }
    ...
    private boolean match(String[] patterns, int patternIndex, String[] segments, int segmentIndex) {
        ...
        if ((Symbol.STAR + Symbol.STAR).equals(pattern)) {
    ...
    private boolean matchesSegment(String pattern, String segment) {
        ...
        String regex = pattern.replace(Symbol.DOT, "\\.").replace(Symbol.STAR, "[^.]*");
    ...
    private String packageNameOf(String className) {
        int index = className.lastIndexOf(Symbol.C_DOT);
