...
public class TextBanner extends AbstractBanner {
     ...
     * <p>
     * The custom ASCII art is retrieved from {@code BannerKeys.BUS_BANNER} and colored green. Spring Boot and Bus
    ...
    protected String printBanner(String bannerText) {
        ...
        for (String line : BannerKeys.BUS_BANNER) {
            builder.append(AnsiOutput.toString(AnsiColor.BRIGHT_GREEN, line)).append(Symbol.LF);
        }

        String springVersion = BannerKeys.SPRING_BOOT_BANNER + String.format("(v%s)", SpringBootVersion.getVersion());
        String busVersion = BannerKeys.BUS_BOOT_BANNER + String.format("(v%s)", Version.all());
