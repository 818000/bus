...
**Bus** 是一个综合性、模块化的企业级 Java 框架，提供开箱即用的生产级组件，涵盖核心工具库、加密安全、HTTP
客户端、分布式链路追踪、支付集成、云存储等全场景能力。所有模块拥有一致的 API 风格和统一的配置方式，并通过 `bus-starter` 与
Spring Boot 无缝集成。
...
支持。Bus 各模块在 `META-INF/native-image` 下提供 GraalVM 元数据，`bus-all` 在打包阶段会将各模块元数据合并为一份聚合
native-image 配置。
