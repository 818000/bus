 ...
 * <p>
 * The delay is calculated as {@code initialDelay * multiplier^(attempt - 1)}, capped by {@code maxDelay}. When
 * jitter is enabled, a random delay between zero and the capped delay is returned.
 * </p>
 *
 * @author Kimi Liu
 * @see Backoff
...
public class ExponentialBackoff implements Backoff {
    ...
    public Duration next(final int attempt) {
        ...
        final long delayMillis =
                (computedMillis <= 0 || computedMillis > maxMillis)
                        ? maxMillis
                        : computedMillis;
