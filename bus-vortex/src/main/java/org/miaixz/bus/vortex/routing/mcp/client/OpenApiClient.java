...
import org.miaixz.bus.vortex.routing.mcp.Tool;
...
public class OpenApiClient implements McpClient {
    ...
    public Mono<Void> initialize() {
        return Mono.fromRunnable(() -> {
            Logger.info(
                    false,
                    "Vortex",
                    "OpenAPI tool catalog initialized: baseUrl={}, tools={}",
                    this.baseUrl,
                    this.tools.size());
        });
    }
    ...
    private Map<String, Tool> initializeTools() {
        if (StringKit.isEmpty(assets.getMetadata())) {
            Logger.warn(false, "Vortex", "OpenAPI metadata is missing; no tools loaded: asset={}", assets.getName());
        ...
        if (toolConfigs == null || toolConfigs.isEmpty()) {
            Logger.warn(
                    false,
                    "Vortex",
                    "OpenAPI tool definition is missing; no tools loaded: asset={}",
                    assets.getName());
            return Collections.emptyMap();
        }
     ...
     * <p>
     * The current implementation only logs shutdown because the underlying {@link WebClient} does not require an
     * explicit close operation.
     */
    @Override
    public void close() {
        Logger.info(false, "Vortex", "Closing OpenAPI endpoint client: baseUrl={}", this.baseUrl);
    }

    /**
     * Returns the currently configured tools exposed by this client.
     *
     * @return copy of the configured tool list
     */
    @Override
    ...
    public Mono<Object> callTool(String toolName, Map<String, Object> arguments) {
        ...
        Logger.info(
                true,
                "Vortex",
                "OpenAPI tool invocation started: name={}, method={}, endpoint={}, queryParameterCount={}, bodyPresent={}",
                toolName,
                httpMethod.value(),
                endpoint,
        ...
        return requestSpec.retrieve().bodyToMono(String.class).map(response -> (Object) response).doOnSuccess(
                response -> Logger.info(
                        false,
                        "Vortex",
                        "OpenAPI tool completed: name={}, responseType={}",
                        toolName,
