 ...
 * <p>
 * MCP qualification owns ingress-route matching, HTTP method verification, authorization, invocation of the shared
 * signature verifier, and final downstream parameter cleanup.
 *
 * @author Kimi Liu
 */
    ...
    public Mono<Void> apply(ServerWebExchange exchange, Chain chain) {
        return Mono.deferContextual(contextView -> {
            ...
            return Mono.defer(() -> Mono.justOrEmpty(this.registry.get(requestRoute))).switchIfEmpty(Mono.defer(() -> {
                ...
                return validationMono.then(authMono).then(
                        SignatureVerifier.verifyMcp(
                                exchange,
                                context,
                                assets,
                                validatedExchange -> finalizeParameters(validatedExchange, context).then(
                                        Mono.fromRunnable(
                                                () -> Logger.info(
                                                        true,
                                                        "Vortex",
