...
/**
 * Holds immutable deployment and security settings for one OpenID Provider.
 * <p>
 * OAuth authorization, token, scope, grant, and credential policy remains in the composed OAuth settings. Signing key
 * material and JWK Set data are resolved externally at runtime; this record stores only the standard key identifier.
 * </p>
 *
 * @param oauth2Settings          validated OAuth authorization-server settings
 ...
 * @author Kimi Liu
 */
public record OpenIdProviderSettings(OAuth2ProviderSettings oauth2Settings, Optional<Endpoint> discoveryEndpoint,
        ...
        String idTokenSigningKeyId, Duration idTokenLifetime) implements ProviderSettings {
     ...
     *                                  invalid
     */
    public OpenIdProviderSettings {
        Assert.notNull(oauth2Settings, "OpenID Connect OAuth 2.x Provider settings must not be null");
        if (oauth2Settings.authorizationEndpoint().isEmpty() || oauth2Settings.tokenEndpoint().isEmpty()) {
            throw new ValidateException(
                    "OpenID Connect Authorization Code Flow requires OAuth authorization and token endpoints");
        }
        if (!oauth2Settings.scopesSupported().contains("openid")) {
            throw new ValidateException("OpenID Connect Provider scopes must contain openid");
        }
        if (!oauth2Settings.grantTypesSupported().contains(GrantType.AUTHORIZATION_CODE)) {
            throw new ValidateException("OpenID Connect Provider must enable the authorization_code grant");
        }
        if (!oauth2Settings.pkceRequired() || !oauth2Settings.refreshTokenRotationRequired()) {
    ...
    /**
     * Returns the issuer from the composed OAuth Provider settings.
     *
     * @return exact OpenID Provider issuer
     */
    public String issuer() {
        return oauth2Settings.issuer();
    }

    /**
     * Returns a diagnostic summary without key identifier, OAuth internals, or endpoint query values.
     *
     * @return redacted OpenID Provider settings summary
     */
    @Override
    public String toString() {
        return "OpenIdProviderSettings[oauth2Settings=[REDACTED], discoveryEndpoint=" + present(discoveryEndpoint)
