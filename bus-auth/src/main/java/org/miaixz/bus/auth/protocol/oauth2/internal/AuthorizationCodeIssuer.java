...
public final class AuthorizationCodeIssuer {
    ...
    /**
     * Frozen authorization-server settings governing this issuer.
     */
     ...
     * @param settings             validated authorization-server settings
     ...
     */
    public AuthorizationCodeIssuer(final String providerId, final OAuth2ProviderSettings settings,
            final ExecutionServices services, final RedirectUriValidator redirectUriValidator,
     ...
     */
    private static boolean nativeClient(final ClientResolver.Client client) {
     ...
     */
    private static String clientName(final ClientResolver.Client client) {
    ...
    /**
     * Extracts RFC 8707 resource indicators from the provider-neutral extension representation.
     *
    ...
    public CompletionStage<Outcome<Result>> authorize(
        ...
        try {
            resolution = services.clientResolver().resolve(authorizationRequest.clientId(), context, timeout);
        ...
        return resolution.handle(
                ...
                .thenCompose(outcome -> switch (outcome) {
                    case Outcome.Succeeded<ClientResolver.Client> success -> success.value() == null
                            ? completed(
                                    Outcome.failed(
                                            failure(
                                                    ErrorCode._500,
    ...
    private CompletionStage<Outcome<Result>> authorizeRegistered(
            ...
            final Optional<AuthorizationCodeStore.OpenIdBinding> openIdBinding,
     ...
     */
    private String selectRedirectUri(final AuthorizationRequest request, final ClientResolver.Client client) {
     ...
     */
    private Validation validateAuthorizedRequest(
            final AuthorizationRequest request,
            final ClientResolver.Client client) {
        ...
        if (!request.scope().isEmpty()) {
            ...
            try {
                ...
                scopeValidator.validateRequested(requested, settings.scopesSupported());
            } catch (RuntimeException exception) {
                return Validation.INVALID_SCOPE;
            }
        }
        if (settings.pkceRequired() && (request.codeChallenge().isEmpty()
    ...
    private CompletionStage<Outcome<Result>> issue(
        ...
        final Instant expiresAt = timeout.clock().now().plus(settings.authorizationCodeLifetime());
        final AuthorizationCodeStore.Entry entry = new AuthorizationCodeStore.Entry(providerId, client.id(),
        ...
        final ExpiringValue<AuthorizationCodeStore.Entry> stored = new ExpiringValue<>(entry, expiresAt);
        final CompletionStage<Boolean> creation;
        try {
            creation = services.authorizationCodeStore()
                    .create(codeKey(code), stored, settings.authorizationCodeLifetime().toMillis());
        } catch (RuntimeException exception) {
            return completed(
                    Outcome.failed(
                            failure(
        ...
        return creation.handle((created, thrown) -> new CreateResult(Boolean.TRUE.equals(created), thrown))
                .thenCompose(result -> {
                    ...
                    extensions.put(OAuth2.Parameters.ISS, new JsonValue.StringValue(settings.issuer()));
    ...
    /**
     * Produces a Provider-isolated irreversible key for the opaque authorization code.
     *
    ...
    private enum Validation {
        ...
        /**
         * At least one requested scope lies outside the client or server-role Source settings.
         */
        INVALID_SCOPE,
        /**
     ...
     * {@link #openId(AuthorizationRequest, AuthorizationCodeStore.OpenIdBinding)}.
     * </p>
     *
    ...
    public record Request(AuthorizationRequest authorizationRequest,
        ...
        public static Request openId(
                final AuthorizationRequest request,
                final AuthorizationCodeStore.OpenIdBinding binding) {
