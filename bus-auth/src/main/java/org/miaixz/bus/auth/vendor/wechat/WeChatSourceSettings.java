 ...
 * @author Kimi Liu
 */
public record WeChatSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, String loginType,
        String agentId, String language, String userType) implements VendorSettings {
     ...
     */
    public WeChatSourceSettings {
        if (!WeChatDefinition.ID.equals(vendor) || !supported(variant)) {
            throw new ValidateException("WeChat settings must select one frozen wechat variant");
        }
        ...
        if (WeChatDefinition.OPEN.equals(variant)) {
            browser(redirectUri);
        ...
        } else if (WeChatDefinition.EE_QRCODE.equals(variant)) {
            browser(redirectUri);
    ...
    private static boolean supported(final Vendor.Variant variant) {
        return WeChatDefinition.OPEN.equals(variant) || WeChatDefinition.MP.equals(variant)
                || WeChatDefinition.MINI.equals(variant) || WeChatDefinition.EE.equals(variant)
                || WeChatDefinition.EE_QRCODE.equals(variant) || WeChatDefinition.EE_WEB.equals(variant);
    }
    ...
    private static void browser(final Optional<String> redirectUri) {
        if (redirectUri.isEmpty()) {
            throw new ValidateException("WeChat browser settings require a registered redirect URI");
        }
    ...
    private static void direct(final Optional<String> redirectUri, final List<String> scopes) {
        if (redirectUri.isPresent() || !scopes.isEmpty()) {
            throw new ValidateException("WeChat Mini Program settings prohibit callback and scope values");
        }
    }
    ...
    private static void requireNoScopes(final List<String> scopes, final String label) {
        if (!scopes.isEmpty()) {
            throw new ValidateException(label + " settings prohibit scope values");
        }
    }
    ...
    private static void prohibit(
        ...
        if (!loginType.isEmpty() || !agentId.isEmpty() || !language.isEmpty() || !userType.isEmpty()) {
            throw new ValidateException(label + " settings prohibit WeCom variant selectors");
        }
    }
    ...
    private static void workQr(
        ...
        if (CORPORATE_APPLICATION.equals(loginType)) {
            Assert.notBlank(agentId, "WeCom CorpApp QR settings require agentid");
        }
        if (!userType.isEmpty()) {
            throw new ValidateException("WeCom QR settings prohibit service-provider usertype");
        }
    }
    ...
    private static void thirdParty(
        ...
        if (!loginType.isEmpty() || !agentId.isEmpty() || !language.isEmpty()) {
            throw new ValidateException("WeCom service-provider QR settings prohibit corporate selectors");
        }
    ...
    private static void workWeb(
        ...
        if (!loginType.isEmpty() || !language.isEmpty() || !userType.isEmpty()) {
            throw new ValidateException("WeCom web settings prohibit QR-only selectors");
        }
        Assert.notBlank(agentId, "WeCom web settings require agentid");
    }
    ...
    public String toString() {
        return "WeChatSourceSettings[vendor=" + vendor + ", variant=" + variant
