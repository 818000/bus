...
import org.miaixz.bus.auth.source.protocol.saml.server.*;
...
/**
 * Compiles one registered SAML 2.0 identity-provider Source into its typed service runtime.
 * <p>
 * Compilation performs all options and security-rule validation before creating services. The runtime receives only
 * standard SAML models and exposes only endpoint-backed capabilities. Successful response and metadata models contain
 * the complete XML Signatures required by the compiled server-role Source policy before they cross the Roster boundary;
 * transport adapters only serialize those already secured models.
 * </p>
 *
 * @author Kimi Liu
 */
public class SamlServerDriver implements ProtocolDriver<SamlServerOptions> {

    /**
     * Creates a stateless SAML identity-provider driver.
     */
    public SamlServerDriver() {
        // No initialization required.
    }

    /**
     * Builds the exact capability set represented by configured identity-provider endpoints.
     *
     * @param options validated SAML identity-provider options
     * @return immutable endpoint-accurate manifest
     */
    private static Capability.Manifest manifest(final SamlServerOptions options) {
        final List<Capability<?, ?>> capabilities = new ArrayList<>();
        capabilities.add(SamlServerScheme.SINGLE_SIGN_ON);
        if (options.singleLogoutServiceEndpoint().isPresent()) {
            capabilities.add(SamlServerScheme.SINGLE_LOGOUT);
        }
        capabilities.add(SamlServerScheme.METADATA);
        return new Capability.Manifest(capabilities);
    }

    /**
     * Returns the SAML identity-provider scheme bound to this driver.
     *
     * @return immutable SAML identity-provider scheme
     */
    @Override
    public SamlServerScheme scheme() {
        return new SamlServerScheme();
    }

    /**
     * Narrows generic Source options to SAML server options.
     *
     * @param options generic Source options
     * @return validated SAML server options
     */
    @Override
    public SamlServerOptions require(final Options<?> options) {
        if (options instanceof SamlServerOptions value) {
            return value;
        }
        throw new ValidateException("SAML server driver requires SamlServerOptions");
    }

    /**
     * Declares consumer, attribute, key, certificate, and session slots required by the identity provider.
     *
     * @param source  Source configuration
     * @param options validated SAML server options
     * @return exact project integration slots
     */
    @Override
    public WorkerSlots slots(final Source source, final SamlServerOptions options) {
        return WorkerSlots.of(
                WorkerSlots.Slot.CONSUMER,
                WorkerSlots.Slot.ATTRIBUTE,
                WorkerSlots.Slot.KEY,
                WorkerSlots.Slot.CERTIFICATE,
                WorkerSlots.Slot.SESSION);
    }

    /**
     * Declares session-state and security-rule dependencies for the SAML identity provider.
     *
     * @param source  Source configuration
     * @param options validated SAML server options
    ...
    public Dependencies dependencies(final Source source, final SamlServerOptions options) {
        return Dependencies.of(Dependencies.Service.SESSION_CACHE, Dependencies.Service.POLICIES);
    }
    ...
    public SourceWorker compile(final Prepared<SamlServerOptions> prepared, final DriverServices services) {
        ...
        if (!scheme().id().equals(source.getType()) || !supports(source.getProtocol())
            ...
            throw new ValidateException("SAML server driver requires a matching Source configuration");
        }
        final SamlServerOptions options = prepared.options();
        if (!services.policies().require(Protocol.SAML).algorithms().contains(options.signatureAlgorithm())) {
            throw new ValidateException(
                    "SAML identity-provider signature algorithm is not permitted by its security rule");
        }
        if (options.clockSkew().compareTo(services.policies().require(Protocol.SAML).maximumClockSkew()) > 0) {
            throw new ValidateException("SAML identity-provider clock skew exceeds its security rule");
        }
        final long maximumBytes = services.policies().require(Protocol.SAML).maximumMessageBytes();
        ...
        final SingleSignOnService signOn = new SingleSignOnService(options, services, assertionIssuer, errorMapper,
                sessions);
        final SingleLogoutService logout = new SingleLogoutService(options, services, errorMapper, sessions);
        final MetadataService metadata = new MetadataService(options, services, metadataCodec);
        final XmlSigner signer = new XmlSigner(options, services, messageCodec, metadataCodec);
        return new CompiledServer(manifest(options), new SamlIdentityProvider(signOn, logout, metadata), signer);
    }

    /**
     * Routes exact declared SAML identity-provider capabilities to the compiled facade.
     *
     * @author Kimi Liu
     */
    private static final class CompiledServer implements SourceWorker {
        ...
        /**
         * Identity-provider-policy XML signer applied before successful models leave the runtime.
         */
         ...
         * @param signer   XML signer bound to the same identity-provider options and key set
         */
        private CompiledServer(final Capability.Manifest manifest, final SamlIdentityProvider provider,
                final XmlSigner signer) {
            this.manifest = Assert.notNull(manifest, "SAML identity-provider manifest must not be null");
            this.provider = Assert.notNull(provider, "SAML identity-provider facade must not be null");
            this.signer = Assert.notNull(signer, "SAML identity-provider XML signer must not be null");
        }
        ...
        private static <S> CompletionStage<Outcome<S>> missing() {
            return completed(
                    Outcome.rejected(
                            new Outcome.Failure(ErrorCode._404, "SAML identity-provider capability is not available",
            ...
            return completed(
                    Outcome.rejected(
                            new Outcome.Failure(ErrorCode._400,
                                    "SAML identity-provider capability type does not match the request",
                                    new JsonValue.ObjectValue(Map.of()))));
        }
         ...
         *
         * @return immutable SAML identity-provider manifest
         */
        @Override
        ...
        public <Q, S> CompletionStage<Outcome<S>> invoke(
            ...
            Assert.notNull(capability, "SAML identity-provider capability must not be null");
            Assert.notNull(context, "SAML identity-provider context must not be null");
            Assert.notNull(timeout, "SAML identity-provider timeout must not be null");
            ...
            if (capability == SamlServerScheme.SINGLE_LOGOUT) {
            ...
            if (capability == SamlServerScheme.METADATA) {
    ...
    private static final class XmlSigner {
        ...
        /**
         * Immutable identity-provider signing policy.
         */
        ...
        /**
         * Signs the required assertions and/or response according to identity-provider policy.
         *
            ...
            return resolve(context, timeout).thenApply(outcome -> switch (outcome) {
                case Outcome.Succeeded<KeyMaterial> success -> {
                    try {
                        ...
                        if (options.signAssertions() && !signed.assertions().isEmpty()) {
                            signed = signAssertions(signed, privateKey(success.value(), timeout));
                        }
                        if (options.signResponses() || signed.assertions().isEmpty())
        ...
        /**
         * Resolves the exact current identity-provider signing key.
         *
         * @param context immutable invocation context
         * @param timeout shared end-to-end timeout
        ...
        private CompletionStage<Outcome<KeyMaterial>> resolve(final Context context, final Timeout timeout) {
            ...
            final KeyLoader.Request query = new KeyLoader.Request(services.entry(), options.entityId(),
            ...
            try {
                ...
                return stage.handle(
                        (outcome, cause) -> cause == null && outcome != null ? outcome
                                : XmlSigner.<KeyMaterial>failed("SAML signing key resolution failed"));
            } catch (RuntimeException exception) {
        ...
        private PrivateKey privateKey(final KeyMaterial resolved, final Timeout timeout) {
            ...
            if (!options.signingKeyId().equals(resolved.keyId())
                    || !options.signatureAlgorithm().equals(resolved.algorithm())
                    || !(resolved.key() instanceof PrivateKey privateKey) || now.isBefore(resolved.notBefore())
                    || !now.isBefore(resolved.notAfter())) {
                throw new ValidateException("Resolved SAML signing key does not match identity-provider policy");
            }
            return privateKey;
        }
        ...
        private Response signAssertions(final Response response, final PrivateKey key) {
            ...
            for (Response.AssertionContent content : response.assertions()) {
                if (!(content instanceof Response.PlainAssertion plain)) {
                    throw new ValidateException("SAML identity-provider cannot emit an encrypted unsigned assertion");
                }
        ...
        private byte[] signature(final byte[] xml, final String id, final PrivateKey key) {
            try {
                ...
                final SignedInfo signedInfo = factory.newSignedInfo(
                        ...
                        factory.newSignatureMethod(options.signatureAlgorithm(), null),
                        List.of(reference));
