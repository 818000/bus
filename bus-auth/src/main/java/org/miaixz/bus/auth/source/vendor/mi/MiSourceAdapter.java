...
public class MiSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Xiaomi manifest.
     */
    private final VendorManifest.Variant variant;

    /**
     * Validated externally loaded Xiaomi options.
     */
    private final MiOptions options;
     ...
     *
     * @param spaceId  Source space used to isolate state and secret resolution
    ...
    public MiSourceAdapter(final String spaceId, final String sourceId, final MiManifest manifest,
        ...
        this.options = Assert.notNull(options, "Xiaomi options must not be null");
        this.services = Assert.notNull(services, "Xiaomi execution services must not be null");
        if (!MiManifest.ID.equals(selected.vendor()) || !selected.variant(MiManifest.DEFAULT).equals(variant)
                || !MiManifest.DEFAULT.equals(variant.variant()) || variant.protocol() != Protocol.OAUTH2
                || !MiManifest.ID.equals(options.vendor()) || !MiManifest.DEFAULT.equals(options.variant())
    ...
    /**
     * Returns the exact capability manifest frozen by the Xiaomi manifest.
     *
    ...
    private Outcome<TokenResponse> send(
        ...
        try {
            ...
            if (grant instanceof AuthorizationCodeGrant authorization) {
                ...
                target.query(OAuth2.Parameters.CLIENT_ID, options.clientId())
                        .query(OAuth2.Parameters.CLIENT_SECRET, secretValue)
    ...
    private Outcome<TokenResponse> token(final Response response) {
        ...
        try {
            ...
            final JsonValue value = JsonKit.readValue(json, Normal._32, true);
     ...
     */
    private Outcome<Identity> profileNow(final TokenResponse token, final Timeout timeout) {
    ...
    private String contactEndpoint() {
        final URI profile = URI.create(variant.targets().resolve(options).userInfo().getOrNull().url().toString());
    ...
    /**
     * Returns explicit scopes or immutable manifest defaults.
     *
