...
public class RevocationService {
    ...
    /**
     * Deletes only unexpired token entries owned by this authorization server and authenticated client.
     *
    ...
    private CompletionStage<Void> revokeAuthorization(
        ...
        final Instant now = timeout.clock().now();
        final String authorizationKey = AuthorizationCache.key(sourceId, authorizationId);
        return services.authorizationCache().find(authorizationKey).thenCompose(stored -> {
            ...
            final AuthorizationCache.Entry revoked = new AuthorizationCache.Entry(sourceId, clientId,
     ...
     *
     * @param authorizationKey Source-isolated authorization key
     * @param clientId         authenticated client identifier
     * @param timeout          shared operation timeout
    ...
    /**
     * Tests whether an access-token entry belongs to the current authorization server and client and remains active.
     *
    ...
    private boolean accessOwned(
            ...
            final String clientId,
            final Instant now) {
        return stored != null && stored.expiresAt().isAfter(now) && sourceId.equals(stored.value().sourceId())
                && clientId.equals(stored.value().clientId());
    }

    /**
     * Tests whether a refresh-token entry belongs to the current authorization server and client and remains active.
     *
