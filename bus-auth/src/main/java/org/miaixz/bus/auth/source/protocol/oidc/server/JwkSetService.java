...
/**
 * Publishes the validated public signing-key view of an OpenID Provider's external key set.
 * <p>
...
public class JwkSetService {
    ...
    /**
     * Resolves and validates the OpenID Provider's current public signing keys.
     *
    ...
    public CompletionStage<Outcome<JwkSet>> jwks(final Context context, final Timeout timeout) {
        ...
        try {
            final KeyLoader.Criteria criteria = new KeyLoader.Criteria(services.entry(), options.issuer(),
                    Builder.SIGNATURE, now);
            resolution = Outcome.mapStage(
                    () -> services.keyLoader().list(criteria, context, timeout),
                    listing -> services.keyParser().parsePublic(services.entry(), criteria, listing));
