...
public class SlackSourceAdapter implements VendorAdapter {
    ...
    private CompletionStage<Outcome<Url>> authorization(final AuthorizationRequest request) {
        if (!valid(request)) {
            return completed(rejected("Slack authorization request differs from the configured Source"));
        }
        try {
    ...
    private CompletionStage<Outcome<Identity>> profile(final TokenResponse token, final Timeout timeout) {
        return CompletableFuture.supplyAsync(() -> {
            if (!TokenType.BEARER.equals(token.tokenType())) {
                return SlackSourceAdapter.<Identity>rejected("Slack token response must use the Bearer token type");
            }
            final String userId;
            try {
                userId = userId(token);
                ...
                return SlackSourceAdapter
                        .<Identity>failed(ErrorCode._502, "Slack token response lacks a valid authorized user");
            }
            if (timeout.expired()) {
                return SlackSourceAdapter
                        .<Identity>failed(ErrorCode._408, "Slack profile request has no remaining timeout");
            }
            try {
                ...
                return SlackSourceAdapter.<Identity>failed(ErrorCode._502, "Slack profile request failed");
            }
        }, services.executor());
    }
     ...
     */
    private Outcome<Identity> profile(final Response response, final String expectedUserId, final Timeout timeout) {
        try {
