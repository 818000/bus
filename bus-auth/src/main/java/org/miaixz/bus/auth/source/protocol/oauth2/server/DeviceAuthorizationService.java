...
public class DeviceAuthorizationService {
    ...
    /**
     * Source identifier used to isolate device-code state.
     */
    private final String sourceId;

    /**
     * Frozen authorization-server options supplying grant, lifetime, interval, and verification URI policy.
     */
    ...
    /**
     * Number of secure random bytes required by the OAuth security rule.
     */
    private final int deviceCodeBytes;

    /**
     * Creates a device authorization service for one compiled OAuth authorization server.
     *
    ...
    public DeviceAuthorizationService(final String sourceId, final OAuth2ServerOptions options,
        ...
        final int entropyBits = Math
                .max(Normal._256, services.policies().require(Protocol.OAUTH2).minimumEntropyBits());
            ...
            return completed(
                    Outcome.rejected(
                            failure(
                                    ...
                                    "OAuth 2.x device authorization is disabled by the authorization server")));
        }
    ...
    private CompletionStage<Outcome<DeviceAuthorizationResponse>> create(
        ...
        final DeviceCodeCache.Entry entry = new DeviceCodeCache.Entry(sourceId, clientId, userCode, scope,
    ...
    /**
     * Produces a Source-isolated irreversible key for an opaque device code.
     *
    ...
    private String key(final String deviceCode) {
        return Builder.sha256Hex(sourceId + Symbol.C_NUL + deviceCode);
    }
