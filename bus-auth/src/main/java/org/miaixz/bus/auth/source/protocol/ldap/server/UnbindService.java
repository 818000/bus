...
import org.miaixz.bus.auth.source.protocol.ldap.UnbindRequest;
...
public class UnbindService {
    ...
    /**
     * Frozen LDAP server options retained for a uniform service construction contract.
     */
    ...
    /**
     * Creates an Unbind service for one compiled LDAP server.
     *
     ...
     */
    public UnbindService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Unbind Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Unbind LDAP server options must not be null");
    ...
    public CompletionStage<Outcome<Void>> unbind(
        ...
        try {
            stage = store.unbind(sourceId, connectionId, request, context, timeout);
