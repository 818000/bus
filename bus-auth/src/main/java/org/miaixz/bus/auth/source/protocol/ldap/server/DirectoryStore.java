 ...
 * {@code sourceId + connectionId} pair supplied by the framework. They enforce directory authorization, schema,
 * matching rules, alias behavior, atomic modifications, and Abandon correlation. Bus-auth retains the standard wire
 * models and applies server limits but does not supply an in-memory directory, hidden session map, or persistence
 * implementation.
 * </p>
 *
 * @author Kimi Liu
...
public interface DirectoryStore {
     ...
     *
     * @param sourceId     owning LDAP server Source identifier
     ...
     * @param sourceId     owning LDAP server Source identifier
     * @param connectionId stable trusted transport connection identifier
     * @param request      standard Search request already tightened to LDAP server limits
     * @param context      immutable invocation context carrying the same connection snapshot
     * @param timeout      shared end-to-end timeout
