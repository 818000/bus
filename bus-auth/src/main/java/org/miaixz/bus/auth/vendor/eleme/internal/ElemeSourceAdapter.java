...
import org.miaixz.bus.auth.vendor.eleme.ElemeSourceSettings;
...
public final class ElemeSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Eleme definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Eleme settings.
     */
    private final ElemeSourceSettings settings;
     ...
     * @param vendorDefinition  selected Eleme definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded Eleme settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if profile, variant, protocol, manifest, or settings routing is inconsistent
     */
    public ElemeSourceAdapter(final String namespaceId, final String sourceId, final ElemeDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final ElemeSourceSettings settings,
            final ExecutionServices services) {
        final ElemeDefinition selectedProfile = Assert.notNull(vendorDefinition, "Eleme definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Eleme Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Eleme definition must not be null");
        this.settings = Assert.notNull(settings, "Eleme settings must not be null");
        this.services = Assert.notNull(services, "Eleme execution services must not be null");
        if (!ElemeDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(ElemeDefinition.DEFAULT).equals(variantDefinition)
                || !ElemeDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !ElemeDefinition.ID.equals(settings.vendor())
                || !ElemeDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("Eleme adapter requires the eleme/default OAuth 2.0 definition");
        }
     ...
     *
     * @param definition      selected Eleme definition
    ...
    private static StandardAdapter standardAdapter(
            ...
            final ElemeSourceSettings settings,
            final ExecutionServices services,
    ...
    private static Outcome<TokenResponse> validateToken(final TokenResponse response) {
        try {
            if (!TokenType.BEARER.equals(response.tokenType()) || response.expiresIn().isEmpty()
                ...
                throw new ValidateException("Eleme token response violates the registered OAuth definition");
            }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected("Eleme capability is not declared by the selected definition"));
    }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected Eleme definition.
     *
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        final TokenRequest request = new TokenRequest(new AuthorizationCodeGrant(response.code(),
                settings.redirectUri(), Optional.of(settings.clientId()), Optional.empty()), emptyObject());
    ...
    private Outcome<TokenResponse> token(
        ...
        try {
            ...
            final String authorization = HttpAuth
                    .basic(formComponent(settings.clientId()), formComponent(new String(secret.material()))).value();
    ...
    private Outcome<ExternalIdentity> merchant(
        ...
        try {
            ...
            appKeyJson = services.jsonProvider().writeValue(new JsonValue.StringValue(settings.clientId()));
            ...
            metas.put("app_key", new JsonValue.StringValue(settings.clientId()));
