...
public record SamlServerOptions(String entityId, Endpoint singleSignOnServiceEndpoint,
    ...
    public SamlServerOptions {
        ...
        if (!signAssertions && !signResponses) {
            throw new ValidateException("SAML identity-provider must sign assertions, responses, or both");
        }
    ...
    /**
     * Validates one externally routed endpoint against the SAML identity-provider transport policy.
     *
    ...
    /**
     * Returns this already immutable SAML server configuration snapshot.
     *
     ...
     *
     * @return redacted SAML identity-provider options summary
     */
    @Override
