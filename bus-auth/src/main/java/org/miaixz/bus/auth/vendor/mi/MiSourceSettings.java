 ...
 * <p>
 * Fixed Xiaomi endpoints and all query-wire deviations remain definition- and adapter-owned. This record retains only
 * the public Client ID, external Client Secret reference, exact callback, and approved account scopes.
 * </p>
 *
 ...
 * @param scopes      ordered Xiaomi scopes, or empty to use definition defaults
 * @author Kimi Liu
 */
public record MiSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     *                                  invalid
     */
    public MiSourceSettings {
        if (!MiDefinition.ID.equals(vendor) || !MiDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Xiaomi settings must select mi/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Xiaomi settings require a registered redirect URI");
        }
    ...
    public String toString() {
        return "MiSourceSettings[vendor=" + vendor + ", variant=" + variant
