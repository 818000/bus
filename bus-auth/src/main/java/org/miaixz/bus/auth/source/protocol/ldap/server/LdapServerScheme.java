...
/**
 * Declares the standards-based LDAPv3 directory-server scheme.
 * <p>
 * Every Roster operation accepts the complete LDAPMessage so the message identifier and Controls survive the transport
 * boundary. Bind establishes connection authentication within the external DirectoryStore; consequently the protocol
 * capabilities are publicly reachable at the framework boundary while directory authorization remains connection-scoped
 * and operation-specific.
 * </p>
 *
 * @author Kimi Liu
 */
public class LdapServerScheme implements ProtocolScheme<LdapServerOptions> {
    ...
    private static final Form FORM = new Form(List.of(
            new Form.Section("ldap-directory-server", "LDAPv3 Directory Server",
                    List.of(
    ...
    /**
     * Creates the stateless LDAP server scheme used to validate and compile server Source configurations.
     */
    ...
    private static Capability<LdapMessage, LdapMessage> message(final Capability.Key key) {
        return new Capability<>(key, LdapMessage.class, LdapMessage.class, Capability.Direction.SERVER,
    ...
    private static Capability<LdapMessage, Void> empty(final Capability.Key key) {
        return new Capability<>(key, LdapMessage.class, Void.class, Capability.Direction.SERVER,
                Set.of(Capability.Interaction.DIRECT), Capability.Security.PUBLIC);
    }
    ...
    /**
     * Returns the stable LDAP server-role Source configuration type.
     *
     * @return {@value #ID}
     */
    @Override
    ...
    /**
     * Returns management presentation metadata for an LDAP directory server.
     *
     * @return immutable LDAP server metadata
     */
    @Override
    public Metadata metadata() {
        return new Metadata("LDAP Server", "Exposes framework identities through an LDAP server endpoint.", "ldap");
    }

    /**
     * Returns the LDAP directory-server category.
     *
    ...
    /**
     * Returns all ten standard LDAPv3 server capabilities.
     *
     * @return immutable LDAP server manifest
     */
    @Override
    ...
    /**
     * Returns the external management form for LDAP server options.
     *
     * @return immutable LDAP server form
     */
    @Override
    public Form form() {
        return FORM;
