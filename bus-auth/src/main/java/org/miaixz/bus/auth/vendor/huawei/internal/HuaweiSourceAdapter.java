...
import org.miaixz.bus.auth.vendor.huawei.HuaweiSourceSettings;
...
public final class HuaweiSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Huawei variant definition.
     */
     ...
     */
    private final HuaweiSourceSettings settings;

    /**
     * Caller-owned runtime, resolver, JSON, executor, clock, and Fabric dependencies.
     */
    ...
    /**
     * Creates one Source-bound Huawei adapter from the frozen default definition.
     *
     * @param namespaceId       registration namespace used for browser correlation isolation
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected Huawei definition
     ...
     *                                  profile
     */
    public HuaweiSourceAdapter(final String namespaceId, final String sourceId, final HuaweiDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final HuaweiSourceSettings settings,
            final ExecutionServices services) {
        Assert.notNull(vendorDefinition, "Huawei definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Huawei Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Huawei definition must not be null");
        this.settings = Assert.notNull(settings, "Huawei settings must not be null");
        this.services = Assert.notNull(services, "Huawei execution services must not be null");
        if (!HuaweiDefinition.ID.equals(vendorDefinition.type())
                || !vendorDefinition.variant(HuaweiDefinition.DEFAULT).equals(variantDefinition)
                || !HuaweiDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OIDC || !HuaweiDefinition.ID.equals(settings.vendor())
                || !HuaweiDefinition.DEFAULT.equals(settings.variant())) {
            throw new ValidateException("Huawei adapter requires the huawei/default OIDC definition");
        }
        ...
        this.standardAdapter = new StandardAdapter(variantDefinition, settings, Optional.of(redirectManager), List.of(
                ...
                new StandardAdapter.Binding<>(OpenIdSourceProfile.REVOCATION,
                        (request, ignored, timeout) -> revoke(request, timeout)),
                new StandardAdapter.Binding<>(OpenIdSourceProfile.DISCOVERY, (ignored, context, timeout) -> openIdClient
    ...
    /**
     * Routes only Huawei capabilities declared by the selected variant definition.
     *
    ...
    private CompletionStage<Outcome<ExternalIdentity>> complete(
        ...
        final TokenRequest request = new TokenRequest(new AuthorizationCodeGrant(values.code(), settings.redirectUri(),
                Optional.of(settings.clientId()), Optional.of(completion.codeVerifier().getOrNull().value())),
                emptyObject());
    ...
    private Outcome<PrivateToken> token(final HttpResponse response) {
        try {
            ...
            if (response.code() == Http.Status.OK) {
                ...
                if (!settings.scopes().equals(scope.values())) {
    ...
    /**
     * Binds every security-relevant Huawei metadata value to the compiled definition.
     *
    ...
    private Outcome<OpenIdProviderMetadata> metadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            if (!resolvedTargets.authorization().getOrNull().url().toString().equals(metadata.authorizationEndpoint())
                    ...
                    || !metadata.scopesSupported().containsAll(settings.scopes())
                ...
                throw new ValidateException("Huawei Discovery metadata differs from the frozen definition");
            }
