...
import org.miaixz.bus.auth.vendor.vk.VkSourceSettings;
...
public final class VkSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable VK definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded VK settings.
     */
    private final VkSourceSettings settings;
     ...
     *
     * @param namespaceId       registration namespace used to isolate browser state
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected VK Source definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded VK settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if routing, protocol, definition, callback, or PKCE settings are inconsistent
     */
    public VkSourceAdapter(final String namespaceId, final String sourceId, final VkDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final VkSourceSettings settings,
            final ExecutionServices services) {
        final VkDefinition selected = Assert.notNull(vendorDefinition, "VK definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "VK Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "VK definition must not be null");
        this.settings = Assert.notNull(settings, "VK settings must not be null");
        this.services = Assert.notNull(services, "VK execution services must not be null");
        if (!VkDefinition.ID.equals(selected.type())
                || !selected.variant(VkDefinition.DEFAULT).equals(variantDefinition)
                || !VkDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !VkDefinition.ID.equals(settings.vendor())
                || !VkDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("VK adapter requires the vk/default OAuth 2.0 definition with S256 PKCE");
        }
    ...
    /**
     * Creates an empty provider-neutral JSON object.
     *
    ...
    /**
     * Creates one provider-neutral JSON integer.
     *
     ...
     * @param error       shared Bus error code
     * @param description non-sensitive failure description
     * @param details     provider-neutral safe details
     * @param <T>         expected success type
     * @return failed outcome
     */
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected("VK capability is not declared by the selected definition"));
        }
        ...
        if (capability.equals(OAuth2SourceProfile.TOKEN) && request instanceof TokenRequest token) {
            return narrow(token(token, timeout), capability.responseType());
        }
        if (capability.equals(OAuth2SourceProfile.REVOCATION) && request instanceof RevocationRequest revocation) {
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        final AuthorizationCodeGrant grant = new AuthorizationCodeGrant(values.code(), settings.redirectUri(),
    ...
    private List<Parameter> tokenParameters(final TokenRequest request) {
        ...
        if (request.grant() instanceof AuthorizationCodeGrant grant) {
            if (!settings.redirectUri().equals(grant.redirectUri())
                    || !Optional.of(settings.clientId()).equals(grant.clientId()) || grant.codeVerifier().isEmpty()) {
                throw new ValidateException("VK authorization-code grant registration values are invalid");
            }
        ...
        if (request.grant() instanceof RefreshTokenGrant) {
            ...
            result.add(new Parameter(OAuth2.Parameters.CLIENT_ID, settings.clientId()));
    ...
    private CompletionStage<Outcome<ExternalIdentity>> profile(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                return VkSourceAdapter.<ExternalIdentity>failed(ErrorCode._502, "VK current-user request failed");
            } finally {
                clear(body);
            }
        }, services.executor());
     ...
     * @return expected rejection or upstream failure
     */
    private Outcome<ExternalIdentity> profileError(final int status, final JsonValue.ObjectValue object) {
        if (!members(WireKind.ERROR, object) || object.values().isEmpty()) {
