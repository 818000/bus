...
/**
 * Publishes RFC 8414 authorization server metadata derived solely from one validated authorization-server policy.
 * <p>
 * The service reports only endpoints and features that the compiled OAuth authorization server can actually execute.
 * Deployment information that is not represented by {@link OAuth2ServerOptions}, including JWK Set, documentation,
 * policy, signed metadata, dynamic registration, and DPoP transport support, remains absent.
 * </p>
 *
 * @author Kimi Liu
 */
public class AuthorizationServerMetadataService {
    ...
    /**
     * Immutable authorization-server options used as the single metadata source of truth.
     */
    private final OAuth2ServerOptions options;

    /**
     * Creates a metadata service for one validated OAuth authorization server.
     *
     * @param options validated immutable authorization-server options
    ...
    /**
     * Returns the exact standard metadata currently implemented by this authorization server.
     *
