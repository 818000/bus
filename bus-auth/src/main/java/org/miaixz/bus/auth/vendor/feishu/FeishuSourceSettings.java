 ...
 * <p>
 * Fixed endpoints, mandatory S256 PKCE, and v3 JSON wire behavior remain definition-owned. This record retains only the
 ...
 * @param redirectUri exact Security Settings redirect URI
 * @param scopes      ordered requested scopes; empty means no explicit scope
 * @author Kimi Liu
 */
public record FeishuSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     * @throws ValidateException        if routing, credential, callback, or scope differs from the frozen v3 definition
     */
    public FeishuSourceSettings {
        if (!FeishuDefinition.ID.equals(vendor) || !FeishuDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Feishu settings must select feishu/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Feishu settings require a registered redirect URI");
        }
    ...
    public String toString() {
        return "FeishuSourceSettings[vendor=" + vendor + ", variant=" + variant
