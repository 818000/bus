...
/**
 * Carries externally managed Figma.com OAuth app values.
 * <p>
 * Fixed Figma endpoints and mandatory S256 PKCE remain definition-owned. This record retains only the public Client ID,
 ...
 * @param variant     exact default Figma.com variant
 * @param clientId    registered Figma OAuth Client ID
 * @param credential  external Client Secret reference
 * @param redirectUri exact registered OAuth callback URI
 * @param scopes      ordered Figma OAuth scopes
 * @author Kimi Liu
 */
public record FigmaSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes) implements VendorSettings {

    /**
     * Minimum identity scope required by the frozen Figma login definition.
     */
    private static final String CURRENT_USER_READ = "current_user:read";

    /**
     * Validates and freezes one Figma registration without resolving its Client Secret.
     *
     ...
     */
    public FigmaSourceSettings {
        if (!FigmaDefinition.ID.equals(vendor) || !FigmaDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Figma settings must select figma/default");
        }
        Assert.notBlank(clientId, "Figma client id must not be blank");
        Assert.notNull(credential, "Figma credential reference must not be null");
        if (credential.type() != Credential.Type.CLIENT_SECRET) {
            throw new ValidateException("Figma credential must reference a client secret");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Figma settings require a registered redirect URI");
        }
    ...
    public String toString() {
        return "FigmaSourceSettings[vendor=" + vendor + ", variant=" + variant
