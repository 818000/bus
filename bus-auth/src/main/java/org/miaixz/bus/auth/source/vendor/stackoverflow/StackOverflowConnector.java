...
/**
 * Connects the complete StackOverflow platform manifest and its exact variant factories.
 *
 * @author Kimi Liu
 */
public class StackOverflowConnector implements VendorConnector {

    /**
     * Creates a stateless StackOverflow SPI connector.
     */
    ...
    /**
     * Returns the stable StackOverflow platform key.
     *
     * @return stable StackOverflow platform key
     */
    @Override
    public Vendor.Id key() {
        return StackOverflowManifest.ID;
    }

    /**
     * Binds the StackOverflow manifest, Options factory, and all adapter factories as one registration.
     *
    ...
    public void connect(final VendorRegistry registry) {
        Assert.notNull(registry, "Vendor registry must not be null").bind(
                ...
                (variant, clientId, credential, callback, scopes, parameters) -> new StackOverflowOptions(
                        ...
                        VendorRegistry.requiredString(parameters, "key"),
                        VendorRegistry.requiredString(parameters, "siteId")),
                List.of(
                        VendorRegistry.adapter(
                                StackOverflowManifest.class,
                                StackOverflowSourceAdapter::new,
