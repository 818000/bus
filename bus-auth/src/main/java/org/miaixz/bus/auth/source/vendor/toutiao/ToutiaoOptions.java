 ...
 * @author Kimi Liu
 */
public record ToutiaoOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
     ...
     */
    public ToutiaoOptions {
        if (!ToutiaoManifest.ID.equals(vendor) || !ToutiaoManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("Toutiao options must select toutiao/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Toutiao options require a registered redirect URI");
        }
