...
import org.miaixz.bus.auth.protocol.ldap.AbandonRequest;
...
public final class AbandonService {
    ...
    /**
     * Creates an Abandon service for one compiled LDAP Provider.
     *
     ...
     */
    public AbandonService(final String providerId, final LdapProviderSettings settings, final DirectoryStore store) {
        this.providerId = Assert.notBlank(providerId, "LDAP Abandon Provider id must not be blank");
        this.settings = Assert.notNull(settings, "LDAP Abandon Provider settings must not be null");
    ...
    public CompletionStage<Outcome<Void>> abandon(
        ...
        try {
            stage = store
                    .abandon(providerId, context.network().connection().getOrThrow().id(), request, context, timeout);
