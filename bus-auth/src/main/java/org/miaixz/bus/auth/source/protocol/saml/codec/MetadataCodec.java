/*
 ...
 ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
*/
package org.miaixz.bus.auth.source.protocol.saml.codec;
 ...
 * violations. The original document bytes and its unique ID index are retained for signature validation.
 * </p>
 *
 * @author Kimi Liu
 */
public class MetadataCodec {
    ...
    private static List<byte[]> endpointChildren(final Element root) {
        ...
        for (Element child : children(root)) {
            if (Saml.Namespaces.METADATA.equals(child.getNamespaceURI())) {
    ...
    private static Element writeIdentityProvider(final org.w3c.dom.Document document, final IdpSsoDescriptor value) {
        ...
        appendOptional(root, value.signature(), Saml.Namespaces.SIGNATURE, "Signature");
        extensions(root, value.extensions());
        for (KeyDescriptor key : value.keys())
            root.appendChild(writeKey(document, key));
        appendOptional(root, value.organization(), Saml.Namespaces.METADATA, "Organization");
        appendAll(root, value.contacts(), Saml.Namespaces.METADATA, "ContactPerson", false);
    ...
    private static Element writeServiceProvider(final org.w3c.dom.Document document, final SpSsoDescriptor value) {
        ...
        appendAll(
                root,
                value.attributeConsumingServices(),
                Saml.Namespaces.METADATA,
    ...
    private static Element writeKey(final org.w3c.dom.Document document, final KeyDescriptor value) {
        ...
        append(root, value.keyInfo().xml(), Saml.Namespaces.SIGNATURE, "KeyInfo", false);
        appendAll(
                root,
    ...
    private static void append(
        ...
        if (foreign && Saml.Namespaces.METADATA.equals(source.getNamespaceURI())) {
    ...
    private static Element metadata(final org.w3c.dom.Document document, final String localName) {
        return document.createElementNS(Saml.Namespaces.METADATA, "md:" + localName);
    }
    ...
    private static Element signature(final org.w3c.dom.Document document, final String localName) {
        return document.createElementNS(Saml.Namespaces.SIGNATURE, "ds:" + localName);
    }
     ...
     * @return immutable typed descriptor, original bytes, and unique ID index
    ...
    public SamlMessageCodec.Document<EntityDescriptor> decode(final byte[] xml) {
        ...
        require(root, Saml.Namespaces.METADATA, "EntityDescriptor");
    ...
    public byte[] encode(final EntityDescriptor descriptor) {
        ...
        appendAll(
                root,
                value.additionalMetadataLocations(),
                Saml.Namespaces.METADATA,
    ...
    private EntityDescriptor entity(final Element root) {
        ...
        for (Element child : children(root)) {
            final int current;
            if (is(child, Saml.Namespaces.SIGNATURE, "Signature")) {
                current = 1;
                if (signature != null)
                    duplicate("EntityDescriptor Signature");
                signature = fragment(child);
            } else if (is(child, Saml.Namespaces.METADATA, "Extensions")) {
                current = 2;
                if (!extensions.isEmpty())
                    duplicate("EntityDescriptor Extensions");
                extensions.addAll(extensionChildren(child));
            } else if (is(child, Saml.Namespaces.METADATA, "IDPSSODescriptor")) {
                current = 3;
                identityProviders.add(identityProvider(child));
            } else if (is(child, Saml.Namespaces.METADATA, "SPSSODescriptor")) {
                current = 3;
                serviceProviders.add(serviceProvider(child));
            } else if (is(child, Saml.Namespaces.METADATA, "Organization")) {
                current = 4;
                if (organization != null)
                    duplicate("EntityDescriptor Organization");
                organization = fragment(child);
            } else if (is(child, Saml.Namespaces.METADATA, "ContactPerson")) {
                current = 5;
                contacts.add(fragment(child));
            } else if (is(child, Saml.Namespaces.METADATA, "AdditionalMetadataLocation")) {
                current = 6;
    ...
    private IdpSsoDescriptor identityProvider(final Element root) {
        ...
        for (Element child : children(root)) {
            ...
            if (is(child, Saml.Namespaces.SIGNATURE, "Signature")) {
                ...
                extensions.addAll(extensionChildren(child));
            } else if (is(child, Saml.Namespaces.METADATA, "KeyDescriptor")) {
                current = 3;
                keys.add(key(child));
            } else if (is(child, Saml.Namespaces.METADATA, "Organization")) {
                current = 4;
                ...
                current = 5;
                contacts.add(fragment(child));
            } else if (is(child, Saml.Namespaces.METADATA, "SingleLogoutService")) {
                current = 6;
                logout.add(logoutEndpoint(child));
            } else if (is(child, Saml.Namespaces.METADATA, "NameIDFormat")) {
                current = 7;
                formats.add(requiredContent(child, "NameIDFormat"));
            } else if (is(child, Saml.Namespaces.METADATA, "SingleSignOnService")) {
                current = 8;
    ...
    private SpSsoDescriptor serviceProvider(final Element root) {
        ...
        for (Element child : children(root)) {
            ...
            if (is(child, Saml.Namespaces.SIGNATURE, "Signature")) {
                ...
                current = 7;
                formats.add(requiredContent(child, "NameIDFormat"));
            } else if (is(child, Saml.Namespaces.METADATA, "AssertionConsumerService")) {
                current = 8;
                consumers.add(consumerEndpoint(child));
            } else if (is(child, Saml.Namespaces.METADATA, "AttributeConsumingService")) {
                current = 9;
    ...
    private KeyDescriptor key(final Element root) {
        ...
        for (Element child : children(root)) {
            final int current;
            if (is(child, Saml.Namespaces.SIGNATURE, "KeyInfo")) {
                current = 1;
                if (keyInfo != null)
                    duplicate("KeyDescriptor KeyInfo");
                keyInfo = fragment(child);
            } else if (is(child, Saml.Namespaces.METADATA, "EncryptionMethod")) {
                current = 2;
     ...
     * @return immutable unique ID index
     */
     ...
     * @param ids     mutable document-wide ID index
     */
