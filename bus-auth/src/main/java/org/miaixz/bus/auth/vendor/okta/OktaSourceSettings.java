 ...
 * authorization-server identifier are validated before the definition substitutes them into its fixed Okta endpoint and
 * issuer templates.
 * </p>
 *
 ...
 * @param scopes                ordered OIDC scopes, or empty to use the definition defaults
 ...
 * @author Kimi Liu
 */
public record OktaSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, String instance,
        String authorizationServerId) implements VendorSettings {
     ...
     */
    public OktaSourceSettings {
        if (!OktaDefinition.ID.equals(vendor) || !OktaDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Okta settings must select okta/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Okta settings require a Sign-in redirect URI");
        }
    ...
    /**
     * Returns the organization label consumed by definition-owned host templates.
     *
    ...
    /**
     * Returns the identifier consumed by definition-owned authorization-server path templates.
     *
    ...
    public String toString() {
        return "OktaSourceSettings[vendor=" + vendor + ", variant=" + variant
