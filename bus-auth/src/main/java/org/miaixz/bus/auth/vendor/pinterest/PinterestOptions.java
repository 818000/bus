 ...
 * <p>
 * Fixed endpoints, default scopes, and platform wire deviations remain manifest-owned. This immutable record retains
 ...
 * @param scopes      ordered requested Pinterest scopes, or empty to use the manifest default
 * @author Kimi Liu
 */
public record PinterestOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId,
    ...
    public PinterestOptions {
        if (!PinterestManifest.ID.equals(vendor) || !PinterestManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("Pinterest options must select pinterest/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Pinterest options require a registered redirect URI");
        }
