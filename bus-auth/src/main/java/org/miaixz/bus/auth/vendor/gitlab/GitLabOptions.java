...
public record GitLabOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
    ...
    public GitLabOptions {
        if (!GitLabManifest.ID.equals(vendor) || !GitLabManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("GitLab options must select gitlab/default");
        }
        ...
        for (String scope : scopes) {
            ...
            if (!"read_user".equals(checked) || copy.contains(checked)) {
                throw new ValidateException("GitLab scope is unsupported or duplicated by the frozen login manifest");
            }
            copy.add(checked);
        }
        if (copy.isEmpty()) {
    ...
    public String toString() {
        return "GitLabOptions[vendor=" + vendor + ", variant=" + variant
