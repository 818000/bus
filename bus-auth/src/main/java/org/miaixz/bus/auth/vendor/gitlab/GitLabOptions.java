...
public record GitLabOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
    ...
    /**
     * Preserves the historical six-argument GitLab login construction surface.
     *
     * @param vendor      exact GitLab platform identifier
     * @param variant     exact login Variant
     * @param clientId    registered OAuth application ID
     * @param credential  external Client Secret reference
     * @param redirectUri exact OAuth redirect
     * @param scopes      ordered login scopes
     */
    public GitLabOptions(final Vendor.Id vendor, final Vendor.Variant variant, final String clientId,
            final Credential.Reference credential, final Optional<String> redirectUri, final List<String> scopes) {
        this(vendor, variant, clientId, credential, redirectUri, scopes, Normal.EMPTY, Normal.EMPTY);
    }
