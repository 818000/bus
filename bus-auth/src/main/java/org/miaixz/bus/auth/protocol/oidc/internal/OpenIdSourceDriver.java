 ...
 * @author Kimi Liu
 */
public final class OpenIdSourceDriver implements SourceDriver<OpenIdClientSettings> {
     ...
     */
    private static void validateAlgorithms(final OpenIdClientSettings settings, final ExecutionServices services) {
        final var allowed = services.securityBaseline().require(Protocol.OIDC).algorithms();
        for (var algorithm : settings.idTokenSigningAlgorithms()) {
     ...
     *
     * @param settings composed OAuth client settings
    ...
    private static Optional<IntrospectionClient> optionalIntrospection(
            final OAuth2ClientSettings settings,
            final ExecutionServices services) {
        return settings.introspectionEndpoint().isEmpty() ? Optional.empty()
                : Optional.of(
    ...
    private static Optional<RevocationClient> optionalRevocation(
            final OAuth2ClientSettings settings,
            final ExecutionServices services) {
        return settings.revocationEndpoint().isEmpty() ? Optional.empty()
                : Optional.of(new RevocationClient(settings, services, new RevocationRequestEncoder()));
    }
    ...
    private static Optional<DeviceAuthorizationClient> optionalDeviceAuthorization(
            final OAuth2ClientSettings settings,
            final ExecutionServices services) {
        return settings.deviceAuthorizationEndpoint().isEmpty() ? Optional.empty()
                : Optional.of(
    ...
    private static Optional<AuthorizationServerMetadataClient> optionalMetadata(
            final OAuth2ClientSettings settings,
            final ExecutionServices services) {
        return settings.authorizationServerMetadataEndpoint().isEmpty() ? Optional.empty()
                : Optional.of(
     ...
     *
     * @param settings OpenID Connect Source settings
     * @param services runtime dependencies
     * @return normalized optional client
     */
    private static Optional<UserInfoClient> optionalUserInfo(
            final OpenIdClientSettings settings,
            final ExecutionServices services) {
        return settings.userInfoEndpoint().isEmpty() ? Optional.empty()
                : Optional.of(new UserInfoClient(settings, services, new UserInfoCodec(services.jsonProvider())));
    }
     ...
     * @return normalized optional client
     */
    private static Optional<EndSessionClient> optionalEndSession(final OpenIdClientSettings settings) {
        return settings.endSessionEndpoint().isEmpty() ? Optional.empty()
                : Optional.of(new EndSessionClient(settings, new EndSessionRequestCodec()));
    }
     ...
     */
    private static Capability.Manifest manifest(final OpenIdClientSettings settings) {
        final List<Capability<?, ?>> capabilities = new ArrayList<>();
        capabilities.add(OpenIdSourceProfile.AUTHENTICATION);
        capabilities.add(OpenIdSourceProfile.TOKEN);
        if (settings.oauth2Settings().introspectionEndpoint().isPresent()) {
            capabilities.add(OpenIdSourceProfile.INTROSPECTION);
        }
        if (settings.oauth2Settings().revocationEndpoint().isPresent()) {
            capabilities.add(OpenIdSourceProfile.REVOCATION);
        }
        if (settings.oauth2Settings().deviceAuthorizationEndpoint().isPresent()) {
            capabilities.add(OpenIdSourceProfile.DEVICE_AUTHORIZATION);
        }
        if (settings.oauth2Settings().authorizationServerMetadataEndpoint().isPresent()) {
            capabilities.add(OpenIdSourceProfile.AUTHORIZATION_SERVER_METADATA);
        }
        capabilities.add(OpenIdSourceProfile.DISCOVERY);
        capabilities.add(OpenIdSourceProfile.JWK_SET);
        if (settings.userInfoEndpoint().isPresent()) {
            capabilities.add(OpenIdSourceProfile.USERINFO);
        }
        if (settings.endSessionEndpoint().isPresent()) {
            capabilities.add(OpenIdSourceProfile.END_SESSION);
        }
    ...
    /**
     * Returns the OpenID relying-party profile bound to this driver.
     *
     * @return immutable OpenID Source profile
     */
    @Override
    public OpenIdSourceProfile profile() {
        return new OpenIdSourceProfile();
    }

    /**
     * Consumes typed settings and assembles only operations backed by declared endpoints.
     *
     ...
     * @throws ValidateException        if routing, settings, or signing policy is invalid
     */
    @Override
    public RuntimeProvider compile(
        ...
        final OpenIdClientSettings settings = decode(source);
        validateAlgorithms(settings, services);

        final OAuth2ClientSettings oauth = settings.oauth2Settings();
        ...
        final OpenIdClient client = new OpenIdClient(oauthClient,
                ...
                new DiscoveryClient(settings, services, new OpenIdProviderMetadataCodec(services.jsonProvider())),
                optionalUserInfo(settings, services), optionalEndSession(settings));
        return new CompiledSource(manifest(settings), client,
                new JwkSetOperation(settings, services, new JwkSetCodec(services.jsonProvider())));
    }
    ...
    private static final class JwkSetOperation {
         ...
         *
         * @param settings validated OpenID Connect Source settings
         * @param services externally owned runtime dependencies
         * @param codec    strict public-key resource codec
         */
        private JwkSetOperation(final OpenIdClientSettings settings, final ExecutionServices services,
                final JwkSetCodec codec) {
            this.endpoint = Assert.notNull(
                    settings.jwkSetEndpoint().getOrNull(),
    ...
    private static final class CompiledSource implements RuntimeProvider {
        ...
        public <Q, S> CompletionStage<Outcome<S>> invoke(
            ...
            if (capability.equals(OpenIdSourceProfile.USERINFO)) {
                return narrow(
                        client.userInfo(UserInfoRequest.class.cast(request), context, timeout),
                        capability.responseType());
            }
            if (capability.equals(OpenIdSourceProfile.END_SESSION)) {
                return narrow(
