 ...
 * target, and launch URL template syntax. Placeholder values are interpreted only by an integrating project.
 * </p>
 *
 * @author Kimi Liu
 */
public final class LibraryValidator {
     ...
     */
    public List<EntityViolation> validate(final Library library) {
        ...
        check(issues, "code", () -> validateCode(library.getCode()));
        check(
                issues,
                "name",
                () -> Validator
                        .validateTrue(StringKit.isNotBlank(library.getName()), "Library name must not be blank"));
        check(issues, "target", () -> {
        ...
        check(issues, "icon", () -> validateLocation(library.getIcon(), true, "icon"));
        check(issues, "url", () -> validateLocation(replacePlaceholders(library.getUrl()), false, "launch URL"));
        return List.copyOf(issues);
    }

    private static void check(final List<EntityViolation> issues, final String field, final Runnable validation) {
        try {
            validation.run();
        } catch (RuntimeException cause) {
            final String message = StringKit.isBlank(cause.getMessage()) ? "Library field is invalid"
                    : cause.getMessage();
            issues.add(new EntityViolation(field, ErrorCode._100101, message));
        }
    }
