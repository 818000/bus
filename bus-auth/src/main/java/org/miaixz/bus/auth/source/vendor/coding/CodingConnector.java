...
/**
 * Connects the complete Coding platform manifest and its exact variant factories.
 *
 * @author Kimi Liu
 */
public class CodingConnector implements VendorConnector {

    /**
     * Creates a stateless Coding SPI connector.
     */
    ...
    /**
     * Returns the stable Coding platform key.
     *
     * @return stable Coding platform key
     */
    @Override
    public Vendor.Id key() {
        return CodingManifest.ID;
    }

    /**
     * Binds the Coding manifest, Options factory, and all adapter factories as one registration.
     *
    ...
    public void connect(final VendorRegistry registry) {
        Assert.notNull(registry, "Vendor registry must not be null").bind(
                ...
                (variant, clientId, credential, callback, scopes, parameters) -> new CodingOptions(variant.platform(),
                        ...
                        VendorRegistry.requiredString(parameters, "team")),
                List.of(
                        VendorRegistry
                                .adapter(CodingManifest.class, CodingSourceAdapter::new, CodingManifest.DEFAULT)));
    }
