...
import org.miaixz.bus.auth.vendor.aliyun.AliyunSourceSettings;
...
public final class AliyunSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Alibaba Cloud variant definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Alibaba Cloud Source settings.
     */
    private final AliyunSourceSettings settings;
    ...
    /**
     * Creates one Source-bound Alibaba Cloud adapter from an immutable Vendor definition.
     *
     * @param namespaceId       registration namespace used to isolate browser state and credentials
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected Alibaba Cloud definition
     * @param variantDefinition selected default variant definition
     * @param settings          decoded externally loaded Alibaba Cloud settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a required collaborator is {@code null}
     * @throws ValidateException        if the supplied profile, definition, settings, or security baseline does not
     *                                  represent the frozen Alibaba Cloud OIDC variant
     */
    public AliyunSourceAdapter(final String namespaceId, final String sourceId, final AliyunDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final AliyunSourceSettings settings,
            final ExecutionServices services) {
        Assert.notNull(vendorDefinition, "Alibaba Cloud definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Alibaba Cloud Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Alibaba Cloud definition must not be null");
        this.settings = Assert.notNull(settings, "Alibaba Cloud settings must not be null");
        this.services = Assert.notNull(services, "Alibaba Cloud execution services must not be null");
        if (!AliyunDefinition.ID.equals(vendorDefinition.type())
                || !vendorDefinition.variant(AliyunDefinition.DEFAULT).equals(variantDefinition)
                || !AliyunDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OIDC || !settings.vendor().equals(AliyunDefinition.ID)
                || !settings.variant().equals(AliyunDefinition.DEFAULT)) {
            throw new ValidateException("Alibaba Cloud adapter requires the aliyun/default OIDC definition");
        }
     ...
     *
     * @param definition      selected Alibaba Cloud definition
     * @param settings        validated Alibaba Cloud Source settings
     * @param services        caller-owned execution services
    ...
    private static StandardAdapter standardAdapter(
            ...
            final AliyunSourceSettings settings,
            final ExecutionServices services,
            final RedirectManager redirectManager) {
        final var targets = definition.targets().resolve(settings);
        final String redirectUri = settings.redirectUri().orElseThrow(
        ...
        final OAuth2ClientSettings oauthSettings = new OAuth2ClientSettings(targets.authorization(), targets.token(),
                ...
                Optional.of(ISSUER), settings.clientId(), Set.of(redirectUri),
                Endpoint.Authentication.CLIENT_SECRET_POST, Optional.of(settings.credential()), true, false);
        ...
        return new StandardAdapter(definition, settings, Optional.of(redirectManager),
                List.of(
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.AUTHENTICATION, openIdClient::authorize),
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.TOKEN, openIdClient::token),
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.REVOCATION, openIdClient::revoke),
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.DISCOVERY,
                                (ignored, context, timeout) -> openIdClient.discover(context, timeout)),
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.USERINFO, openIdClient::userInfo)));
    }
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> token(
        ...
        return standardAdapter.invoke(OpenIdSourceProfile.TOKEN, request, context, timeout)
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        try {
            ...
            return standardAdapter.invoke(OpenIdSourceProfile.AUTHENTICATION, authentication, context, timeout)
                    .thenApply(outcome -> switch (outcome) {
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        final TokenRequest tokenRequest = new TokenRequest(new AuthorizationCodeGrant(response.code(),
                settings.redirectUri(), Optional.empty(), Optional.of(verifier)), emptyObject());
        return standardAdapter.invoke(OpenIdSourceProfile.TOKEN, tokenRequest, context, timeout)
    ...
    private CompletionStage<Outcome<ExternalIdentity>> verifyToken(
        ...
        try {
            ...
            return completed(rejected("Alibaba Cloud token response does not satisfy the registered OIDC definition"));
        }
     ...
     * frozen Alibaba Cloud definition.
     *
     * @param context immutable invocation context
     * @param timeout shared end-to-end budget
    ...
    private CompletionStage<Outcome<OpenIdProviderMetadata>> discover(
            final Context context,
            final Timeout.Budget timeout) {
        return standardAdapter.invoke(OpenIdSourceProfile.DISCOVERY, null, context, timeout)
                .thenApply(outcome -> switch (outcome) {
    ...
    private Outcome<OpenIdProviderMetadata> metadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            if (!resolvedTargets.authorization().getOrNull().url().toString().equals(metadata.authorizationEndpoint())
                ...
                throw new ValidateException("Alibaba Cloud discovery metadata differs from the frozen definition");
            }
    ...
    /**
     * Enforces the token success constraints required by the frozen Alibaba Cloud OIDC definition.
     *
    ...
    /**
     * Returns the explicit registered scopes or the immutable definition defaults.
     *
