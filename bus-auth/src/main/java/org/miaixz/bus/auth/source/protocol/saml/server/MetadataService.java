 ...
 * <p>
 * The certificate set is resolved externally on every publication operation so key rotation is reflected without
 * rebuilding the compiled Source. The Metadata codec converts the trusted Bus certificate chain into standard
...
public class MetadataService {
    ...
    public CompletionStage<Outcome<EntityDescriptor>> metadata(final Context context, final Timeout timeout) {
        ...
        final CertificateLoader.Request query = new CertificateLoader.Request(services.entry(), options.entityId(),
                Builder.SIGNING, timeout.clock().now());
        return Outcome
                .mapStage(
                        () -> services.certificateLoader().load(query, context, timeout),
                        loaded -> services.certificateParser().parse(services.entry(), query, loaded))
    ...
    private EntityDescriptor descriptor(final CertificateMaterial certificate) {
        ...
        final IdpSsoDescriptor identityProvider = new IdpSsoDescriptor(Optional.empty(), Optional.empty(),
                Optional.empty(), List.of(Saml.Namespaces.PROTOCOL), Optional.empty(), Optional.empty(), List.of(),
