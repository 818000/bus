...
public class BindService {
    ...
    /**
     * Frozen LDAP server Bind and transport options.
     */
    ...
    /**
     * Creates a Bind service for one compiled LDAP server.
     *
     ...
     */
    public BindService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Bind Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Bind LDAP server options must not be null");
    ...
    public CompletionStage<Outcome<LdapMessage>> bind(
        ...
        if (request.version() != Ldap.VERSION_3) {
            return completed(
        ...
        try {
            stage = store.bind(sourceId, connection.id(), request, context, timeout);
    ...
    /**
     * Validates the selected standard authentication choice against LDAP server options and transport confidentiality.
     *
