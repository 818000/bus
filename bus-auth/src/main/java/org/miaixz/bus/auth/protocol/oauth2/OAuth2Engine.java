...
public final class OAuth2Engine implements OAuth2.Engine {
     ...
     */
    public OAuth2Engine(final Policy policy, final OAuth2Dependencies dependencies, final StateStore states,
            final SubjectResolver subjects, final SecretResolver secrets, final Clock clock, final SecureRandom random,
            final JsonProvider json, final Limits limits, final KeyResolver keys) {
        final Policy trusted = Assert.notNull(policy, "OAuth policy must be not null!");
        authorizationCode = new AuthorizationCodeFlow(trusted, dependencies, states, subjects, secrets, clock, random,
                json, limits);
        clientCredentials = new ClientCredentialsFlow(trusted, dependencies, states, secrets, clock, random, json,
                limits);
        refreshToken = new RefreshTokenFlow(trusted, dependencies, states, secrets, clock, random, json, limits);
        deviceAuthorization = new DeviceAuthorizationFlow(trusted, dependencies, states, subjects, secrets, clock,
                random, json, limits);
        pushedAuthorization = new PushedAuthorizationRequest(trusted, dependencies, states, secrets, random, json,
                limits);
        jwtRequest = new JwtAuthorizationRequest(trusted, dependencies, json, clock, keys, states, limits);
        jwtResponse = new JwtAuthorizationResponse(trusted, keys, clock, random, json, limits);
        tokenIntrospection = new TokenIntrospection(dependencies, states, secrets, clock, json, limits);
        tokenRevocation = new TokenRevocation(dependencies, states, secrets, clock, json, limits);
    }
    ...
    private static CompletionStage<Outcome<Void>> completion(final Supplier<CompletionStage<Void>> operation) {
        ...
        return stage.handle((value, failure) -> failure == null ? Outcome.completed() : failure(failure));
    }
    ...
    private static <T> Outcome<T> failure(final Throwable failure) {
        ...
        if (cause instanceof ProtocolException protocol) {
            ...
            if (error == null) {
                return new Failed<>(new Failure(Kind.INTERNAL, ProtocolError.TEMPORARILY_UNAVAILABLE, true), cause);
            }
            final Kind kind = kind(error);
     ...
     */
    private static Kind kind(final ProtocolError error) {
        return switch (error) {
            case INVALID_CLIENT -> Kind.AUTHENTICATION;
            case ACCESS_DENIED, UNAUTHORIZED_CLIENT -> Kind.AUTHORIZATION;
            case SLOW_DOWN -> Kind.RATE_LIMIT;
            case AUTHORIZATION_PENDING, EXPIRED_TOKEN, INVALID_GRANT, INVALID_REQUEST_URI -> Kind.CONFLICT;
            case TEMPORARILY_UNAVAILABLE -> Kind.REMOTE;
     ...
     */
    @Override
    public CompletionStage<Outcome<Outbound>> authorize(
     ...
     */
    @Override
    public CompletionStage<Outcome<TokenResponse>> token(final Context invocation, final TokenRequest request) {
     ...
     */
    @Override
    public CompletionStage<Outcome<Void>> revoke(final Context invocation, final RevocationRequest request) {
     ...
     */
    private CompletionStage<Outbound> secured(
            final Context invocation,
            final AuthorizationRequest request,
