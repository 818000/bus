...
import org.miaixz.bus.auth.vendor.line.LineSourceSettings;
 ...
 * {@code access_token} revocation form. No LINE-private token or profile model crosses the Registry boundary.
 * </p>
 *
 * @author Kimi Liu
 */
public final class LineSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable LINE definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded LINE settings.
     */
    private final LineSourceSettings settings;
    ...
    /**
     * Creates one Source-bound LINE adapter from the frozen default definition.
     *
     * @param namespaceId       registration namespace used to isolate browser state and credentials
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected LINE definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded LINE settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if routing, protocol, settings, or the security baseline differs from the frozen
     *                                  LINE web profile
     */
    public LineSourceAdapter(final String namespaceId, final String sourceId, final LineDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final LineSourceSettings settings,
            final ExecutionServices services) {
        final LineDefinition selectedProfile = Assert.notNull(vendorDefinition, "LINE definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "LINE Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "LINE definition must not be null");
        this.settings = Assert.notNull(settings, "LINE settings must not be null");
        this.services = Assert.notNull(services, "LINE execution services must not be null");
        if (!LineDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(LineDefinition.DEFAULT).equals(variantDefinition)
                || !LineDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OIDC || !LineDefinition.ID.equals(settings.vendor())
                || !LineDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("LINE adapter requires the line/default OIDC definition");
        }
        ...
        this.discoveryClient = new DiscoveryClient(openIdSettings, services,
                new OpenIdProviderMetadataCodec(services.jsonProvider()));
        this.standardAdapter = new StandardAdapter(variantDefinition, settings, Optional.of(redirectManager),
                List.of(
                        ...
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.TOKEN, this::token),
                        new StandardAdapter.Binding<>(OpenIdSourceProfile.REVOCATION, this::revoke),
    ...
    /**
     * Returns the exact capability manifest frozen by the selected LINE definition.
     *
    ...
    /**
     * Routes only the Source and OIDC capabilities declared by LINE's frozen definition.
     *
    ...
    private CompletionStage<Outcome<UnoUrl>> authentication(final AuthenticationRequest request) {
        try {
            if (!valid(request)) {
                return completed(rejected("LINE Authentication Request differs from the registered definition"));
            }
            ...
            final UnoUrl url = variantDefinition.targets().resolve(settings).authorization().getOrNull().url()
                    .newBuilder().query(OAuth2.Parameters.RESPONSE_TYPE, ResponseType.CODE.value())
    ...
    private boolean valid(final AuthenticationRequest request) {
        ...
        return ResponseType.CODE.equals(authorization.responseType())
                ...
                && settings.scopes().equals(scope.values()) && authorization.state().isPresent()
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        if (resolution == null) {
            return completed(failed(ErrorCode._502, "LINE channel-secret resolver returned no stage"));
        }
        return resolution
                .handle(
    ...
    private boolean valid(final TokenRequest request) {
        ...
        if (request.grant() instanceof AuthorizationCodeGrant grant) {
            ...
            return settings.redirectUri().equals(grant.redirectUri())
                    && (clientId == null || settings.clientId().equals(clientId)) && grant.codeVerifier().isPresent();
        }
        ...
        try {
            ...
            if (response.code() == Http.Status.OK) {
                ...
                if (!TokenType.BEARER.equals(token.tokenType())
                        ...
                        || !settings.scopes().equals(token.scope().getOrNull().values())) {
                    throw new ValidateException("LINE token success does not match the registered definition");
                }
     ...
     */
    private CompletionStage<Outcome<ExternalIdentity>> verifyHs256(
            final OpenIdTokenResponse token,
            final String code,
            final SecretLease secret,
            final RedirectManager.Completion completion,
     ...
     */
    private CompletionStage<Outcome<ExternalIdentity>> verifyEs256(
            final OpenIdTokenResponse token,
            final String code,
            final RedirectManager.Completion completion,
            final Context context,
    ...
    /**
     * Delegates standard Discovery and binds every security-relevant value to the frozen LINE definition.
     *
     * @param context immutable invocation context
     * @param timeout shared end-to-end budget
    ...
    private Outcome<OpenIdProviderMetadata> metadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            if (!resolvedTargets.authorization().getOrNull().url().toString().equals(metadata.authorizationEndpoint())
                ...
                throw new ValidateException("LINE Discovery metadata differs from the frozen definition");
            }
            return Outcome.succeeded(metadata);
        } catch (RuntimeException cause) {
            return rejected("LINE Discovery metadata does not match the registered Source");
        }
    }
