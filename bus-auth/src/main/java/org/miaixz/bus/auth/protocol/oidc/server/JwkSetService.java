 ...
 * <p>
 * The external {@link KeyResolver} remains the single source for both execution and publication keys. This service
 * never derives a JWK from a JCA execution key and fails closed if a resolver attempts to publish symmetric, private,
 * unknown, ambiguously identified, or non-advertised signing material.
 * </p>
 *
 * @author Kimi Liu
 */
public final class JwkSetService {
    ...
    /**
     * Frozen OpenID Provider signing settings.
     */
     ...
     */
    public JwkSetService(final OpenIdProviderSettings settings, final ExecutionServices services) {
        this.settings = Assert.notNull(settings, "OpenID Connect JWK Set settings must not be null");
        this.services = Assert.notNull(services, "OpenID Connect JWK Set execution services must not be null");
    }

    /**
     * Creates a safe key-publication failure without retaining resolver details or key material.
     *
    ...
    public CompletionStage<Outcome<JwkSet>> jwks(final Context context, final Timeout.Budget timeout) {
        ...
        try {
            resolution = services.keyResolver().resolvePublicJwkSet(
                    new KeyResolver.PublicQuery(settings.issuer(), SIGNATURE_USE, now),
                    context,
                    timeout);
    ...
    /**
     * Preserves resolver outcome classification while validating a successful public JWK Set.
     *
     * @param outcome external resolver outcome
     ...
     *
     * @param source resolver-supplied JWK Set
    ...
    private Outcome<JwkSet> validateSet(final JwkSet source) {
        ...
        try {
            ...
            for (Jwk key : source.keys()) {
                ...
                if (!SIGNATURE_USE.equals(use) || !settings.idTokenSigningAlgorithm().name().equals(algorithm)) {
                ...
                if (publicKey.keyId().filter(settings.idTokenSigningKeyId()::equals).isPresent()) {
                    configuredMatches++;
                }
            }
