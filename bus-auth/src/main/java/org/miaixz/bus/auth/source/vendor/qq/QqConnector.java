...
/**
 * Connects the complete Qq platform manifest and its exact variant factories.
 *
 * @author Kimi Liu
 */
public class QqConnector implements VendorConnector {

    /**
     * Creates a stateless Qq SPI connector.
     */
    public QqConnector() {
    ...
    /**
     * Returns the stable Qq platform key.
     *
     * @return stable Qq platform key
     */
    @Override
    public Vendor.Id key() {
        return QqManifest.ID;
    }

    /**
     * Binds the Qq manifest, Options factory, and all adapter factories as one registration.
     *
    ...
    public void connect(final VendorRegistry registry) {
        Assert.notNull(registry, "Vendor registry must not be null").bind(
                ...
                (variant, clientId, credential, callback, scopes, parameters) -> new QqOptions(variant.platform(),
                        ...
                        VendorRegistry.bool(parameters, "preferUnionId", false)),
                List.of(
                        VendorRegistry.adapter(
                                QqManifest.class,
                                QqSourceAdapter::new,
                                QqManifest.OPEN,
                                QqManifest.MINI_PROGRAM)));
    }
