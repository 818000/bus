...
/**
 * Defines the bounded wire versions and security behavior of one server-role RADIUS Source configuration.
 * <p>
 ...
 * @param eapSupported                whether the server processes RFC 3579 EAP-Message Attributes
...
public record RadiusServerOptions(Set<RadiusPacket.Version> versions, boolean eapSupported,
    ...
    /**
     * Validates and freezes the RADIUS server options.
     *
     ...
     * @throws IllegalArgumentException if a component violates the supported RADIUS server bounds
     */
    public RadiusServerOptions {
        Assert.notNull(versions, "RADIUS server versions must not be null");
        Assert.notEmpty(versions, "RADIUS server versions must not be empty");
    ...
    /**
     * Returns this already immutable RADIUS server configuration snapshot.
     *
