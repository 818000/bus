...
import org.miaixz.bus.auth.source.protocol.oidc.client.*;
 ...
 * @author Kimi Liu
 */
public class OpenIdClientDriver implements ProtocolDriver<OpenIdClientOptions> {

    /**
     * Creates a stateless OpenID Connect Source driver.
     */
    public OpenIdClientDriver() {
    ...
    private static void validateAlgorithms(final OpenIdClientOptions options, final DriverServices services) {
        ...
        for (var algorithm : options.idTokenSigningAlgorithms()) {
            if (!allowed.contains(algorithm.name())) {
                throw new ValidateException("OpenID Connect ID Token algorithm is not permitted by the security rules");
            }
        }
    }
    ...
    private static boolean usesClientSecret(final OpenIdClientOptions options) {
        ...
        return oauth.tokenEndpoint().isPresent() || oauth.introspectionEndpoint().isPresent()
                || oauth.revocationEndpoint().isPresent() || oauth.deviceAuthorizationEndpoint().isPresent();
    }
    ...
    /**
     * Narrows generic Source options to OpenID Connect client options.
     *
     * @param options generic Source options
     * @return validated OpenID Connect client options
     */
    @Override
    ...
    /**
     * Declares the secret slot only when the selected OpenID client authentication requires it.
     *
    ...
    /**
     * Declares JSON, execution, and security-policy services used by the OpenID client.
     *
     * @param source  Source configuration
     * @param options validated OpenID Connect client options
    ...
    public SourceWorker compile(final Prepared<OpenIdClientOptions> prepared, final DriverServices services) {
        ...
        if (!scheme().id().equals(source.getType()) || !supports(source.getProtocol())
            ...
            throw new ValidateException("OpenID Connect Source driver requires a matching Source configuration");
        }
        final OpenIdClientOptions options = prepared.options();
        ...
        final OAuth2Client oauthClient = new OAuth2Client(
                ...
                new TokenClient(oauth, services, new TokenRequestEncoder(), new TokenResponseDecoder()),
        ...
        final OpenIdClient client = new OpenIdClient(oauthClient, new AuthenticationRequestEncoder(),
                new DiscoveryClient(options, services, new OpenIdProviderMetadataCodec()),
                optionalUserInfo(options, services), optionalEndSession(options));
        return new CompiledClient(manifest(options), client, new JwkSetOperation(options, services, new JwkSetCodec()));
    }
    ...
    private static final class JwkSetOperation {
        ...
        private Outcome<JwkSet> execute(final Timeout timeout) {
            try {
                ...
                final var response = FabricX.http(Protocol.OIDC, timeout, services.policies())
    ...
    private static final class CompiledClient implements SourceWorker {
         ...
         */
        private CompiledClient(final Capability.Manifest manifest, final OpenIdClient client,
        ...
        public <Q, S> CompletionStage<Outcome<S>> invoke(
            ...
            if (capability.equals(OAuth2ClientScheme.TOKEN)) {
                return narrow(
                        client.token(TokenRequest.class.cast(request), context, timeout),
                        capability.responseType());
            }
            if (capability.equals(OAuth2ClientScheme.INTROSPECTION)) {
                return narrow(
                        client.introspect(IntrospectionRequest.class.cast(request), context, timeout),
                        capability.responseType());
            }
            if (capability.equals(OAuth2ClientScheme.REVOCATION)) {
                return narrow(
                        client.revoke(RevocationRequest.class.cast(request), context, timeout),
                        capability.responseType());
            }
            if (capability.equals(OAuth2ClientScheme.DEVICE_AUTHORIZATION)) {
                return narrow(
                        client.deviceAuthorization(DeviceAuthorizationRequest.class.cast(request), context, timeout),
                        capability.responseType());
            }
            if (capability.equals(OAuth2ClientScheme.AUTHORIZATION_SERVER_METADATA) && request == null) {
