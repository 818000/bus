...
import org.miaixz.bus.auth.protocol.scim.ScimFilter.*;
...
public final class ScimFilterParser {
     ...
     * @throws ValidateException if the input is blank, oversized, malformed, truncated, or contains trailing data
     */
    ...
    private static final class Parser {
         ...
         *
         * @return decoded Unicode scalar as one or two UTF-16 code units
         * @throws ValidateException if the escape is incomplete, invalid, or contains an unpaired surrogate
         */
        private String unicode() {
            final char first = unicodeUnit();
            if (Character.isLowSurrogate(first)) {
                throw new ValidateException("SCIM filter Unicode escape contains an unpaired surrogate");
            }
            if (!Character.isHighSurrogate(first)) {
                return String.valueOf(first);
            }
            if (index + Normal._6 > source.length() || source.charAt(index) != '\\'
        ...
        /**
         * Decodes one four-hex-digit UTF-16 code unit.
         *
         * @return decoded UTF-16 code unit
         * @throws ValidateException if four valid hexadecimal digits are not available
         */
