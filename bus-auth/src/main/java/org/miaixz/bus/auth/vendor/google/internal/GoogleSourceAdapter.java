...
import org.miaixz.bus.auth.vendor.google.GoogleSourceSettings;
...
public final class GoogleSourceAdapter implements VendorAdapter {
    ...
    /**
     * Sole Google token success extension admitted by the frozen web-server definition.
     */
    ...
    /**
     * Selected immutable Google variant definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Google Source settings.
     */
    private final GoogleSourceSettings settings;
    ...
    /**
     * Creates one Source-bound Google adapter from an immutable Vendor definition.
     *
     * @param namespaceId       registration namespace used to isolate browser state and credentials
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected Google definition
     * @param variantDefinition selected default variant definition
     * @param settings          decoded externally loaded Google settings
     ...
     */
    public GoogleSourceAdapter(final String namespaceId, final String sourceId, final GoogleDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final GoogleSourceSettings settings,
            final ExecutionServices services) {
        Assert.notNull(vendorDefinition, "Google definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Google Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Google definition must not be null");
        this.settings = Assert.notNull(settings, "Google settings must not be null");
        this.services = Assert.notNull(services, "Google execution services must not be null");
        if (!GoogleDefinition.ID.equals(vendorDefinition.type())
                || !vendorDefinition.variant(GoogleDefinition.DEFAULT).equals(variantDefinition)
                || !GoogleDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OIDC || !settings.vendor().equals(GoogleDefinition.ID)
                || !settings.variant().equals(GoogleDefinition.DEFAULT)) {
            throw new ValidateException("Google adapter requires the google/default OIDC definition");
        }
    ...
    private CompletionStage<Outcome<ExternalIdentity>> verifyToken(
        ...
        try {
            ...
            return completed(rejected("Google token response does not satisfy the registered OIDC definition"));
        }
    ...
    private CompletionStage<Outcome<ExternalIdentity>> verifyIdToken(
        ...
        final IdTokenVerifier.Request verification = new IdTokenVerifier.Request(new IdToken(compact),
                new JwtVerifier.Signed(key, Set.of()), tokenIssuer, settings.clientId(),
    ...
    private Outcome<OpenIdProviderMetadata> metadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            if (!resolvedTargets.authorization().getOrNull().url().toString().equals(metadata.authorizationEndpoint())
                ...
                throw new ValidateException("Google discovery metadata differs from the frozen definition");
            }
    ...
    /**
     * Enforces the token success constraints required by the frozen Google OIDC definition.
     *
