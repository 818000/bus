...
import org.miaixz.bus.auth.source.protocol.oauth2.TokenResponse;
...
import org.miaixz.bus.auth.source.protocol.oidc.IdTokenClaims;
import org.miaixz.bus.auth.source.protocol.oidc.OpenIdConnect;
import org.miaixz.bus.auth.source.protocol.oidc.OpenIdTokenResponse;
...
public class IdTokenIssuer implements AccessTokenIssuer.Augmenter {
    ...
    public IdTokenIssuer(final OpenIdServerOptions options, final DriverServices services, final IdTokenCodec codec) {
        ...
        services.policies().require(Protocol.OIDC).algorithms().stream()
                .filter(options.idTokenSigningAlgorithm().name()::equals).findFirst().orElseThrow(
                        () -> new ValidateException(
                                "OpenID Connect ID Token signing algorithm is not permitted by the security rules"));
        ...
        this.jwtIssuer = new JwtIssuer(jwsService, FabricX.clock());
        this.jweService = options.idTokenEncryptionAlgorithmsSupported().isEmpty() ? null
                : new JweService(new org.miaixz.bus.auth.guard.AlgorithmGuard(),
    ...
    public CompletionStage<Outcome<TokenEndpointResponse>> augment(
        ...
        final ConsumerMetadata consumer = grant.consumer();
        final int encryptionParts = (consumer.idTokenEncryptionKeyId().isPresent() ? 1 : 0)
                + (consumer.idTokenEncryptionAlgorithm().isPresent() ? 1 : 0)
                + (consumer.idTokenEncryptionMethod().isPresent() ? 1 : 0);
            ...
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
                                    OAuth2ErrorCode.INVALID_GRANT,
                                    "OpenID Connect consumer ID Token encryption registration is incomplete")));
        }
        if (timeout.expired()) {
            return completed(
                    Outcome.failed(
        ...
        try {
            attributes = Outcome.mapStage(
                    () -> services.attributeLoader().load(
                            new AttributeLoader.Request(services.entry(), new Subject.Key(grant.subjectId())),
                            context,
                            timeout),
                    loaded -> services.attributeParser()
                            .parse(services.entry(), new Subject.Key(grant.subjectId()), loaded));
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> resolveKey(
        ...
        final KeyLoader.Request query = new KeyLoader.Request(services.entry(), options.issuer(),
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> issue(
        ...
        try {
            ...
            return grant.consumer().idTokenEncryptionKeyId().isPresent()
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> encrypt(
        ...
        final String algorithmName = consumer.idTokenEncryptionAlgorithm().getOrNull();
        final String methodName = consumer.idTokenEncryptionMethod().getOrNull();
        if (keyId == null || algorithmName == null || methodName == null) {
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
        ...
        try {
            algorithm = JwaAlgorithm.of(algorithmName);
            method = JwaAlgorithm.of(methodName);
            ...
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
                                    OAuth2ErrorCode.INVALID_GRANT,
                                    "OpenID Connect consumer ID Token encryption registration is invalid")));
        }
        ...
        final KeyLoader.Request query = new KeyLoader.Request(services.entry(), consumer.id(), Optional.of(keyId),
                "enc", algorithm.name(), now);
        final CompletionStage<Outcome<KeyMaterial>> resolution;
        try {
            resolution = Outcome.mapStage(
                    () -> services.keyLoader().load(query, context, timeout),
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> bind(
            ...
            final String compact,
            final Instant expiration) {
        final String sourceId = services.entry().resource().getId();
    ...
    private JsonValue.ObjectValue requestedClaims(
        ...
        if (root == null || root.values().get(OpenIdConnect.Parameters.ID_TOKEN) == null) {
            return new JsonValue.ObjectValue(selected);
        }
        if (!(root.values().get(OpenIdConnect.Parameters.ID_TOKEN) instanceof JsonValue.ObjectValue requests)) {
