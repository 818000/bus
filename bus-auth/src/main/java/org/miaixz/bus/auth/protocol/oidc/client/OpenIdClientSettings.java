...
/**
 * Holds immutable deployment settings for one OpenID Connect relying-party Source.
 * <p>
 * OAuth client identity, credentials, redirect URIs, and authorization/token endpoints remain owned by the composed
 * OAuth settings. Discovery documents and JWK Sets are runtime data and are never stored in this registration model.
 * </p>
 *
 * @param oauth2Settings           validated OAuth 2.x client settings used by the relying party
 ...
 * @author Kimi Liu
 */
public record OpenIdClientSettings(OAuth2ClientSettings oauth2Settings, Optional<Endpoint> discoveryEndpoint,
        Optional<Endpoint> userInfoEndpoint, Optional<Endpoint> jwkSetEndpoint, Optional<Endpoint> endSessionEndpoint,
        Set<JwaAlgorithm> idTokenSigningAlgorithms) implements SourceSettings {
     ...
     */
    public OpenIdClientSettings {
        Assert.notNull(oauth2Settings, "OpenID Connect OAuth 2.x client settings must not be null");
    ...
    /**
     * Returns the trusted issuer from the composed OAuth settings.
     *
     * @return exact expected OpenID Provider issuer
     */
    public String expectedIssuer() {
        return oauth2Settings.expectedIssuer().getOrNull();
    }

    /**
     * Returns the client identifier from the composed OAuth settings.
     *
     ...
     *
     * @return redacted OpenID Connect settings summary
     */
    @Override
    public String toString() {
        return "OpenIdClientSettings[oauth2Settings=[REDACTED], discoveryEndpoint=" + present(discoveryEndpoint)
