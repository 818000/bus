        ...
        return Outcome
                .mapStage(
                        () -> services.secretLoader().load(
                                new SecretLoader.Request(services.entry(), options.clientCredential().getOrNull()),
                                context,
                                timeout),
                        loaded -> services.secretParser()
                                .parse(services.entry(), options.clientCredential().getOrNull(), loaded))
    ...
    private CompletionStage<Outcome<Void>> execute(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                final var builder = FabricX.http(Protocol.OAUTH2, timeout, services.policies())
