...
import org.miaixz.bus.auth.source.protocol.scim.ResourceType;
import org.miaixz.bus.auth.source.protocol.scim.Scim;
import org.miaixz.bus.auth.source.protocol.scim.ServiceProviderConfig;
import org.miaixz.bus.core.lang.Assert;
import org.miaixz.bus.core.lang.exception.ValidateException;
import org.miaixz.bus.core.net.Protocol;

/**
 * Associates one SCIM Service Provider with its typed standard discovery objects and implementation safety limits.
 *
 ...
 * @author Kimi Liu
 */
public record ScimServerOptions(URI baseUri, ServiceProviderConfig serviceProviderConfig,
        List<ResourceType> resourceTypes, long maximumRequestBytes, int maximumJsonDepth)
    ...
    public ScimServerOptions {
        ...
        Assert.notNull(resourceTypes, "SCIM Service Provider ResourceTypes must not be null");
        if (resourceTypes.isEmpty()) {
            throw new ValidateException("SCIM Service Provider ResourceTypes must not be empty");
        }
        ...
        for (ResourceType resourceType : resourceTypes) {
            final ResourceType value = Assert
                    .notNull(resourceType, "SCIM Service Provider ResourceType must not be null");
            if (!names.add(value.name()) || !endpoints.add(value.endpoint()) || !schemas.add(value.schema())) {
                throw new ValidateException(
                        "SCIM Service Provider ResourceType names, endpoints, and schemas must be unique");
            }
            if (!Scim.USER_SCHEMA.equals(value.schema()) && !Scim.GROUP_SCHEMA.equals(value.schema())) {
                throw new ValidateException(
                        "SCIM Service Provider currently supports only core User and Group resources");
            }
        }
        resourceTypes = List.copyOf(resourceTypes);
        if (maximumRequestBytes <= 0 || maximumJsonDepth <= 0) {
            throw new ValidateException("SCIM Service Provider safety limits must be positive");
        }
    ...
    private static URI secureBaseUri(final URI value) {
        final URI uri = Assert.notNull(value, "SCIM Service Provider base URI must not be null");
        if (!uri.isAbsolute() || !Protocol.HTTPS.name.equalsIgnoreCase(uri.getScheme()) || uri.getHost() == null
                || uri.getRawUserInfo() != null || uri.getRawQuery() != null || uri.getRawFragment() != null) {
            throw new ValidateException(
                    "SCIM Service Provider base URI must be absolute credential-free HTTPS without query or fragment");
        }
        return uri;
    }
    ...
    /**
     * Returns this already immutable SCIM server configuration snapshot.
     *
