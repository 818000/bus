...
public class CodingSourceAdapter implements VendorAdapter {
    ...
    /**
     * Source identifier copied into the verified identity.
     */
    ...
    private CompletionStage<Outcome<Identity>> profile(final Access access, final Timeout timeout) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                if (timeout.expired()) {
                    return CodingSourceAdapter.<Identity>failed("CODING profile request has no remaining timeout");
                }
                ...
                return CodingSourceAdapter.<Identity>failed("CODING profile request failed");
            } finally {
                if (body != null) {
                    Arrays.fill(body, (byte) 0);
                }
        ...
        try {
            ...
            final Evidence evidence = new Evidence(Evidence.Type.FEDERATED, Evidence.Strength.SINGLE_FACTOR,
                    new Evidence.Claim("Id", new JsonValue.StringValue(profile.subject()), AUTHORITY,
                            timeout.clock().now()));
            return Outcome
                    .succeeded(new Identity(sourceId, profile.subject(), profile.attributes(), List.of(evidence)));
        } catch (RuntimeException cause) {
