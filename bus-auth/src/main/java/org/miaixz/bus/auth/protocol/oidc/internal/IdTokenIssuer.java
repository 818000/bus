...
public final class IdTokenIssuer implements AccessTokenIssuer.Augmenter {
    ...
    /**
     * Frozen OpenID Provider settings.
     */
     ...
     */
    public IdTokenIssuer(final OpenIdProviderSettings settings, final ExecutionServices services,
            final IdTokenCodec codec) {
        this.settings = Assert.notNull(settings, "OpenID Provider settings must not be null");
        ...
        services.securityBaseline().require(Protocol.OIDC).algorithms().stream()
                .filter(settings.idTokenSigningAlgorithm().name()::equals).findFirst().orElseThrow(
    ...
    public CompletionStage<Outcome<TokenEndpointResponse>> augment(
        ...
        final AuthorizationCodeStore.OpenIdBinding binding = grant.openIdBinding().getOrNull();
        ...
        final CompletionStage<Outcome<AttributeResolver.Attributes>> attributes;
        try {
            attributes = services.attributeResolver().resolve(new Subject.Key(grant.subjectId()), context, timeout);
        ...
        return attributes
                ...
                .thenCompose(outcome -> switch (outcome) {
                    ...
                    case Outcome.Rejected<AttributeResolver.Attributes> rejected -> completed(
                            Outcome.rejected(
                                    failure(
                                            rejected.failure().error(),
                                            OAuth2ErrorCode.INVALID_GRANT,
                                            "OpenID Connect subject attributes are unavailable")));
                    case Outcome.Failed<AttributeResolver.Attributes> failed -> completed(
                            Outcome.failed(
                                    failure(
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> resolveKey(
        ...
        return resolution
                ...
                .thenApply(outcome -> switch (outcome) {
                    case Outcome.Succeeded<KeyResolver.ResolvedKey> success -> issue(
                            response,
                            grant,
                            binding,
                            attributes,
                            success.value(),
                            now);
                    case Outcome.Rejected<KeyResolver.ResolvedKey> rejected -> Outcome.rejected(
                            failure(
                                    rejected.failure().error(),
                                    OAuth2ErrorCode.SERVER_ERROR,
    ...
    private Outcome<TokenEndpointResponse> issue(
        ...
        try {
            ...
            final Instant expiration = now.plus(settings.idTokenLifetime());
            final IdTokenClaims claims = new IdTokenClaims(settings.issuer(), grant.subjectId(),
                    ...
                    Optional.of(artifactHash(response.accessToken(), settings.idTokenSigningAlgorithm().name())),
            ...
            final Jwt jwt = jwtIssuer.issue(
                    new JwtIssuer.Request(
                            new JwtIssuer.Profile(settings.issuer(), List.of(grant.clientId()),
                                    settings.idTokenLifetime(), true),
                            JoseHeader.jws(
                                    settings.idTokenSigningAlgorithm(),
                                    Optional.of(settings.idTokenSigningKeyId()),
                                    Optional.empty(),
                                    Optional.empty(),
    ...
    /**
     * Validates that the resolver returned exactly the configured, currently valid signing key.
     *
     ...
     */
    private void validateResolvedKey(final KeyResolver.ResolvedKey key, final Instant now) {
        if (key == null || !settings.idTokenSigningKeyId().equals(key.keyId())
                || !settings.idTokenSigningAlgorithm().name().equals(key.algorithm()) || now.isBefore(key.notBefore())
    ...
    private JsonValue.ObjectValue requestedClaims(
        ...
        for (Map.Entry<String, JsonValue> entry : requests.values().entrySet()) {
            ...
            if (!settings.claimsSupported().contains(name)) {
