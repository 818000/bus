...
*/
/**
 * Implements the OAuth 2.x authorization-server and authorization-server direction.
 * <p>
 ...
 * service contracts own protocol decisions, while
 * {@link org.miaixz.bus.auth.source.protocol.oauth2.server.OAuth2ErrorMapper} maps failures to the error response
 * defined for the specific OAuth endpoint. {@link org.miaixz.bus.auth.source.protocol.oauth2.server.OAuth2ServerScheme}
 * and {@link org.miaixz.bus.auth.source.protocol.oauth2.server.OAuth2ServerOptions} declare only capabilities and
 * options implemented by the compiled server-role Source runtime.
 * </p>
 * <p>
 * This package consumes the standard OAuth model and codec packages, verified client identity carried by Context,
 * consent and claim ports, the sibling grant package, atomic stores, Policies, and Fabric transport. It does not load
 * registrations, persist clients or grants, authenticate an end user, implement project permissions, select a Vendor,
 * or expose Roster, Outcome, exceptions, or Bus errors as OAuth wire content.
 * </p>
 * <p>
