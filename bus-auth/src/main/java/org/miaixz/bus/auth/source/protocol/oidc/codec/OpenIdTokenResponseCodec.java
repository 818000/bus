...
import org.miaixz.bus.auth.source.protocol.oauth2.TokenErrorResponse;
...
public class OpenIdTokenResponseCodec {
    ...
    /**
     * Creates an OpenID Connect token response codec from shared OAuth collaborators and {@link JsonKit}.
     *
     * @param oauthDecoder strict OAuth token response decoder
     * @param oauthEncoder strict OAuth token response encoder
     * @throws IllegalArgumentException if a collaborator is {@code null}
     */
    public OpenIdTokenResponseCodec(final TokenResponseDecoder oauthDecoder, final TokenResponseEncoder oauthEncoder) {
    ...
    public Response encode(final Request request, final OpenIdTokenResponse response) {
        ...
        try (Response oauth = oauthEncoder.encode(request, response.tokenResponse())) {
            final JsonValue value = JsonKit.readValue(oauth.bytes(Builder.MAXIMUM_DOCUMENT_BYTES));
            if (!(value instanceof JsonValue.ObjectValue object)) {
