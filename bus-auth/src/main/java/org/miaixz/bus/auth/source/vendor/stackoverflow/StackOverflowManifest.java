 ...
 * @author Kimi Liu
 */
public class StackOverflowManifest implements VendorManifest<StackOverflowOptions> {
    ...
    private static final VendorManifest.Variant VARIANT = new VendorManifest.Variant(ID, DEFAULT, Protocol.OAUTH2,
            VendorManifest.Pkce.DISABLED, Credential.Type.CLIENT_SECRET, List.of("read_inbox"),
            new VendorTargets(
                    Optional.of(
     ...
     *
     * @param variant exact Stack Overflow variant whose form is requested
    ...
    public Scheme.Form form(final Vendor.Variant variant) {
        ...
        return VendorManifest.Forms.extended(
                selected,
                List.of(
                        VendorManifest.Forms.field("key", "Stack Apps API key", Scheme.Form.Type.TEXT, true),
                        VendorManifest.Forms.field("siteId", "Stack Exchange site", Scheme.Form.Type.TEXT, true)));
    }
    ...
    public Scheme.Metadata metadata() {
        return new Scheme.Metadata("Stack Overflow", "Stack Overflow account authorization", "stackoverflow");
    }
