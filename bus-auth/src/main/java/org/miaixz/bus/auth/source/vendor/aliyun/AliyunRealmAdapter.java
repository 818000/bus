...
/**
 * Implements the implementation-neutral Alibaba Cloud RAM realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public class AliyunRealmAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Alibaba Cloud Realm Variant.
     */
    private final VendorManifest.Variant variant;

    /**
     * Validated Alibaba Cloud Realm deployment options.
     */
    ...
    public AliyunRealmAdapter(final String spaceId, final String sourceId, final AliyunManifest manifest,
        ...
        this.variant = Assert.notNull(variant, "Alibaba Cloud Realm Variant must not be null");
        this.options = Assert.notNull(options, "Alibaba Cloud Realm options must not be null");
        this.services = Assert.notNull(services, "Alibaba Cloud Realm services must not be null");
        ...
        if (!List.copyOf(this.targets.management().keySet()).equals(MANAGEMENT_TARGETS)) {
            throw new ValidateException("Alibaba Cloud Realm manifest has an invalid management target set");
        }
    }
    ...
    /**
     * Routes one exact Alibaba Cloud Realm management capability.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Alibaba Cloud Realm capability must not be null");
        Assert.notNull(context, "Alibaba Cloud Realm context must not be null");
        Assert.notNull(timeout, "Alibaba Cloud Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected(ErrorCode._400, "Alibaba Cloud Realm capability is not declared"));
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(AliyunManifest.realmDescription())));
        }
        ...
        return completed(
                rejected(ErrorCode._400, "Alibaba Cloud Realm request does not match the capability contract"));
    }
    ...
    private CompletionStage<Outcome<Realm.Page>> snapshot(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "Alibaba Cloud Realm snapshot request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Alibaba Cloud Realm snapshot has no remaining timeout"));
        }
    ...
    private <T> CompletionStage<Outcome<T>> withSecret(
        ...
        try {
            ...
            return completed(
                    failed(ErrorCode._500, "Alibaba Cloud Realm Secret Loader failed before returning a stage"));
        }
        if (loaded == null) {
            return completed(failed(ErrorCode._500, "Alibaba Cloud Realm Secret Loader returned no stage"));
        }
        return loaded.<Outcome<SecretLoader.Record>>handle(
                (outcome, cause) -> cause == null && outcome != null ? outcome
                        : failed(ErrorCode._500, "Alibaba Cloud Realm Secret Loader stage failed"))
                .thenCompose(outcome -> switch (outcome) {
                    ...
                    default -> completed(
                            failed(
                                    ErrorCode._500,
                                    "Alibaba Cloud Realm Secret Loader returned an unsupported outcome"));
                });
    }
    ...
    private <T> CompletionStage<Outcome<T>> execute(
        ...
        try {
            ...
            return completed(failed(ErrorCode._500, "Alibaba Cloud Realm loaded Secret could not be validated"));
        }
        try {
            return CompletableFuture.<Outcome<T>>supplyAsync(() -> {
                if (timeout.expired()) {
                    return failed(ErrorCode._408, "Alibaba Cloud Realm operation has no remaining timeout");
                }
                try {
                    return Assert.notNull(operation.apply(secret), "Alibaba Cloud Realm operation returned no outcome");
                } catch (TimeoutException ignored) {
                    return failed(ErrorCode._408, "Alibaba Cloud Realm operation timed out");
                } catch (ValidateException ignored) {
                    return failed(ErrorCode._502, "Alibaba Cloud Realm returned an invalid projection");
                } catch (RuntimeException ignored) {
                    return failed(ErrorCode._503, "Alibaba Cloud Realm transport is unavailable");
                }
            }, services.executor()).whenComplete((ignored, cause) -> secret.close());
        } catch (RuntimeException ignored) {
            secret.close();
            return completed(failed(ErrorCode._500, "Alibaba Cloud Realm operation could not be scheduled"));
        }
    }
    ...
    private List<String> principals(final String encoded) {
        ...
        final JsonValue.ObjectValue policy = requiredObject(
                JsonKit.readValue(decoded.getBytes(Charset.UTF_8), Builder.MAXIMUM_REALM_JSON_DEPTH, true),
    ...
    private Outcome<Optional<JsonValue.ObjectValue>> rpc(
        ...
        final String timestamp = DateTimeFormatter.ISO_INSTANT
                .format(FabricX.clock().now().truncatedTo(ChronoUnit.SECONDS));
        ...
        try {
            response = FabricX.http(Protocol.HTTPS, timeout, services.policies()).url(url).method(Http.Method.POST)
    ...
    private Endpoint target(final String name) {
        ...
        if (endpoint == null) {
            throw new ValidateException("Alibaba Cloud Realm manifest omits a required management target");
        }
        return endpoint;
    }
    ...
    private JsonValue.ObjectValue object(final Response response) {
        return requiredObject(value(response), "Alibaba Cloud Realm response");
    }
