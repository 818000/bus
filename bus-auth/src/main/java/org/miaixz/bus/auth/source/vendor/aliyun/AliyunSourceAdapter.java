 ...
 * <p>
 * All Roster-visible protocol operations preserve their standard request and response types. Browser authentication
 * adds only the application-level correlation lifecycle needed to turn a verified ID Token and UserInfo response into
 * an {@link Identity}; it never publishes a platform-specific token or profile model.
 * </p>
 *
 * @author Kimi Liu
 */
public class AliyunSourceAdapter implements VendorAdapter {
    ...
    /**
     * Source identifier used for every produced external identity.
     */
    ...
    public AliyunSourceAdapter(final String spaceId, final String sourceId, final AliyunManifest manifest,
        ...
        final Set<String> requiredAlgorithms = services.policies().require(Protocol.OIDC).algorithms();
        if (!requiredAlgorithms.contains(JwaAlgorithm.RS256.name())) {
            throw new ValidateException("Alibaba Cloud RS256 is not permitted by the OIDC security rules");
        }
        ...
        this.idTokenVerifier = new IdTokenVerifier(idTokenCodec, issuerValidator,
                new org.miaixz.bus.auth.guard.TimeGuard(FabricX.clock(),
    ...
    private static StandardAdapter standardAdapter(
        ...
        final OpenIdClient openIdClient = new OpenIdClient(oauthClient, new AuthenticationRequestEncoder(),
                new DiscoveryClient(openIdSettings, services, new OpenIdProviderMetadataCodec()),
                Optional.of(new UserInfoClient(openIdSettings, services, new UserInfoCodec())), Optional.empty());
        return new StandardAdapter(variant, options, Optional.of(redirectManager),
                List.of(
                        ...
                        new StandardAdapter.Binding<>(OAuth2ClientScheme.TOKEN, openIdClient::token),
                        new StandardAdapter.Binding<>(OAuth2ClientScheme.REVOCATION, openIdClient::revoke),
    ...
    /**
     * Creates a implementation-neutral empty JSON object for standard extension fields.
     *
    ...
    private CompletionStage<Outcome<Identity>> identity(
        ...
        return standardAdapter.invoke(OAuth2ClientScheme.TOKEN, tokenRequest, context, timeout)
    ...
    private Outcome<Identity> identity(
        ...
        final Evidence evidence = new Evidence(Evidence.Type.FEDERATED, Evidence.Strength.SINGLE_FACTOR,
                new Evidence.Claim(JwtClaims.SUBJECT, new JsonValue.StringValue(claims.subject()), ISSUER,
                        timeout.clock().now()));
        return Outcome.succeeded(new Identity(sourceId, claims.subject(), attributes, List.of(evidence)));
    }
    ...
    private Outcome<OpenIdProviderMetadata> metadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            return rejected("Alibaba Cloud discovery metadata does not match the configured Source");
        }
    }
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                return Outcome.succeeded(
                        jwkSetCodec.decode(
                                FabricX.http(Protocol.OIDC, timeout, services.policies()).url(endpoint)
