...
public class AccountingService {
    ...
    /**
     * Creates an Accounting service for one compiled RADIUS server.
     *
    ...
    public CompletionStage<Outcome<AccountingResponse>> accounting(
        ...
        try {
            stage = handler.resolve(sourceId, context.network().remoteAddress(), effective, context, timeout);
    ...
    private CompletionStage<Outcome<AccountingResponse>> invokeHandler(
        ...
        try {
            stage = handler.accounting(sourceId, client, request, context, timeout);
        } catch (RuntimeException exception) {
