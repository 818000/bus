 ...
 * </p>
 * <p>
 * This exported package provides Source configuration and Realm management metadata; execution must enter a configured
 * Source through Dispatcher. Source completion requires the token envelope's {@code authed_user.id} to equal the
 * {@code users.info user.id}, and only that bound ID becomes Identity subject. Client secrets, state, codes, tokens,
 * Slack profile values, response bodies, and platform errors must not enter diagnostics, Context, tracing, logs, or
 ...
 * collection. External projects call Dispatcher and own synchronization, durable state, and reconciliation.
 * </p>
 *
 * @author Kimi Liu
 */
