...
import org.miaixz.bus.auth.metric.SCIM.MutationResult;
import org.miaixz.bus.auth.metric.SCIM.Page;
import org.miaixz.bus.auth.metric.SCIM.ProtocolError;
import org.miaixz.bus.auth.metric.SCIM.ResourceType;
import org.miaixz.bus.auth.metric.scim.ScimBulk.Entry;
import org.miaixz.bus.auth.metric.scim.ScimPatch.Operation;
import org.miaixz.bus.auth.metric.scim.ScimPatch.Type;
...
public final class ScimCodec {
    ...
    /**
     * Runtime providing strict JSON limits and the framework JSON port.
     */
     ...
     */
    public ScimCodec(final Runtime runtime) {
        this.runtime = Assert.notNull(runtime, () -> new ValidateException("SCIM runtime must not be null"));
        this.reader = new StrictJsonReader(runtime.json(), runtime.limits());
    }
    ...
    public ScimBulk.Request bulk(final byte[] body) {
        Assert.isTrue(
                body.length <= runtime.limits().maxScimBulkBytes(),
        ...
        Assert.isTrue(
                operations.size() <= runtime.limits().maxScimBulkOperations(),
        ...
        for (final Object item : operations) {
            ...
            if (method == Http.Method.POST || method == Http.Method.PUT) {
                ...
                final byte[] encoded = runtime.json().write(data);
                data = patch(encoded);
            }
        ...
        return new ScimBulk.Request(entries, failOnErrors, body.length);
    }
    ...
    public byte[] write(final ScimBulk.Response response) {
        ...
        return runtime.json().write(Map.of("schemas", List.of(SCIM.BULK_RESPONSE_SCHEMA), "Operations", operations));
    }
    ...
    public byte[] write(final ScimResource resource) {
        return runtime.json().write(resource(resource));
    }
    ...
    public byte[] write(final Page page, final int startIndex) {
        ...
        return runtime.json().write(result);
    }
