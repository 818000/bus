...
import org.miaixz.bus.auth.metric.AuthMetric.StateStore;
 ...
 * the same name. In particular, one-time consumption uses only {@link StateStore#take(Invocation, String)} and never
 * composes a read with removal; replay admission uses only put-if-absent, and compare-and-set remains one backend
 * linearization point.
 * <p>
 * <strong>Bus dependencies:</strong> {@link StateStore} supplies all atomic semantics, {@link Invocation} supplies
 ...
 * @author Kimi Liu
 */
public final class StateStoreGuard implements StateStore {
     ...
     */
    private static Invocation invocation(final Invocation invocation) {
        return Assert.notNull(invocation, () -> new ValidateException("Invocation must not be null"));
    }
     ...
     */
    @Override
    public CompletionStage<Optional<byte[]>> get(final Invocation invocation, final String key) {
     ...
     */
    @Override
    public CompletionStage<Optional<byte[]>> take(final Invocation invocation, final String key) {
     ...
     */
    @Override
    public CompletionStage<Boolean> remove(final Invocation invocation, final String key) {
