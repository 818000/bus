...
import org.miaixz.bus.auth.source.protocol.oauth2.codec.RevocationRequestEncoder;
 ...
 * <p>
 * The adapter keeps OAuth and OpenID Connect public models on the Roster boundary. Apple-only form-post callback
...
public class AppleSourceAdapter implements VendorAdapter {
     ...
     *
     * @param spaceId  Source space used to isolate browser correlation and key resolution
    ...
    public AppleSourceAdapter(final String spaceId, final String sourceId, final AppleManifest manifest,
        ...
        this.clientSecretJws = new JwsService(new org.miaixz.bus.auth.guard.AlgorithmGuard(),
        ...
        this.metadataCodec = new OpenIdProviderMetadataCodec();
        this.jwkSetCodec = new JwkSetCodec();
        this.jwkSelector = new JwkSelector();
        this.issuerValidator = new IssuerValidator();
        this.idTokenJws = new JwsService(new org.miaixz.bus.auth.guard.AlgorithmGuard(),
        ...
        final IdTokenCodec idTokenCodec = new IdTokenCodec(new JwtVerifier(idTokenJws, dormantJwe));
    ...
    /**
     * Creates a implementation-neutral JSON integer.
     *
    ...
    private CompletionStage<Outcome<String>> clientSecret(final Context context, final Timeout timeout) {
        ...
        final KeyLoader.Request query = new KeyLoader.Request(services.entry(), ISSUER,
    ...
    private Outcome<String> sign(final KeyMaterial resolved, final Instant now) {
        ...
        try {
            ...
            final byte[] payload = JsonKit.writeValue(new JsonValue.ObjectValue(claims));
            try {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                final OpenIdProviderMetadata metadata = metadataCodec.decode(
                        FabricX.http(Protocol.OIDC, timeout, services.policies()).url(endpoint).method(Http.Method.GET)
    ...
    private Outcome<OpenIdProviderMetadata> validateMetadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            return rejected("Sign in with Apple discovery metadata does not match the configured Source");
        }
    }
     ...
     * @return verified Apple external identity
     */
    private CompletionStage<Outcome<Identity>> verifyCallbackToken(
     ...
     */
    private Outcome<Identity> identity(final IdTokenClaims claims, final String callbackUser, final Timeout timeout) {
        try {
            ...
            if (callbackUser != null) {
                ...
                final JsonValue value = JsonKit.readValue(userBytes, 32, true);
    ...
    private boolean appleError(final Response response) {
        try {
            ...
            final JsonValue value = JsonKit.readValue(response.bytes(Normal.MEBI), 16, true);
