 ...
 * <p>
 * Fixed Google endpoints, issuer, S256 PKCE, and RS256 validation remain manifest- and adapter-owned. This record
...
public record GoogleOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
    ...
    public GoogleOptions {
        if (!GoogleManifest.ID.equals(vendor) || !GoogleManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("Google options must select google/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Google options require an Authorized redirect URI");
        }
        ...
        for (String scope : scopes) {
            ...
            if (!approvedScope(checked) || copy.contains(checked)) {
                throw new ValidateException("Google scope is unsupported or duplicated by the frozen login manifest");
            }
            copy.add(checked);
        }
        if (copy.isEmpty()) {
    ...
    public String toString() {
        return "GoogleOptions[vendor=" + vendor + Builder.VARIANT + variant + Builder.REDACTED_SOURCE_OPTIONS + scopes
