...
public class RefreshTokenRotator {
    ...
    /**
     * Source identifier used in token and family key isolation.
     */
    private final String sourceId;

    /**
     * Frozen authorization-server grant and lifetime policy.
     */
    ...
    /**
     * Creates a refresh-token rotator for one compiled OAuth authorization server.
     *
            ...
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
                                    OAuth2ErrorCode.UNSUPPORTED_GRANT_TYPE,
                                    "OAuth 2.x refresh-token rotation is disabled by the authorization server")));
        }
    ...
    private CompletionStage<Outcome<TokenResponse>> checkFamily(
        ...
        try {
            lookup = services.authorizationCache().find(AuthorizationCache.key(sourceId, entry.familyId()));
        } catch (RuntimeException exception) {
        ...
        return lookup.handle((family, thrown) -> new FamilyResult(family, thrown)).thenCompose(result -> {
            ...
            if (family == null || !family.expiresAt().isAfter(timeout.clock().now())
                    || !sourceId.equals(family.value().sourceId()) || !client.id().equals(family.value().clientId())) {
    ...
    private CompletionStage<Outcome<TokenResponse>> createSuccessor(
        ...
        final RefreshTokenCache.Entry successor = new RefreshTokenCache.Entry(sourceId, client.id(), entry.subjectId(),
                entry.familyId(), entry.generation() + 1L, scope, entry.audience(), entry.confirmation(),
                entry.openIdBinding(), RefreshTokenCache.Status.ACTIVE);
    ...
    private CompletionStage<Outcome<TokenResponse>> rotateOld(
            ...
            final Timeout timeout) {
        final RefreshTokenCache.Entry rotated = new RefreshTokenCache.Entry(entry.sourceId(), entry.clientId(),
    ...
    private CompletionStage<Outcome<TokenResponse>> markReuse(
        ...
        return compromiseFamily(entry, timeout, 1).handle((confirmed, thrown) -> thrown).thenCompose(thrown -> {
            ...
            if (entry.status() == RefreshTokenCache.Status.ROTATED) {
                final RefreshTokenCache.Entry reused = new RefreshTokenCache.Entry(entry.sourceId(), entry.clientId(),
                        entry.subjectId(), entry.familyId(), entry.generation(), entry.scope(), entry.audience(),
    ...
    private CompletionStage<Boolean> compromiseFamily(
        ...
        final String key = AuthorizationCache.key(sourceId, entry.familyId());
        final CompletionStage<ExpiringValue<AuthorizationCache.Entry>> lookup;
        try {
        ...
        return lookup.thenCompose(family -> {
            if (family == null || !family.expiresAt().isAfter(timeout.clock().now())
                    || !sourceId.equals(family.value().sourceId())
            ...
            final AuthorizationCache.Entry compromised = new AuthorizationCache.Entry(sourceId, entry.clientId(),
     ...
     *
     * @param key     Source-isolated authorization family key
    ...
    private CompletionStage<Boolean> confirmFamilyInactive(
        ...
        return services.authorizationCache().find(key).thenCompose(family -> {
            if (family != null && family.expiresAt().isAfter(timeout.clock().now())
                    && sourceId.equals(family.value().sourceId()) && entry.clientId().equals(family.value().clientId())
    ...
    private CompletionStage<Outcome<TokenResponse>> revokeCompromised(
            ...
            final RefreshTokenCache.Entry entry,
            final Timeout timeout) {
        final RefreshTokenCache.Entry revoked = new RefreshTokenCache.Entry(entry.sourceId(), entry.clientId(),
                entry.subjectId(), entry.familyId(), entry.generation(), entry.scope(), entry.audience(),
    ...
    /**
     * Validates a refreshed scope against the original grant, client registration, and authorization-server bounds.
     *
     * @param scope    candidate effective scope
     * @param original original refresh-token scope
     * @param client   client-registered scope set
     * @param server   authorization-server-supported scope set
    ...
    private boolean validScope(
        ...
        try {
            ...
            scopeValidator.validateRequested(scope, server);
            return true;
        } catch (RuntimeException exception) {
            return false;
        }
