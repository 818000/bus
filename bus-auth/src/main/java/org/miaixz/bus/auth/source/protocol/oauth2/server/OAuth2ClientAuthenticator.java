...
public class OAuth2ClientAuthenticator implements ClientAuthenticator<Request> {
    ...
    private CompletionStage<Outcome<ClientAuthentication>> load(
        ...
        try {
            loading = services.consumerLoader()
                    .load(new ConsumerLoader.Request(services.entry(), evidence.clientId()), context, timeout);
    ...
    private CompletionStage<Outcome<ClientAuthentication>> verify(
        ...
        try {
            consumer = services.consumerParser().parse(services.entry(), evidence.clientId(), record);
        } catch (RuntimeException cause) {
        ...
        if (!validAuthenticationRegistration(consumer)) {
            return completed(Outcome.failed(failure(true, "OAuth 2.x consumer authentication data is invalid")));
        }
        if (!consumer.authenticationMethods().contains(evidence.method())) {
        ...
        if (Endpoint.Authentication.NONE.equals(evidence.method())) {
            return consumer.authenticationMethods().equals(Set.of(Endpoint.Authentication.NONE))
        ...
        try {
            verification = services.consumerVerifier()
                    .verify(services.entry(), consumer.id(), evidence.method(), lease, context, timeout);
        } catch (RuntimeException cause) {
     ...
     *
     * @param request OAuth 2.x client-authentication request
     * @param context trusted invocation context
     * @param timeout authentication deadline
     * @return asynchronous authenticated-client outcome
     */
    @Override
