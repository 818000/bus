...
/**
 * Implements the implementation-neutral Figma administrator SCIM realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public class FigmaRealmAdapter implements VendorAdapter {
    ...
    /**
     * Routes one exact Figma SCIM Realm capability.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Figma Realm capability must not be null");
        Assert.notNull(context, "Figma Realm context must not be null");
        Assert.notNull(timeout, "Figma Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected(ErrorCode._400, "Figma Realm capability is not declared"));
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(FigmaManifest.realmDescription())));
        }
        ...
        return completed(rejected(ErrorCode._400, "Figma Realm request does not match the capability contract"));
    }
    ...
    private Outcome<Realm.Retrieved> retrieve(
            ...
            final Instant observedAt,
            final Timeout timeout) {
        final int count = Math.min(Builder.MAXIMUM_REALM_PAGE_SIZE, Normal._3000);
        int start = Normal._1;
        while (true) {
            if (timeout.expired()) {
