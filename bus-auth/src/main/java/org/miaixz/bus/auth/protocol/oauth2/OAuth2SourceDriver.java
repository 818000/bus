 ...
 * @author Kimi Liu
 */
public final class OAuth2SourceDriver implements SourceDriver<OAuth2ClientOptions> {
    ...
    public SourceWorker compile(
        ...
        final OAuth2ClientOptions options = require(source.getOptions());
        final Endpoint authorizationEndpoint = options.authorizationEndpoint().getOrNull();
        return new CompiledSource(manifest(options),
                ...
                options.tokenEndpoint().isEmpty() ? null
                        : new TokenClient(options, services, new TokenRequestEncoder(),
