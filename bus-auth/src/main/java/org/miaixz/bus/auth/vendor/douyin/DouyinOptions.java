 ...
 * @author Kimi Liu
 */
public record DouyinOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
     ...
     */
    public DouyinOptions {
        if (!DouyinManifest.ID.equals(vendor)
                || !DouyinManifest.OPEN.equals(variant) && !DouyinManifest.MINI_PROGRAM.equals(variant)) {
            throw new ValidateException("Douyin options must select douyin/open or douyin/mini-program");
        }
        ...
        if (DouyinManifest.OPEN.equals(variant)) {
            open(redirectUri, scopes);
        } else if (redirectUri.isPresent() || !scopes.isEmpty()) {
            throw new ValidateException("Douyin mini-program options prohibit redirect URI and scopes");
        }
    }
    ...
    private static void open(final Optional<String> redirectUri, final List<String> scopes) {
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Douyin open options require a registered redirect URI");
        }
    ...
    public String toString() {
        return "DouyinOptions[vendor=" + vendor + Builder.VARIANT + variant + Builder.REDACTED_SOURCE_OPTIONS + scopes
