...
public final class AddService {
    ...
    private static Outcome<LdapMessage> local(final int messageId, final LdapResultCode code) {
        final LdapResult result = new LdapResult(code, new DistinguishedName(Normal.EMPTY),
                "The LDAP Add request was not accepted.", org.miaixz.bus.core.lang.Optional.empty());
    ...
    public CompletionStage<Outcome<LdapMessage>> add(
        ...
        Assert.notNull(timeout, "LDAP Add time budget must not be null");
        ...
        if (timeout.expired()) {
            return completed(Outcome.failed(failure("LDAP Add time budget expired")));
        }
        final CompletionStage<Outcome<AddResponse>> stage;
        try {
            stage = store.add(providerId, context.network().connection().getOrThrow().id(), request, context, timeout);
