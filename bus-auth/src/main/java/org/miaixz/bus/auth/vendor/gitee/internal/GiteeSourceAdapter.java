...
import org.miaixz.bus.auth.vendor.gitee.GiteeSourceSettings;
...
public final class GiteeSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Gitee definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Gitee settings.
     */
    private final GiteeSourceSettings settings;
     ...
     * @param vendorDefinition  selected Gitee definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded Gitee settings
     ...
     */
    public GiteeSourceAdapter(final String namespaceId, final String sourceId, final GiteeDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final GiteeSourceSettings settings,
            final ExecutionServices services) {
        final GiteeDefinition selectedProfile = Assert.notNull(vendorDefinition, "Gitee definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Gitee Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Gitee definition must not be null");
        this.settings = Assert.notNull(settings, "Gitee settings must not be null");
        this.services = Assert.notNull(services, "Gitee execution services must not be null");
        if (!GiteeDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(GiteeDefinition.DEFAULT).equals(variantDefinition)
                || !GiteeDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !GiteeDefinition.ID.equals(settings.vendor())
                || !GiteeDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("Gitee adapter requires the gitee/default OAuth 2.0 definition");
        }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected("Gitee capability is not declared by the selected definition"));
    }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected Gitee definition.
     *
