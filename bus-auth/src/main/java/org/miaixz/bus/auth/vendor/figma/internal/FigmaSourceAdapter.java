...
import org.miaixz.bus.auth.vendor.figma.FigmaSourceSettings;
...
public final class FigmaSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Figma definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Figma settings.
     */
    private final FigmaSourceSettings settings;
     ...
     * @param vendorDefinition  selected Figma definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded Figma settings
     ...
     */
    public FigmaSourceAdapter(final String namespaceId, final String sourceId, final FigmaDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final FigmaSourceSettings settings,
            final ExecutionServices services) {
        final FigmaDefinition selectedProfile = Assert.notNull(vendorDefinition, "Figma definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Figma Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Figma definition must not be null");
        this.settings = Assert.notNull(settings, "Figma settings must not be null");
        this.services = Assert.notNull(services, "Figma execution services must not be null");
        if (!FigmaDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(FigmaDefinition.DEFAULT).equals(variantDefinition)
                || !FigmaDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !FigmaDefinition.ID.equals(settings.vendor())
                || !FigmaDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("Figma adapter requires the figma/default OAuth 2.0 definition");
        }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected("Figma capability is not declared by the selected definition"));
    }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected Figma definition.
     *
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        try {
            ...
            final String redirect = endpoint.url().newBuilder().query(OAuth2.Parameters.CLIENT_ID, settings.clientId())
    ...
    private Outcome<Access> token(
        ...
        try {
            ...
            basicBytes = basic(settings.clientId(), secret.material());
