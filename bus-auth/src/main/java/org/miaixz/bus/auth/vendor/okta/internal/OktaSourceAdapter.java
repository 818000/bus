...
import org.miaixz.bus.auth.vendor.okta.OktaSourceSettings;
...
public final class OktaSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Okta variant definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Okta registration settings.
     */
    private final OktaSourceSettings settings;
    ...
    /**
     * Creates one Source-bound Okta adapter from the frozen default definition.
     *
     * @param namespaceId       registration namespace isolating browser and credential state
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected Okta definition
     * @param variantDefinition selected default variant definition
     * @param settings          decoded externally loaded Okta settings
     * @param services          caller-owned runtime dependencies
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if profile, definition, settings, or security baseline do not match the frozen
     *                                  variant
     */
    public OktaSourceAdapter(final String namespaceId, final String sourceId, final OktaDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final OktaSourceSettings settings,
            final ExecutionServices services) {
        Assert.notNull(vendorDefinition, "Okta definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Okta Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Okta definition must not be null");
        this.settings = Assert.notNull(settings, "Okta settings must not be null");
        this.services = Assert.notNull(services, "Okta execution services must not be null");
        if (!OktaDefinition.ID.equals(vendorDefinition.type())
                || !vendorDefinition.variant(OktaDefinition.DEFAULT).equals(variantDefinition)
                || !OktaDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OIDC || !OktaDefinition.ID.equals(settings.vendor())
                || !OktaDefinition.DEFAULT.equals(settings.variant())) {
            throw new ValidateException("Okta adapter requires the okta/default OIDC definition");
        }
        ...
        final OAuth2ClientSettings oauthSettings = new OAuth2ClientSettings(targets.authorization(), targets.token(),
                ...
                Endpoint.Authentication.CLIENT_SECRET_BASIC, Optional.of(settings.credential()), false, false);
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        if (response.issuer().isEmpty() || !issuer.equals(response.issuer().getOrNull())
            ...
            return completed(rejected("Okta authorization response does not satisfy the registered OIDC definition"));
        }
    ...
    private CompletionStage<Outcome<ExternalIdentity>> verify(
        ...
        return standardAdapter.invoke(OpenIdSourceProfile.JWK_SET, null, context, timeout)
                .thenCompose(outcome -> switch (outcome) {
    ...
    private AuthorizationResponseDecoder.Decoded decode(final Callback.Inbound callback) {
        ...
        if (!settings.redirectUri().getOrNull().equals(inbound.requestUri()) || inbound.method() != Http.Method.GET) {
    ...
    /**
     * Returns explicit Source scopes or the immutable definition defaults.
     *
