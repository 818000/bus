 ...
 * to the closed {@link AddressPolicy}; enforces HTTPS where requested; compares complete origins; and admits only
 * same-origin redirects within an explicit redirect count. DNS result and CIDR enforcement remain in bus-fabric's
 ...
 * defaults, {@link AddressPolicy} provides scheme and port admission, and {@link ErrorCode} provides stable URL and
 * HTTPS failures.
 *
 * @author Kimi Liu
 */
public final class UriValidator {
     ...
     *
     * @param uri    source URI
     * @param policy Fabric address policy
     * @return unchanged validated URI
     */
    public static URI transport(final URI uri, final AddressPolicy policy) {
        final URI current = absolute(uri);
        final AddressPolicy rules = Assert
                .notNull(policy, () -> new ValidateException("Address policy must not be null"));
     ...
     * @param limit    positive maximum redirect count
     * @return unchanged validated redirect URI
     */
    public static URI redirect(
            final URI previous,
            final URI redirect,
        ...
        final AddressPolicy rules = Assert
                .notNull(policy, () -> new ValidateException("Address policy must not be null"));
        require(limit > Normal._0 && count > Normal._0 && count <= limit);
