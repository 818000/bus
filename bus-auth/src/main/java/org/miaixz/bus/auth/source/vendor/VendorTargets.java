...
public record VendorTargets(Optional<Target> authorization, Optional<Target> token, Optional<Target> userInfo,
    ...
    /**
     * Creates a Vendor target set without named management endpoints.
     *
     * @param authorization       authorization endpoint
     * @param token               token or platform credential endpoint
     * @param userInfo            user information endpoint
    ...
    private static String resolveValue(final String value, final VendorOptions<?> options) {
        ...
        if (resolved.contains(Variable.TENANT.token)) {
            resolved = resolved
                    .replace(Variable.TENANT.token, segment(require(options.templateTenant(), "Vendor tenant")));
        }
        if (resolved.contains(Variable.AUTHORIZATION_SERVER_ID.token)) {
            resolved = resolved.replace(
                    Variable.AUTHORIZATION_SERVER_ID.token,
                    segment(require(options.templateAuthorizationServerId(), "Vendor authorization server id")));
        }
        if (resolved.contains(Variable.INSTANCE.token)) {
            final String instance = require(options.templateInstance(), "Vendor instance");
            final String authority = authority(value);
            resolved = resolved.replace(
                    Variable.INSTANCE.token,
                    authority.equals(Variable.INSTANCE.token) ? dnsHost(instance) : dnsLabel(instance));
        }
        return resolved;
    }
    ...
    private static void validateTemplate(final String value) {
        ...
        if (value.contains(Variable.INSTANCE.token) && !(authority.equals(Variable.INSTANCE.token)
                || authority.startsWith(Variable.INSTANCE.token + Symbol.DOT))) {
            throw new ValidateException(
                    "Vendor instance variable must occupy the complete host or its first DNS label");
        }
        if (authority.contains(Variable.TENANT.token) || authority.contains(Variable.AUTHORIZATION_SERVER_ID.token)) {
    ...
    private static String parseable(final String value) {
        final String authority = authority(value);
        return value.replace(Variable.TENANT.token, "tenant").replace(Variable.AUTHORIZATION_SERVER_ID.token, "server")
                .replace(
                        Variable.INSTANCE.token,
                        authority.equals(Variable.INSTANCE.token) ? "example.com" : "instance");
    }
