...
/**
 * Implements the implementation-neutral Okta service-app realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public class OktaRealmAdapter implements VendorAdapter {
    ...
    /**
     * Converts one minimal Okta user projection to a implementation-neutral resource.
     *
    ...
    /**
     * Converts one minimal Okta group projection to a implementation-neutral resource.
     *
    ...
    /**
     * Converts one minimal Okta administrator role to a implementation-neutral resource.
     *
    ...
    /**
     * Creates one implementation-neutral relation with exact allow-listed attributes.
     *
     * @param relationKind normalized relation category
     * @param fromKind     normalized source resource category
     * @param fromId       stable source identifier
    ...
    /**
     * Maps one exact Okta user status to the normalized Realm state closure.
     *
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected(ErrorCode._400, "Okta Realm capability is not declared by the selected manifest"));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> mismatch() {
        return completed(
                rejected(ErrorCode._400, "Okta Realm request does not match the selected capability contract"));
    }
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Okta Realm capability must not be null");
        Assert.notNull(context, "Okta Realm context must not be null");
        Assert.notNull(timeout, "Okta Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return missing();
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(OktaManifest.realmDescription())));
        }
    ...
    private Outcome<Realm.Page> users(
        ...
        final Url requestUrl = state.position().next().isPresent()
                ...
                : url(Builder.REALM_USERS, null, Map.of("limit", Integer.toString(limit)));
    ...
    private Outcome<Realm.Page> groups(
        ...
        final Url requestUrl = state.position().next().isPresent()
                ...
                : url(Builder.REALM_GROUPS, null, Map.of("limit", Integer.toString(limit)));
    ...
    private Outcome<Realm.Retrieved> retrieveUser(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = get(
                url(Builder.REALM_USER, List.of(id), Map.of()),
                access,
                timeout,
                "retrieve",
                true);
    ...
    private Outcome<Realm.Retrieved> retrieveGroup(
            ...
            final Instant observedAt,
            final Timeout timeout) {
        Url requestUrl = url(Builder.REALM_GROUPS, null, Map.of("limit", Integer.toString(Normal._200)));
        while (true) {
            final Outcome<WirePage<Group>> fetched = wirePage(
                    requestUrl,
                    access,
    ...
    private Outcome<List<RoleAssignment>> assignmentList(
        ...
        final String targetName = "users".equals(collection) ? Builder.REALM_ROLE_ASSIGNMENTS
                : Builder.REALM_ROLE_MEMBERS;
    ...
    private Outcome<Optional<String>> parent(
        ...
        final String targetName = users ? Builder.REALM_USERS : Builder.REALM_GROUPS;
    ...
    private Url pagination(final String value, final Phase phase) {
        ...
        final Url base = target(
                phase == Phase.USERS ? Builder.REALM_USERS
                        : phase == Phase.GROUPS ? Builder.REALM_GROUPS : Builder.REALM_GROUP_MEMBERS).url();
        ...
        try {
            ...
            if (state.position().next().isPresent()) {
                ...
                if (phase == Phase.GROUP_MEMBERS) {
                    ...
                    final Url parentUrl = url(Builder.REALM_GROUP_MEMBERS, List.of(parentId, "users"), Map.of());
