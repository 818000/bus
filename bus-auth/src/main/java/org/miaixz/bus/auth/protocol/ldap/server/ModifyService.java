 ...
 * @author Kimi Liu
 */
public final class ModifyService {
     ...
     */
    public ModifyService(final String providerId, final LdapProviderSettings settings, final DirectoryStore store) {
        this.providerId = Assert.notBlank(providerId, "LDAP Modify Provider id must not be blank");
        this.settings = Assert.notNull(settings, "LDAP Modify Provider settings must not be null");
    ...
    private static Outcome<LdapMessage> local(final int messageId, final LdapResultCode code) {
        final LdapResult result = new LdapResult(code, new DistinguishedName(Normal.EMPTY),
                "The LDAP Modify request was not accepted.", org.miaixz.bus.core.lang.Optional.empty());
    ...
    public CompletionStage<Outcome<LdapMessage>> modify(
        ...
        Assert.notNull(timeout, "LDAP Modify time budget must not be null");
        ...
        if (timeout.expired()) {
            return completed(Outcome.failed(failure("LDAP Modify time budget expired")));
        }
