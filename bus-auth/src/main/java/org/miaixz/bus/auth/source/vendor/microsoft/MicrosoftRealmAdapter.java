...
/**
 * Implements the implementation-neutral Microsoft Graph realm surface for the isolated global and China clouds.
 * <p>
 ...
 * @author Kimi Liu
 */
public class MicrosoftRealmAdapter implements VendorAdapter {
    ...
    /**
     * Exact resource categories exposed by both Microsoft Realm-capable variants.
     */
    ...
    /**
     * Selected immutable Microsoft Realm Variant.
     */
    private final VendorManifest.Variant variant;

    /**
     * Validated Microsoft Realm deployment options.
     */
     ...
     *
     * @param spaceId  Source space used for credential isolation
    ...
    public MicrosoftRealmAdapter(final String spaceId, final String sourceId, final MicrosoftManifest manifest,
        ...
        final MicrosoftManifest selectedManifest = Assert
                .notNull(manifest, "Microsoft Realm manifest must not be null");
        this.variant = Assert.notNull(variant, "Microsoft Realm Variant must not be null");
        this.options = Assert.notNull(options, "Microsoft Realm options must not be null");
        this.services = Assert.notNull(services, "Microsoft Realm services must not be null");
        final boolean realm = MicrosoftManifest.ENTERPRISE_GLOBAL.equals(this.variant.variant())
                || MicrosoftManifest.ENTERPRISE_CHINA.equals(this.variant.variant());
        if (!realm || !MicrosoftManifest.ID.equals(selectedManifest.vendor())
            ...
            throw new ValidateException("Microsoft realm adapter requires one frozen Realm HTTPS variant");
        }
        ...
        if (this.targets.token().isEmpty()
            ...
            throw new ValidateException("Microsoft Realm manifest has an invalid target set");
        }
        this.graphHost = target(Builder.REALM_CHANGES).url().host();
        for (Endpoint endpoint : this.targets.management().values()) {
            if (!this.graphHost.equals(endpoint.url().host())) {
                throw new ValidateException("Microsoft Realm Graph targets must use one isolated cloud host");
            }
        }
    ...
    /**
     * Extracts the implementation-neutral resource or relation key from one change variant.
     *
     * @param change exact Realm change
    ...
    private static boolean boundedPositiveInteger(final String value) {
        try {
            ...
            return number > 0 && number <= Builder.MAXIMUM_REALM_PAGE_SIZE && Integer.toString(number).equals(value);
    ...
    /**
     * Maps one Microsoft organization projection to a implementation-neutral resource.
     *
    ...
    /**
     * Maps one Microsoft user projection to a implementation-neutral resource.
     *
    ...
    /**
     * Maps one Microsoft group projection to a implementation-neutral resource.
     *
    ...
    /**
     * Maps one Microsoft directory role projection to a implementation-neutral resource.
     *
    ...
    /**
     * Maps one service principal to the implementation-neutral service-account resource category.
     *
    ...
    /**
     * Creates one attribute-free implementation-neutral resource.
     *
     * @param kind        resource kind
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(
                rejected(ErrorCode._400, "Microsoft Realm capability is not declared by the selected manifest"));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> mismatch() {
        return completed(
                rejected(ErrorCode._400, "Microsoft Realm request does not match the selected capability contract"));
    }
    ...
    /**
     * Routes one exact Microsoft Realm capability and request type.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Microsoft Realm capability must not be null");
        Assert.notNull(context, "Microsoft Realm context must not be null");
        Assert.notNull(timeout, "Microsoft Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return missing();
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(MicrosoftManifest.realmDescription())));
        }
        ...
        if (capability.equals(Realm.CHANGES) && request instanceof Realm.Changes changes) {
    ...
    private CompletionStage<Outcome<Realm.Page>> snapshot(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "Microsoft Realm snapshot request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Microsoft Realm snapshot has no remaining timeout"));
        }
    ...
    private Outcome<Realm.Page> snapshot(
        ...
        try {
            ...
            return failed(ErrorCode._500, "Microsoft Realm snapshot processing failed locally");
        }
    }
    ...
    private Outcome<Realm.Page> organization(
            ...
            final Instant observedAt,
            final Timeout timeout) {
        final Url url = initial(Builder.REALM_ORGANIZATION, Map.of("$select", "id,displayName"));
     ...
     * @param mapper     implementation-neutral resource mapper
    ...
    private Outcome<Realm.Page> managers(
        ...
        final Url url = child(Builder.REALM_USER, userId, "manager", Map.of("$select", "id"));
    ...
    private Outcome<Realm.Page> members(
        ...
        final String targetName = phase == Phase.GROUP_MEMBERS ? Builder.REALM_GROUP_MEMBERS
                : Builder.REALM_ROLE_MEMBERS;
     ...
     *
     * @param request exact implementation-neutral change request
     * @param context immutable context used only for credential loading
     * @param timeout shared end-to-end timeout
    ...
    private CompletionStage<Outcome<Realm.ChangePage>> changes(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "Microsoft Realm changes request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Microsoft Realm changes has no remaining timeout"));
        }
        final Instant observedAt = FabricX.clock().now();
    ...
    private Outcome<Realm.ChangePage> changes(
        ...
        try {
            ...
            return failed(ErrorCode._500, "Microsoft Realm delta processing failed locally");
        }
    }
    ...
    private CompletionStage<Outcome<Realm.Retrieved>> retrieve(
        ...
        if (!SUPPORTED_KINDS.contains(request.key().kind())) {
            return completed(rejected(ErrorCode._400, "Microsoft Realm retrieve kind is unsupported"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Microsoft Realm retrieve has no remaining timeout"));
        }
        final Instant observedAt = FabricX.clock().now();
    ...
    private Outcome<Realm.Retrieved> retrieve(
        ...
        try {
            ...
            return failed(ErrorCode._500, "Microsoft Realm retrieval processing failed locally");
        }
    }
     ...
     *
     * @param access     valid Graph application access
     * @param key        requested implementation-neutral key
     * @param observedAt shared invocation observation instant
     * @param timeout    shared end-to-end timeout
     * @param targetName manifest management target name
     * @param parser     minimal wire projection parser
    ...
    private Outcome<Realm.Retrieved> retrieveOrganization(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = get(
                initial(Builder.REALM_ORGANIZATION, Map.of("$select", "id,displayName")),
                access,
                timeout,
                "retrieve",
                false);
        ...
        try {
            return CompletableFuture.<Outcome<T>>supplyAsync(() -> {
                try {
                    ...
                    return failed(ErrorCode._408, "Microsoft Realm operation timed out");
                } catch (RuntimeException ignored) {
                    return failed(ErrorCode._503, "Microsoft Graph transport is unavailable");
                }
            }, services.executor());
        } catch (RuntimeException ignored) {
            return completed(failed(ErrorCode._500, "Microsoft Realm operation could not be scheduled"));
        }
    }
    ...
    private Outcome<Optional<String>> parent(
        ...
        final String targetName = switch (phase) {
            case USER_MANAGERS -> Builder.REALM_USERS;
            case GROUP_MEMBERS -> Builder.REALM_GROUPS;
            case ROLE_MEMBERS -> Builder.REALM_ROLES;
            case APPLICATION_ASSIGNMENTS -> Builder.REALM_SERVICE_ACCOUNTS;
            default -> throw new ValidateException("Microsoft phase has no parent collection");
        };
        Url url = listUrl(targetName, phase.parentPhase(), Builder.MAXIMUM_REALM_PAGE_SIZE);
    ...
    private Url deltaUrl(final Phase phase) {
        ...
        final String projection = phase == Phase.DELTA_GROUPS ? select(phase.kind()) + ",members"
                : select(phase.kind());
        return build(target(Builder.REALM_CHANGES).url(), path, Map.of("$select", projection));
    }
    ...
    private Endpoint target(final String name) {
        ...
        if (endpoint == null) {
            throw new ValidateException("Microsoft Realm manifest omits a required management target");
        }
        return endpoint;
    }
    ...
    /**
     * Encodes one canonical six-field Microsoft Realm cursor.
     *
    ...
    /**
     * Decodes and canonicalizes one Microsoft Realm cursor.
     *
     * @param cursor    opaque caller-supplied cursor
     * @param operation expected Realm operation
    ...
    private CursorState decode(
        ...
        try {
            final JsonValue value = JsonKit
                    .readValue(Base64.decode(cursor.value()), Builder.MAXIMUM_REALM_JSON_DEPTH, true);
    ...
    private enum Phase {
        ...
        /**
         * Realm operation owning this phase.
         */
    ...
    /**
     * Maps one minimal wire projection to a implementation-neutral resource.
     *
     * @param <T> minimal wire projection type
     * @author Kimi Liu
     */
    ...
    private interface ResourceMapper<T> {
         ...
         * @param observedAt shared invocation observation instant
         * @return immutable implementation-neutral resource
         */
