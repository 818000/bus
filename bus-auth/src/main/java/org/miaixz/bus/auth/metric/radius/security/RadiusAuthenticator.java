...
public final class RadiusAuthenticator {
     ...
     */
    public static byte[] access(final RandomSource random) {
        final byte[] result = Assert
                .notNull(random, () -> new ValidateException("RADIUS random source must not be null"))
                .nextBytes(RadiusPacket.AUTHENTICATOR_BYTES);
        Assert.isTrue(
                result.length == RadiusPacket.AUTHENTICATOR_BYTES,
                () -> new ValidateException("RADIUS random source returned an invalid length"));
        return result.clone();
    }
    ...
    static byte[] digest(final RadiusPacket packet, final byte[] authenticator, final byte[] secret) {
        final byte[] key = Arrays.copyOf(
                ...
                secret.length);
        try {
            final RadiusPacket substituted = new RadiusPacket(packet.code(), packet.identifier(), authenticator,
                    packet.attributes());
