...
public class AccessService {
    ...
    /**
     * Creates an Access service for one compiled RADIUS server.
     *
    ...
    public CompletionStage<Outcome<RadiusPacket>> access(
        ...
        try {
            resolved = handler.resolve(sourceId, context.network().remoteAddress(), effective, context, timeout);
    ...
    private CompletionStage<Outcome<RadiusPacket>> invokeHandler(
        ...
        try {
            stage = handler.access(sourceId, client, request, context, timeout);
        } catch (RuntimeException exception) {
    ...
    /**
     * Validates version selection, RADIUS/1.1 trusted transport, and server version policy.
     *
