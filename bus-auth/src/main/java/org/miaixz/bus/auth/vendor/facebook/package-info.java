 ...
 * <p>
 * FacebookDefinition exposes {@code facebook/default} against the fixed Graph API v26.0 authorization, token, and
 * profile endpoints. It requires CLIENT_SECRET, prohibits PKCE, defaults to {@code public_profile} and {@code email},
 * and publishes only Source authentication and the standard OAuth authorization operation. The GET token request, query
 * client secret, missing token type, Graph error object, and profile proof are registered deviations.
 * </p>
 * <p>
 * FacebookSourceSettings contains routing, client, secret-reference, exact callback, and registered scope values only.
