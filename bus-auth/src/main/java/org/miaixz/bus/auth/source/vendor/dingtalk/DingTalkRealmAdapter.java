...
/**
 * Implements the implementation-neutral DingTalk realm surface over the official management API.
 * <p>
 ...
 * @author Kimi Liu
 */
public class DingTalkRealmAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable DingTalk Realm Variant.
     */
    private final VendorManifest.Variant variant;

    /**
     * Validated DingTalk Realm deployment options containing only a credential reference.
     */
     ...
     * @param options  validated DingTalk Realm options
    ...
    public DingTalkRealmAdapter(final String spaceId, final String sourceId, final DingTalkManifest manifest,
            final VendorManifest.Variant variant, final DingTalkOptions options, final DriverServices services) {
        Assert.notBlank(spaceId, "DingTalk Realm space id must not be blank");
        Assert.notBlank(sourceId, "DingTalk Realm Source id must not be blank");
        final DingTalkManifest selectedManifest = Assert.notNull(manifest, "DingTalk Realm manifest must not be null");
        this.variant = Assert.notNull(variant, "DingTalk Realm Variant must not be null");
        this.options = Assert.notNull(options, "DingTalk Realm options must not be null");
        this.services = Assert.notNull(services, "DingTalk Realm services must not be null");
        ...
        if (this.targets.token().isEmpty()
            ...
            throw new ValidateException("DingTalk Realm manifest has an invalid management target set");
        }
        if (!this.targets.management().get(Builder.REALM_USERS)
                .equals(this.targets.management().get(Builder.REALM_USER))) {
            throw new ValidateException("DingTalk Realm user-detail targets must resolve identically");
        }
    ...
    /**
     * Converts one role projection into a implementation-neutral role resource.
     *
    ...
    private static void requireKinds(final Set<Realm.Kind> kinds) {
        if (!SUPPORTED_KINDS.containsAll(kinds)) {
            throw new ValidateException("DingTalk Realm snapshot contains an unsupported kind");
        }
    }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(
                rejected(ErrorCode._400, "DingTalk Realm capability is not declared by the selected manifest"));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> mismatch() {
        return completed(
                rejected(ErrorCode._400, "DingTalk Realm request does not match the selected capability contract"));
    }
    ...
    /**
     * Routes one exact DingTalk Realm capability and request type.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "DingTalk Realm capability must not be null");
        Assert.notNull(context, "DingTalk Realm context must not be null");
        Assert.notNull(timeout, "DingTalk Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return missing();
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(DingTalkManifest.realmDescription())));
        }
    ...
    private CompletionStage<Outcome<Realm.Page>> snapshot(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "DingTalk Realm snapshot request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "DingTalk Realm snapshot has no remaining timeout"));
        }
    ...
    private Outcome<Realm.Page> snapshot(
        ...
        try {
            ...
            return failed(ErrorCode._500, "DingTalk Realm snapshot processing failed locally");
        }
    }
    ...
    private CompletionStage<Outcome<Realm.Retrieved>> retrieve(
        ...
        if (!SUPPORTED_KINDS.contains(request.key().kind())) {
            return completed(rejected(ErrorCode._400, "DingTalk Realm retrieve kind is unsupported"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "DingTalk Realm retrieve has no remaining timeout"));
        }
        final Instant observedAt = FabricX.clock().now();
    ...
    private Outcome<Realm.Retrieved> retrieve(
        ...
        try {
            return switch (key.kind()) {
                ...
                default -> rejected(ErrorCode._400, "DingTalk Realm retrieve kind is unsupported");
            };
        } catch (ValidateException ignored) {
            return failed(ErrorCode._502, "DingTalk management API returned an invalid retrieval projection");
        } catch (RuntimeException ignored) {
            return failed(ErrorCode._500, "DingTalk Realm retrieval processing failed locally");
        }
    }
    ...
    private <T> CompletionStage<Outcome<T>> execute(
        ...
        return CompletableFuture.<Outcome<T>>supplyAsync(() -> {
            if (timeout.expired()) {
                return failed(ErrorCode._408, "DingTalk Realm operation has no remaining timeout");
            }
            try {
                return Assert.notNull(operation.apply(access), "DingTalk Realm operation returned no outcome");
            } catch (TimeoutException ignored) {
                return failed(ErrorCode._408, "DingTalk Realm operation timed out");
            } catch (RuntimeException ignored) {
    ...
    private Outcome<List<Department>> departmentChildren(
            ...
            final Timeout timeout,
            final String operation) {
        final Endpoint endpoint = target(Builder.REALM_ORGANIZATIONS);
        final Map<String, JsonValue> fields = new LinkedHashMap<>();
    ...
    private Outcome<WirePage<User>> userPage(
            ...
            final Timeout timeout) {
        final Endpoint endpoint = target(Builder.REALM_ORGANIZATION_USERS);
    ...
    private Outcome<WirePage<Role>> rolePage(
            ...
            final Timeout timeout,
            final String operation) {
        final Endpoint endpoint = target(Builder.REALM_ROLES);
    ...
    private Outcome<WirePage<RoleMember>> memberPage(
            ...
            final Optional<String> next,
            final Timeout timeout) {
        final Endpoint endpoint = target(Builder.REALM_ROLE_MEMBERS);
    ...
    private String fingerprint(final String previous, final Department department) {
        ...
        final byte[] encoded = JsonKit.writeValue(new JsonValue.ObjectValue(projection));
    ...
    private Endpoint target(final String name) {
        ...
        if (endpoint == null) {
            throw new ValidateException("DingTalk Realm manifest omits a required management target");
        }
        return endpoint;
    }
