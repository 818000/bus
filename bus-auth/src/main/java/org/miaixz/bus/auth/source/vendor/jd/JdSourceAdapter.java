...
public class JdSourceAdapter implements VendorAdapter {
    ...
    /**
     * Source identifier used by produced external identities.
     */
     ...
     *
     * @param spaceId  Source space used for state isolation
    ...
    public JdSourceAdapter(final String spaceId, final String sourceId, final JdManifest manifest,
            final VendorManifest.Variant variant, final JdOptions options, final SourceServices services) {
    ...
    /**
     * Requires one implementation-neutral JSON object.
     *
     ...
     * @return verified external identity
     */
    private Outcome<Identity> profile(final PrivateToken token, final SecretLease secret, final Timeout timeout) {
     ...
     * @return verified external identity
     */
    private Outcome<Identity> identity(final JsonValue.ObjectValue object, final String openId, final Timeout timeout) {
        ...
        final Evidence evidence = new Evidence(Evidence.Type.FEDERATED, Evidence.Strength.SINGLE_FACTOR,
                new Evidence.Claim("open_id", new JsonValue.StringValue(openId), "https://open.jd.com",
                        timeout.clock().now()));
        return Outcome
                .succeeded(new Identity(sourceId, openId, new JsonValue.ObjectValue(attributes), List.of(evidence)));
    }
