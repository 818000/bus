...
import org.miaixz.bus.auth.vendor.github.GitHubSourceSettings;
...
public final class GitHubSourceAdapter implements VendorAdapter {
    ...
    /**
     * Exact GitHub token error member vocabulary.
     */
    /**
    ...
    /**
     * Selected immutable GitHub definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded GitHub settings.
     */
    private final GitHubSourceSettings settings;
     ...
     * @param vendorDefinition  selected GitHub definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded GitHub settings
     ...
     */
    public GitHubSourceAdapter(final String namespaceId, final String sourceId, final GitHubDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final GitHubSourceSettings settings,
            final ExecutionServices services) {
        final GitHubDefinition selected = Assert.notNull(vendorDefinition, "GitHub definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "GitHub Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "GitHub definition must not be null");
        this.settings = Assert.notNull(settings, "GitHub settings must not be null");
        this.services = Assert.notNull(services, "GitHub execution services must not be null");
        if (!GitHubDefinition.ID.equals(selected.type())
                || !selected.variant(GitHubDefinition.DEFAULT).equals(variantDefinition)
                || !GitHubDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !GitHubDefinition.ID.equals(settings.vendor())
                || !GitHubDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("GitHub adapter requires the github/default OAuth 2.0 definition");
        }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected GitHub definition.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected("GitHub capability is not declared by the selected definition"));
        }
