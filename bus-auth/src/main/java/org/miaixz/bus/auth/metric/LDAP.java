...
import org.miaixz.bus.auth.metric.ldap.client.LdapClient;
import org.miaixz.bus.auth.metric.ldap.server.LdapServer;
 ...
 * non-final class and public empty constructor remain compatible; product code obtains managed protocol instances only
 * through {@link #client(ClientConfig, Runtime)} and {@link #server(ServerConfig, Runtime, Directory)}.
 *
 * @author Kimi Liu
 */
public class LDAP {
    ...
    public static Client client(final ClientConfig configuration, final Runtime runtime) {
        return new LdapClient(configuration, runtime);
    }
     ...
     */
    public static Server server(final ServerConfig configuration, final Runtime runtime, final Directory directory) {
        return new LdapServer(configuration, runtime, directory);
    }
    ...
    public interface Client {
         ...
         */
        CompletionStage<Outcome<Void>> startTls(Invocation invocation);
         ...
         * @return bind outcome
         */
        CompletionStage<Outcome<BindResult>> bind(Invocation invocation, BindRequest request);
         ...
         */
        CompletionStage<Outcome<SearchResult>> search(Invocation invocation, SearchRequest request);
         ...
         */
        CompletionStage<Outcome<CompareResult>> compare(Invocation invocation, CompareRequest request);
         ...
         */
        CompletionStage<Outcome<Void>> abandon(Invocation invocation, int messageId);
         ...
         */
        CompletionStage<Outcome<Void>> unbind(Invocation invocation);
    ...
    public interface Directory {
         ...
         */
        CompletionStage<BindDecision> bind(Invocation invocation, BindIdentity identity);
         ...
         */
        CompletionStage<DirectorySearchResult> search(Invocation invocation, DirectorySearch search);
         ...
         */
        CompletionStage<CompareDecision> compare(Invocation invocation, DirectoryCompare compare);
    }
     ...
     */
    public record ClientConfig(Endpoint endpoint, TransportPolicy transportPolicy, boolean connectWithTls,
        ...
        public ClientConfig {
            ...
            Assert.isTrue(
                    endpoint.protocol() == Protocol.LDAP || endpoint.protocol() == Protocol.LDAPS,
                    () -> new ValidateException("LDAP client endpoint protocol must be LDAP or LDAPS"));
            Assert.isTrue(
                    transportPolicy.allowedSchemes().equals(Set.of(endpoint.protocol()))
                            && transportPolicy.allowedPorts().contains(endpoint.port())
                            && transportPolicy.requireStartTls() == (endpoint.protocol() == Protocol.LDAP),
                    () -> new ValidateException("LDAP client transport policy does not match the endpoint"));
            Assert.isTrue(
                    connectWithTls == (endpoint.protocol() == Protocol.LDAPS),
     ...
     */
    public record ServerConfig(Endpoint endpoint, TransportPolicy transportPolicy, Duration shutdownTimeout,
            int maximumPageSize) {
        ...
        public ServerConfig {
            ...
            Assert.isTrue(
                    endpoint.protocol() == Protocol.LDAP || endpoint.protocol() == Protocol.LDAPS,
                    () -> new ValidateException("LDAP server endpoint protocol must be LDAP or LDAPS"));
            Assert.isTrue(
                    transportPolicy.allowedSchemes().equals(Set.of(endpoint.protocol()))
                            && transportPolicy.allowedPorts().contains(endpoint.port())
                            && transportPolicy.requireStartTls() == (endpoint.protocol() == Protocol.LDAP),
