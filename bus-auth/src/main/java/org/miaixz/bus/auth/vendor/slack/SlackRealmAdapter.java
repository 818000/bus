...
/**
 * Implements the provider-neutral Slack administrator SCIM realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public final class SlackRealmAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Slack SCIM Variant.
     */
    private final VariantManifest.Variant variant;

    /**
     * Validated Slack SCIM deployment options.
     */
    ...
    /**
     * Creates one Source-isolated Slack administrator SCIM realm adapter.
     *
     * @param spaceId  registration space used for isolation
     * @param sourceId registered Source identifier
     * @param manifest exact Slack manifest
     ...
     */
    public SlackRealmAdapter(final String spaceId, final String sourceId, final SlackManifest manifest,
            final VariantManifest.Variant variant, final SlackOptions options, final DriverServices services) {
        Assert.notBlank(spaceId, "Slack SCIM space id must not be blank");
        Assert.notBlank(sourceId, "Slack SCIM Source id must not be blank");
        final SlackManifest selectedManifest = Assert.notNull(manifest, "Slack manifest must not be null");
        this.variant = Assert.notNull(variant, "Slack SCIM Variant must not be null");
        this.options = Assert.notNull(options, "Slack SCIM options must not be null");
        this.services = Assert.notNull(services, "Slack SCIM services must not be null");
        if (!SlackManifest.ID.equals(selectedManifest.vendor()) || !SlackManifest.SCIM.equals(this.variant.variant())
                || !selectedManifest.variant(this.variant.variant()).equals(this.variant)
                || this.variant.protocol() != Protocol.SCIM || !SlackManifest.ID.equals(this.options.vendor())
                || !SlackManifest.SCIM.equals(this.options.variant()) || this.options.redirectUri().isPresent()
                || !this.options.scopes().isEmpty()
                || this.options.credential().type() != org.miaixz.bus.auth.Credential.Type.SHARED_SECRET) {
            throw new ValidateException("Slack realm adapter requires the frozen administrator SCIM Variant");
        }
        ...
        if (!List.copyOf(this.targets.management().keySet()).equals(MANAGEMENT_TARGETS)) {
            throw new ValidateException("Slack SCIM manifest has an invalid management target set");
        }
    }
    ...
    private static User user(final JsonValue.ObjectValue value) {
        final String display = fallback(
                value,
                "displayName",
                () -> requiredString(requiredObject(value, "name"), "formatted"));
        return new User(requiredString(value, "id"), requiredString(value, "userName"), display,
    ...
    /**
     * Reads a positive integer member.
     *
     * @param object parent object
     * @param name   member name
     * @return positive integer
     */
    private static int positiveInt(final JsonValue.ObjectValue object, final String name) {
        final int value = requiredInt(object, name);
        if (value <= 0) {
    ...
    private static int positiveDecimal(final String value) {
        try {
            ...
            if (result <= 0 || !Integer.toString(result).equals(value)) {
                throw new ValidateException("Slack cursor index must be canonical and positive");
            }
            return result;
        } catch (NumberFormatException cause) {
            throw new ValidateException("Slack cursor index is invalid", cause);
        }
    }
    ...
    private static void exact(final JsonValue.ObjectValue object, final Set<String> expected) {
        if (!object.values().keySet().equals(expected)) {
            throw new ValidateException("Slack cursor contains an invalid member set");
        }
    }
    ...
    private static List<Realm.Kind> kinds(final List<JsonValue> values) {
        ...
        for (JsonValue value : values) {
            if (!(value instanceof JsonValue.NumberValue number)) {
                throw new ValidateException("Slack cursor kind must be numeric");
            }
            final int code;
            try {
                code = number.value().intValueExact();
            } catch (ArithmeticException cause) {
                throw new ValidateException("Slack cursor kind is invalid", cause);
            }
            final Realm.Kind kind = kind(code);
            if (code <= previous || !result.add(kind)) {
                throw new ValidateException("Slack cursor kinds are not canonical");
            }
            previous = code;
        }
    ...
    private static Realm.Kind kind(final int code) {
        ...
        throw new ValidateException("Slack cursor contains an unsupported kind");
    }
    ...
    private static void requireKinds(final Set<Realm.Kind> kinds) {
        if (kinds.isEmpty() || !SUPPORTED_KINDS.containsAll(kinds)) {
            throw new ValidateException("Slack snapshot contains an unsupported kind");
        }
    }
    ...
    private static <S> CompletionStage<Outcome<S>> narrow(
        ...
        return stage.thenApply(outcome -> switch (outcome) {
            ...
            default -> failed(ErrorCode._500, "Slack delegated outcome is unsupported");
        });
    }
    ...
    private static <T> Outcome<T> propagate(final Outcome<?> outcome) {
        return switch (outcome) {
            case Outcome.Rejected<?> rejected -> Outcome.rejected(rejected.failure());
            case Outcome.Failed<?> failed -> Outcome.failed(failed.failure());
            default -> failed(ErrorCode._500, "Slack internal outcome cannot be propagated");
        };
    }
    ...
    /**
     * Routes one exact Slack SCIM enterprise capability.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Slack enterprise capability must not be null");
        Assert.notNull(context, "Slack enterprise context must not be null");
        Assert.notNull(timeout, "Slack enterprise timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected(ErrorCode._400, "Slack enterprise capability is not declared"));
        }
        if (capability.equals(Realm.describe(SlackManifest.ID)) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(SlackManifest.enterpriseDescription())));
        }
        if (capability.equals(Realm.snapshot(SlackManifest.ID)) && request instanceof Realm.Snapshot value) {
            return narrow(snapshot(value, context, timeout), capability.responseType());
        }
        if (capability.equals(Realm.retrieve(SlackManifest.ID)) && request instanceof Realm.Retrieve value) {
            return narrow(retrieve(value, context, timeout), capability.responseType());
        }
        return completed(rejected(ErrorCode._400, "Slack enterprise request does not match the capability contract"));
    }
    ...
    private CompletionStage<Outcome<Realm.Page>> snapshot(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "Slack SCIM snapshot request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Slack SCIM snapshot has no remaining timeout"));
        }
    ...
    /**
     * Reads one bounded Slack SCIM user page.
     *
    ...
    private Outcome<Realm.Page> users(
            ...
            final Timeout timeout) {
        final int count = Math.min(request.limit(), Normal._1000);
        ...
        for (User user : success.value().items()) {
            putResource(resources, userResource(user, observedAt), "Slack user page");
        }
    ...
    /**
     * Reads one Slack SCIM group page and resumes embedded members without duplication.
     *
     * @param secret     open administrator-token lease
     * @param request    snapshot request
    ...
    private Outcome<Realm.Page> groups(
        ...
        if (state.position().parentId().isPresent()) {
            ...
            if (groupIndex == groups.size()) {
                return failed(ErrorCode._502, "Slack SCIM replay group no longer exists on its official page");
            }
        ...
        while (groupIndex < groups.size()) {
            ...
            if (memberOffset == 0) {
                putResource(resources, groupResource(group, observedAt), "Slack group page");
            }
            if (memberOffset > group.members().size()) {
                return failed(ErrorCode._502, "Slack SCIM group-member replay offset exceeds the current projection");
            }
    ...
    private CompletionStage<Outcome<Realm.Retrieved>> retrieve(
        ...
        if (!SUPPORTED_KINDS.contains(request.key().kind())) {
            return completed(rejected(ErrorCode._400, "Slack SCIM retrieve kind is unsupported"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Slack SCIM retrieve has no remaining timeout"));
        }
        final Instant observedAt = FabricX.clock(services.fabric()).now();
    ...
    /**
     * Retrieves one official SCIM User or Group resource.
     *
     * @param secret     open administrator-token lease
    ...
    private Outcome<Realm.Retrieved> retrieve(
            ...
            final Instant observedAt,
            final Timeout timeout) {
        final String targetName = key.kind() == Realm.Kind.USER ? Builder.ENTERPRISE_USER : Builder.ENTERPRISE_GROUP;
        final Outcome<JsonValue.ObjectValue> fetched = get(
                url(targetName, key.externalId()),
                secret,
                timeout,
                "retrieve",
                true);
        ...
        final Realm.Resource resource = key.kind() == Realm.Kind.USER ? userResource(user(success.value()), observedAt)
                : groupResource(group(success.value()), observedAt);
        if (!key.equals(resource.key())) {
            return failed(ErrorCode._502, "Slack SCIM retrieval returned a different stable identifier");
        }
    ...
    private <T> CompletionStage<Outcome<T>> withSecret(
        ...
        try {
            ...
            return completed(failed(ErrorCode._500, "Slack SCIM Secret Loader failed before returning a stage"));
        }
        if (loaded == null) {
            return completed(failed(ErrorCode._500, "Slack SCIM Secret Loader returned no stage"));
        }
        return loaded.<Outcome<SecretLoader.Record>>handle(
                (outcome, cause) -> cause == null && outcome != null ? outcome
                        : failed(ErrorCode._500, "Slack SCIM Secret Loader stage failed"))
                .thenCompose(outcome -> switch (outcome) {
                    ...
                    default -> completed(
                            failed(ErrorCode._500, "Slack SCIM Secret Loader returned an unsupported outcome"));
                });
    }
    ...
    private <T> CompletionStage<Outcome<T>> execute(
        ...
        try {
            ...
            return completed(failed(ErrorCode._500, "Slack SCIM loaded Secret could not be validated"));
        }
        try {
            return CompletableFuture.<Outcome<T>>supplyAsync(() -> {
                if (timeout.expired()) {
                    return failed(ErrorCode._408, "Slack SCIM operation has no remaining timeout");
                }
                try {
                    return Assert.notNull(operation.apply(secret), "Slack SCIM operation returned no outcome");
                } catch (TimeoutException ignored) {
                    return failed(ErrorCode._408, "Slack SCIM operation timed out");
                } catch (RuntimeException ignored) {
                    return failed(ErrorCode._503, "Slack SCIM transport is unavailable");
                }
            }, services.executor()).whenComplete((ignored, cause) -> secret.close());
        } catch (RuntimeException ignored) {
            secret.close();
            return completed(failed(ErrorCode._500, "Slack SCIM operation could not be scheduled"));
        }
    }
    ...
    private <T> Outcome<ScimPage<T>> page(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = get(request, secret, timeout, operation, false);
        if (!(fetched instanceof Outcome.Succeeded<JsonValue.ObjectValue> success)) {
            return propagate(fetched);
        }
        final JsonValue.ObjectValue envelope = success.value();
        final int total = nonNegativeInt(envelope, "totalResults");
        final int returnedStart = positiveInt(envelope, "startIndex");
        final int itemsPerPage = nonNegativeInt(envelope, "itemsPerPage");
        final List<JsonValue> values = requiredArray(envelope, "Resources");
        if (returnedStart != start || itemsPerPage != values.size() || itemsPerPage > count
                || total < Math.max(0, start - 1) + itemsPerPage) {
            return failed(ErrorCode._502, "Slack SCIM pagination envelope is inconsistent");
        }
        final List<T> items = new ArrayList<>(values.size());
        for (JsonValue value : values) {
            items.add(parser.apply(requiredObject(value, "Slack SCIM resource")));
        }
        final int next = itemsPerPage > 0 && start - 1 + itemsPerPage < total ? start + itemsPerPage : 0;
     ...
     * @param timeout       shared timeout
     * @param operation     safe operation label
     * @param allowNotFound whether 404 means explicit absence
     * @return decoded object, absence, or classified failure
     */
    private Outcome<JsonValue.ObjectValue> get(
        ...
        if (timeout.expired()) {
            return failed(ErrorCode._408, "Slack SCIM request has no remaining timeout");
        }
        ...
        try {
            ...
            return failed(ErrorCode._408, "Slack SCIM request timed out");
        } catch (RuntimeException ignored) {
            return failed(ErrorCode._503, "Slack SCIM endpoint is unavailable");
        }
        try (response) {
            if (!response.successful()) {
                return httpFailure(response, operation, allowNotFound);
            }
            return Outcome.succeeded(object(response));
        } catch (RuntimeException ignored) {
            return failed(ErrorCode._502, "Slack SCIM returned an invalid response");
        }
    }
     ...
     * @param operation     safe operation label
     * @param allowNotFound whether 404 means explicit absence
     * @return classified failure, rejection, or absence
     */
    private Outcome<JsonValue.ObjectValue> httpFailure(
            final Response response,
            final String operation,
            final boolean allowNotFound) {
        ...
        try {
            ...
            if (schemas.size() != 1 || !(schemas.getFirst() instanceof JsonValue.StringValue schema)
                ...
                return failed(ErrorCode._502, "Slack SCIM returned an invalid error envelope");
            }
        } catch (RuntimeException ignored) {
            return failed(ErrorCode._502, "Slack SCIM returned an invalid error envelope");
        }
        ...
        if (status == Http.Status.BAD_REQUEST || status == Http.Status.CONFLICT
            ...
            return rejected(ErrorCode._400, "Slack SCIM rejected the request", details);
        }
        if (status == Http.Status.UNAUTHORIZED) {
            return rejected(ErrorCode._401, "Slack SCIM rejected the administrator token", details);
        }
        if (status == Http.Status.FORBIDDEN) {
            return rejected(ErrorCode._403, "Slack SCIM plan, installation, or permission is insufficient", details);
        }
        if (status == Http.Status.NOT_FOUND && allowNotFound) {
            return Outcome.succeeded(null);
        }
        if (status == Http.Status.TOO_MANY_REQUESTS) {
            return failed(ErrorCode._429, "Slack SCIM is rate limited", details);
        }
        return failed(ErrorCode._502, "Slack SCIM returned an upstream error", details);
    }

    /**
     * Builds one direct resource URL from a fixed manifest target and stable identifier.
     *
     * @param targetName target key
     * @param id         stable resource identifier
     * @return immutable encoded resource URL
     */
    private Url url(final String targetName, final String id) {
        final Url base = target(targetName).url();
        return Url.builder().scheme(base.scheme()).host(base.host()).path(
                base.path() + "/"
                        + RFC3986.SEGMENT.encode(requireText(id, "Slack SCIM resource identifier"), Charset.UTF_8))
                .build();
    }
    ...
    private Endpoint target(final String name) {
        ...
        if (endpoint == null) {
            throw new ValidateException("Slack SCIM manifest omits a required management target");
        }
        return endpoint;
    }
    ...
    private Realm.Cursor encode(final CursorState state) {
        ...
        envelope.put(Builder.VENDOR_FIELD, new JsonValue.StringValue(SlackManifest.ID.value()));
        envelope.put(Builder.VARIANT_FIELD, new JsonValue.StringValue(SlackManifest.SCIM.value()));
    ...
    private CursorState decode(final Realm.Cursor cursor, final Set<Realm.Kind> kinds) {
        try {
            ...
            if (!SlackManifest.ID.value().equals(requiredString(envelope, Builder.VENDOR_FIELD))
                    || !SlackManifest.SCIM.value().equals(requiredString(envelope, Builder.VARIANT_FIELD))
                    || requiredInt(envelope, Builder.OPERATION_FIELD) != Realm.Operation.SNAPSHOT.code()) {
                throw new ValidateException("Slack cursor context is invalid");
            }
            ...
            if (!decodedKinds.equals(List.copyOf(kinds))) {
                throw new ValidateException("Slack cursor kinds do not match the request");
            }
            ...
            if (!encode(state).value().equals(cursor.value())) {
                throw new ValidateException("Slack cursor is not canonical");
            }
            return state;
        } catch (ValidateException cause) {
            throw cause;
        } catch (RuntimeException cause) {
            throw new ValidateException("Slack cursor is invalid", cause);
        }
    }
    ...
    private Map<String, JsonValue> details(final String operation, final int status, final FabricX.Headers headers) {
        ...
        values.put(Builder.VENDOR_FIELD, new JsonValue.StringValue(SlackManifest.ID.value()));
    ...
    /**
     * Defines the finite Slack snapshot phase order.
     *
     * @author Kimi Liu
     */
    private enum Phase {
        ...
        private static Phase from(final int code) {
            ...
            throw new ValidateException("Slack cursor phase is unknown");
        }
    }
    ...
    private record CursorState(Phase phase, List<Realm.Kind> kinds, Position position) {
         ...
         */
        private CursorState {
            phase = Assert.notNull(phase, "Slack cursor phase must not be null");
            kinds = List.copyOf(Assert.notNull(kinds, "Slack cursor kinds must not be null"));
            position = Assert.notNull(position, "Slack cursor position must not be null");
            if (kinds.isEmpty() || !SUPPORTED_KINDS.containsAll(kinds) || !kinds.contains(phase.kind)) {
                throw new ValidateException("Slack cursor kinds or phase are invalid");
            }
    ...
    private record Position(int start, Optional<String> parentId, int relationOffset) {
        ...
        private Position {
            if (start <= 0 || relationOffset < 0) {
                throw new ValidateException("Slack cursor position is out of range");
            }
            parentId = Assert.notNull(parentId, "Slack cursor parent container must not be null");
            parentId = parentId.isPresent()
                    ? Optional.of(requireText(parentId.getOrNull(), "Slack cursor parent identifier"))
                    : Optional.empty();
        }
        ...
        private void validate(final Phase phase) {
            if (phase == Phase.USERS && (parentId.isPresent() || relationOffset != 0)
                ...
                throw new ValidateException("Slack cursor position does not belong to its phase");
            }
        }
    }
    ...
    private record ScimPage<T>(List<T> items, int next) {
         ...
         */
        private ScimPage {
            items = List.copyOf(Assert.notNull(items, "Slack SCIM items must not be null"));
            if (next < 0) {
                throw new ValidateException("Slack SCIM next index must not be negative");
            }
        }
    }

    /**
     * Minimal Slack SCIM user projection.
     *
     ...
     * @author Kimi Liu
     */
    private record User(String id, String userName, String displayName, boolean active) {

        /**
         * Validates one user.
         */
        private User {
            id = requireText(id, "Slack user identifier");
            userName = requireText(userName, "Slack user name");
            displayName = requireText(displayName, "Slack user display name");
        }
    }

    /**
     * Minimal Slack SCIM group projection.
     *
     * @param id          stable identifier
     * @param displayName display name
    ...
    private record Group(String id, String displayName, List<String> members) {
         ...
         */
        private Group {
            id = requireText(id, "Slack group identifier");
            displayName = requireText(displayName, "Slack group display name");
            members = List.copyOf(Assert.notNull(members, "Slack group members must not be null"));
        }
    }
