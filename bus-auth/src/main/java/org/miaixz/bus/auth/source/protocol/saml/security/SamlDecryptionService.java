...
public class SamlDecryptionService {
    ...
    private CompletionStage<Outcome<Assertion>> decryptPlaintext(
        ...
        try {
            ...
            if (plaintext.length == 0 || plaintext.length > policies.require(Protocol.SAML).maximumMessageBytes()) {
    ...
    /**
     * Ensures both selected algorithms are explicitly allowed by the shared SAML security rule.
     *
    ...
    private void algorithms(final EncryptedPayload payload) {
        ...
        final Set<String> allowed = policies.require(Protocol.SAML).algorithms();
        if (!allowed.contains(payload.keyAlgorithm()) || !allowed.contains(payload.contentAlgorithm())) {
            throw new ValidateException("SAML encrypted assertion algorithm is not permitted by its security rule");
        }
    }
