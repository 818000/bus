 ...
 * endpoints remain in {@link AliyunDefinition}; secret material is resolved only for an adapter operation.
 * </p>
 *
 ...
 * @param scopes      ordered requested scopes, or empty to select the definition defaults
 * @author Kimi Liu
 */
public record AliyunSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     */
    public AliyunSourceSettings {
        if (!AliyunDefinition.ID.equals(vendor) || !AliyunDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Alibaba Cloud settings must select aliyun/default");
        }
    ...
    public String toString() {
        return "AliyunSourceSettings[vendor=" + vendor + ", variant=" + variant
