...
import org.miaixz.bus.auth.vendor.apple.AppleSourceSettings;
...
public final class AppleSourceAdapter implements VendorAdapter {
    ...
    /**
     * Exact key-use value supplied to the external key resolver.
     */
    ...
    /**
     * Selected immutable Sign in with Apple definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Sign in with Apple settings.
     */
    private final AppleSourceSettings settings;
     ...
     * @param vendorDefinition  selected Sign in with Apple definition
     * @param variantDefinition selected default variant definition
     * @param settings          decoded externally loaded Sign in with Apple settings
     ...
     *                                  profile
     */
    public AppleSourceAdapter(final String namespaceId, final String sourceId, final AppleDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final AppleSourceSettings settings,
            final ExecutionServices services) {
        Assert.notNull(vendorDefinition, "Sign in with Apple definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Sign in with Apple Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Sign in with Apple definition must not be null");
        this.settings = Assert.notNull(settings, "Sign in with Apple settings must not be null");
        this.services = Assert.notNull(services, "Sign in with Apple execution services must not be null");
        if (!AppleDefinition.ID.equals(vendorDefinition.type())
                || !vendorDefinition.variant(AppleDefinition.DEFAULT).equals(variantDefinition)
                || !AppleDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OIDC || !AppleDefinition.ID.equals(settings.vendor())
                || !AppleDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("Sign in with Apple adapter requires the apple/default OIDC definition");
        }
        validateRedirectUri(settings.redirectUri().getOrNull());
    ...
    /**
     * Returns the exact capability manifest frozen by the selected Apple definition.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        if (capability.equals(OpenIdSourceProfile.TOKEN) && request instanceof TokenRequest tokenRequest) {
            return narrow(token(tokenRequest, context, timeout), capability.responseType());
        }
        if (capability.equals(OpenIdSourceProfile.REVOCATION) && request instanceof RevocationRequest revocation) {
            return narrow(revoke(revocation, context, timeout), capability.responseType());
        }
        if (capability.equals(OpenIdSourceProfile.DISCOVERY) && request == null) {
            return narrow(discover(context, timeout), capability.responseType());
        }
        if (capability.equals(OpenIdSourceProfile.JWK_SET) && request == null) {
    ...
    private CompletionStage<Outcome<ExternalIdentity>> complete(
        ...
        if (completion.codeVerifier().isPresent() || completion.correlation().nonce().isEmpty()) {
            return completed(failed("Sign in with Apple browser security material violates the registered definition"));
        }
        final String code = callback.code();
        final TokenRequest request = new TokenRequest(
                new AuthorizationCodeGrant(code, settings.redirectUri(), Optional.empty(), Optional.empty()),
                emptyObject());
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> token(
            ...
            final Timeout.Budget timeout) {
        if (!valid(request)) {
            return completed(rejected("Sign in with Apple token request differs from the registered OAuth definition"));
        }
    ...
    private boolean valid(final TokenRequest request) {
        ...
        if (request.grant() instanceof AuthorizationCodeGrant grant) {
            return settings.redirectUri().equals(grant.redirectUri()) && grant.codeVerifier().isEmpty()
                    && (grant.clientId().isEmpty() || settings.clientId().equals(grant.clientId().getOrNull()));
        }
    ...
    private Outcome<TokenEndpointResponse> sendToken(
        ...
        try {
            ...
            fields.add(new Parameter(OAuth2.Parameters.CLIENT_ID, settings.clientId()));
    ...
    private Outcome<Void> sendRevocation(
        ...
        try {
            ...
            fields.add(new Parameter(OAuth2.Parameters.CLIENT_SECRET, clientSecret));
            body = formCodec.encode(fields);
            final String endpoint = variantDefinition.targets().resolve(settings).revocation().getOrNull().url()
                    .toString();
    ...
    private CompletionStage<Outcome<String>> clientSecret(final Context context, final Timeout.Budget timeout) {
        ...
        final KeyResolver.Query query = new KeyResolver.Query(ISSUER, Optional.of(settings.credential().id()),
        ...
        if (resolution == null) {
            return completed(failed("Sign in with Apple signing key resolver returned no stage"));
        }
        return resolution.handle((outcome, cause) -> {
            ...
            return switch (outcome) {
                case Outcome.Succeeded<KeyResolver.ResolvedKey> success -> sign(success.value(), now);
     ...
     */
    private Outcome<String> sign(final KeyResolver.ResolvedKey resolved, final Instant now) {
        if (!settings.credential().id().equals(resolved.keyId())
        ...
        try {
            ...
            headers.put(JoseHeader.KEY_ID, new JsonValue.StringValue(settings.keyId()));
            final Map<String, JsonValue> claims = new LinkedHashMap<>();
            claims.put(JwtClaims.ISSUER, new JsonValue.StringValue(settings.teamId()));
            ...
            claims.put(JwtClaims.SUBJECT, new JsonValue.StringValue(settings.clientId()));
    ...
    /**
     * Binds security-relevant discovery members to the frozen Apple definition.
     *
    ...
    private Outcome<OpenIdProviderMetadata> validateMetadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            if (!resolvedTargets.authorization().getOrNull().url().toString().equals(metadata.authorizationEndpoint())
                ...
                throw new ValidateException("Sign in with Apple metadata differs from the frozen definition");
            }
    ...
    private CompletionStage<Outcome<IdTokenClaims>> verifyIdToken(
        ...
        final IdTokenVerifier.Request verification = new IdTokenVerifier.Request(new IdToken(compact),
                new JwtVerifier.Signed(key, Set.of()), ISSUER, settings.clientId(),
    ...
    private CallbackWire callback(final Callback.Inbound callback) {
        ...
        if (!settings.redirectUri().getOrNull().equals(callback.requestUri()) || callback.method() != Http.Method.POST
                || contentType == null
