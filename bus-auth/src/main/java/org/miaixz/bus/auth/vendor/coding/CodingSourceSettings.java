 ...
 * <p>
 * The team value is not an arbitrary URL or host. It supplies only the first DNS label of definition-owned endpoint
 * templates; all endpoint scheme, suffix, paths, methods, and authentication remain immutable definition data.
 * </p>
 *
 ...
 * @param scopes      ordered requested scopes, or empty to use the definition default
 * @param team        canonical single DNS label used by the constrained endpoint templates
 * @author Kimi Liu
 */
public record CodingSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, String team)
     ...
     *                                  profile
     */
    public CodingSourceSettings {
        if (!CodingDefinition.ID.equals(vendor) || !CodingDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("CODING settings must select coding/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("CODING browser settings require a registered redirect URI");
        }
    ...
    /**
     * Returns the constrained team label consumed by definition-owned endpoint templates.
     *
    ...
    public String toString() {
        return "CodingSourceSettings[vendor=" + vendor + ", variant=" + variant
