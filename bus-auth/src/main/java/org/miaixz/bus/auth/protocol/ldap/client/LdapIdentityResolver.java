 ...
 * @author Kimi Liu
 */
public final class LdapIdentityResolver implements IdentityMapper<SearchResultEntry> {

    /**
     * Immutable LDAP Source mapping settings.
     */
     ...
     *
     * @param settings immutable attribute projection settings
     * @param clock    shared verification time source
     * @throws IllegalArgumentException if an argument is {@code null}
     */
    public LdapIdentityResolver(final LdapSourceSettings settings, final Clock clock) {
        this.settings = Assert.notNull(settings, "LDAP identity settings must not be null");
    ...
    public ExternalIdentity map(final String sourceId, final SearchResultEntry input) {
        ...
        settings.attributes().forEach(name -> projectedValues.put(name, new ArrayList<>()));
    ...
    private Map<String, String> configuredNames() {
        final Map<String, String> names = new LinkedHashMap<>(settings.attributes().size());
        settings.attributes().forEach(name -> names.put(name.toLowerCase(Locale.ROOT), name));
        return names;
    }
