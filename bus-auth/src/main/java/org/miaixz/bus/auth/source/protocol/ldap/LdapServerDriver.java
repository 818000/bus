...
/**
 * Compiles one server-role LDAPv3 Source configuration into ten complete-message server operations.
 * <p>
 * Directory data and connection authentication state remain in the exact external {@link DirectoryStore} binding. The
 * compiled runtime only validates Roster routing and dispatches standard LDAPMessage models; it owns no socket, global
 * connection map, persistence implementation, or reverse Roster dependency.
 * </p>
 *
 * @author Kimi Liu
 */
public class LdapServerDriver implements ProtocolDriver<LdapServerOptions> {

    /**
     * Creates the stateless LDAP server driver.
     */
    public LdapServerDriver() {
     ...
     *
     * @return immutable LDAP server scheme
     */
    @Override
    ...
    /**
     * Narrows generic Source options to LDAP server options.
     *
     * @param options generic Source options
     * @return validated LDAP server options
     */
    @Override
    ...
    /**
     * Declares the directory binding slot required by the LDAP server.
     *
     ...
     */
    @Override
    public WorkerSlots slots(final Source source, final LdapServerOptions options) {
        return WorkerSlots.of(WorkerSlots.Slot.BINDING);
    }

    /**
     * Declares the security rule required by the LDAP server.
     *
     * @param source  Source configuration
     * @param options validated LDAP server options
     ...
     */
    @Override
    public Dependencies dependencies(final Source source, final LdapServerOptions options) {
    ...
    public SourceWorker compile(final Prepared<LdapServerOptions> prepared, final SourceServices services) {
        ...
        if (!scheme().id().equals(source.getType()) || !supports(source.getProtocol())
            ...
            throw new ValidateException("LDAP server driver requires a matching Source configuration");
        }
        final LdapServerOptions options = prepared.options();
        if (options.maximumMessageBytes() > services.policies().require(Protocol.LDAP).maximumMessageBytes()) {
            throw new ValidateException("LDAP server message limit exceeds its shared security rule");
        }
    ...
    private static final class CompiledServer implements SourceWorker {
        ...
        /**
         * Exact ten-operation LDAP server manifest.
         */
         ...
         */
        private CompiledServer(final Capability.Manifest manifest, final BindService bind, final UnbindService unbind,
            ...
            this.manifest = Assert.notNull(manifest, "LDAP server manifest must not be null");
        ...
        private static <S> CompletionStage<Outcome<S>> narrow(
            ...
            return stage.thenApply(outcome -> switch (outcome) {
                ...
                default -> Outcome.failed(
                        new Outcome.Failure(ErrorCode._500, "LDAP server returned an unsupported outcome",
        ...
        private static <S> CompletionStage<Outcome<S>> missing() {
            return completed(
                    Outcome.rejected(
                            new Outcome.Failure(ErrorCode._404, "LDAP server capability is not available",
                                    new JsonValue.ObjectValue(Map.of()))));
        }
            ...
            return completed(
                    Outcome.rejected(
                            new Outcome.Failure(ErrorCode._400,
                                    "LDAP server capability type does not match the request",
                                    new JsonValue.ObjectValue(Map.of()))));
        }
        ...
        /**
         * Returns the exact LDAP server manifest.
         *
        ...
        public <Q, S> CompletionStage<Outcome<S>> invoke(
            ...
            Assert.notNull(capability, "LDAP server capability must not be null");
            Assert.notNull(context, "LDAP server context must not be null");
            Assert.notNull(timeout, "LDAP server timeout must not be null");
