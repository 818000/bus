...
/**
 * Models the fields of SAML Metadata {@code SPSSODescriptorType} consumed and emitted by the metadata codec.
 *
 ...
 * @param assertionConsumerServices  one or more indexed assertion consumer endpoints
 * @param attributeConsumingServices ordered complete AttributeConsumingService element bytes
 * @param authnRequestsSigned        optional lexical-presence signing declaration
 * @param wantAssertionsSigned       optional lexical-presence assertion-signing requirement
 * @author Kimi Liu
 */
public record SpSsoDescriptor(Optional<String> id, Optional<Instant> validUntil, Optional<String> cacheDuration,
        ...
        List<AssertionConsumerServiceEndpoint> assertionConsumerServices, List<byte[]> attributeConsumingServices,
        Optional<Boolean> authnRequestsSigned, Optional<Boolean> wantAssertionsSigned) {
     ...
     * @throws ValidateException        if the descriptor cannot represent an executable SAML 2.0 SP role
     */
    public SpSsoDescriptor {
        id = text(id, "SAML SPSSODescriptor ID");
        validUntil = optional(validUntil, "SAML SPSSODescriptor validUntil container must not be null");
        cacheDuration = text(cacheDuration, "SAML SPSSODescriptor cacheDuration");
        protocolSupportEnumeration = strings(
                protocolSupportEnumeration,
                "SAML SPSSODescriptor protocolSupportEnumeration",
                true);
        if (!protocolSupportEnumeration.contains(IdpSsoDescriptor.PROTOCOL)) {
            throw new ValidateException("SAML SPSSODescriptor must support the SAML 2.0 protocol namespace");
        }
        errorUrl = text(errorUrl, "SAML SPSSODescriptor errorURL");
        signature = bytes(signature, "SAML SPSSODescriptor Signature");
        extensions = elements(extensions, "SAML SPSSODescriptor extension");
        keys = objects(keys, "SAML SPSSODescriptor KeyDescriptor", false);
        organization = bytes(organization, "SAML SPSSODescriptor Organization");
        contacts = elements(contacts, "SAML SPSSODescriptor ContactPerson");
        singleLogoutServices = objects(singleLogoutServices, "SAML SPSSODescriptor SingleLogoutService", false);
        nameIdFormats = strings(nameIdFormats, "SAML SPSSODescriptor NameIDFormat", false);
        assertionConsumerServices = objects(
                assertionConsumerServices,
                "SAML SPSSODescriptor AssertionConsumerService",
                true);
        final long defaults = assertionConsumerServices.stream()
                .filter(endpoint -> Boolean.TRUE.equals(endpoint.defaultEndpoint().getOrNull())).count();
        if (defaults > 1)
            throw new ValidateException("SAML SPSSODescriptor cannot declare multiple default ACS endpoints");
        attributeConsumingServices = elements(
                attributeConsumingServices,
                "SAML SPSSODescriptor AttributeConsumingService");
        Assert.notNull(authnRequestsSigned, "SAML SPSSODescriptor AuthnRequestsSigned container must not be null");
        Assert.notNull(wantAssertionsSigned, "SAML SPSSODescriptor WantAssertionsSigned container must not be null");
        authnRequestsSigned = Optional.ofNullable(authnRequestsSigned.getOrNull());
        wantAssertionsSigned = Optional.ofNullable(wantAssertionsSigned.getOrNull());
    }
    ...
    /**
     * Normalizes one generic optional value.
     *
    ...
    public List<byte[]> extensions() {
        return elements(extensions, "SAML SPSSODescriptor extension");
    }
    ...
    public List<byte[]> contacts() {
        return elements(contacts, "SAML SPSSODescriptor ContactPerson");
    }

    /**
     * Returns defensive copies of AttributeConsumingService XML elements.
     *
     * @return immutable ordered service list whose byte arrays are caller-owned
     */
    @Override
    public List<byte[]> attributeConsumingServices() {
        return elements(attributeConsumingServices, "SAML SPSSODescriptor AttributeConsumingService");
    }
