...
/**
 * Implements the implementation-neutral Feishu realm surface over the official Contact v3 API.
 * <p>
 ...
 * @author Kimi Liu
 */
public class FeishuRealmAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Feishu Realm Variant.
     */
    private final VendorManifest.Variant variant;

    /**
     * Validated Feishu Realm deployment options containing only a credential reference.
     */
     ...
     * @param options  validated Feishu Realm options
    ...
    public FeishuRealmAdapter(final String spaceId, final String sourceId, final FeishuManifest manifest,
        ...
        Assert.notBlank(spaceId, "Feishu Realm space id must not be blank");
        Assert.notBlank(sourceId, "Feishu Realm Source id must not be blank");
        final FeishuManifest selectedManifest = Assert.notNull(manifest, "Feishu Realm manifest must not be null");
        this.variant = Assert.notNull(variant, "Feishu Realm Variant must not be null");
        this.options = Assert.notNull(options, "Feishu Realm options must not be null");
        this.services = Assert.notNull(services, "Feishu Realm services must not be null");
        ...
        if (this.targets.token().isEmpty()
            ...
            throw new ValidateException("Feishu Realm manifest has an invalid management target set");
        }
        if (!this.targets.management().get(Builder.REALM_USERS)
                .equals(this.targets.management().get(Builder.REALM_ORGANIZATION_USERS))) {
            throw new ValidateException("Feishu Realm user-list targets must resolve identically");
        }
    ...
    private static void requireKinds(final Set<Realm.Kind> kinds) {
        if (!SUPPORTED_KINDS.containsAll(kinds)) {
            throw new ValidateException("Feishu Realm snapshot contains an unsupported kind");
        }
    }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected(ErrorCode._400, "Feishu Realm capability is not declared by the selected manifest"));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> mismatch() {
        return completed(
                rejected(ErrorCode._400, "Feishu Realm request does not match the selected capability contract"));
    }
    ...
    /**
     * Routes one exact Feishu Realm capability and request type.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Feishu Realm capability must not be null");
        Assert.notNull(context, "Feishu Realm context must not be null");
        Assert.notNull(timeout, "Feishu Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return missing();
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(FeishuManifest.realmDescription())));
        }
    ...
    private CompletionStage<Outcome<Realm.Page>> snapshot(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "Feishu Realm snapshot request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Feishu Realm snapshot has no remaining timeout"));
        }
    ...
    private Outcome<Realm.Page> snapshot(
        ...
        try {
            ...
            return failed(ErrorCode._500, "Feishu Realm snapshot processing failed locally");
        }
    }
    ...
    private CompletionStage<Outcome<Realm.Retrieved>> retrieve(
        ...
        if (!SUPPORTED_KINDS.contains(request.key().kind())) {
            return completed(rejected(ErrorCode._400, "Feishu Realm retrieve kind is unsupported"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Feishu Realm retrieve has no remaining timeout"));
        }
        final Instant observedAt = FabricX.clock().now();
    ...
    private Outcome<Realm.Retrieved> retrieve(
        ...
        try {
            return switch (key.kind()) {
                ...
                default -> rejected(ErrorCode._400, "Feishu Realm retrieve kind is unsupported");
            };
        } catch (ValidateException ignored) {
            return failed(ErrorCode._502, "Feishu Contact v3 returned an invalid retrieval projection");
        } catch (RuntimeException ignored) {
            return failed(ErrorCode._500, "Feishu Realm retrieval processing failed locally");
        }
    }
    ...
    private <T> CompletionStage<Outcome<T>> execute(
        ...
        return CompletableFuture.<Outcome<T>>supplyAsync(() -> {
            if (timeout.expired()) {
                return failed(ErrorCode._408, "Feishu Realm operation has no remaining timeout");
            }
            try {
                return Assert.notNull(operation.apply(access), "Feishu Realm operation returned no outcome");
            } catch (TimeoutException ignored) {
                return failed(ErrorCode._408, "Feishu Realm operation timed out");
            } catch (RuntimeException ignored) {
    ...
    private Outcome<WirePage<User>> userPage(
            ...
            final Timeout timeout) {
        final Endpoint endpoint = target(Builder.REALM_USERS);
        final Map<String, Object> query = new LinkedHashMap<>();
    ...
    private Outcome<WirePage<Group>> groupPage(
            ...
            final Timeout timeout,
            final String operation) {
        final Endpoint endpoint = target(Builder.REALM_GROUPS);
        final Map<String, Object> query = new LinkedHashMap<>();
        query.put("page_size", Math.min(limit, Normal._50));
        if (next.isPresent()) {
            query.put("page_token", next.getOrNull());
    ...
    private Outcome<WirePage<Member>> memberPage(
            ...
            final Optional<String> next,
            final Timeout timeout) {
        final Endpoint endpoint = target(Builder.REALM_GROUP_MEMBERS);
    ...
    private Endpoint target(final String name) {
        ...
        if (endpoint == null) {
            throw new ValidateException("Feishu Realm manifest omits a required management target");
        }
        return endpoint;
    }
