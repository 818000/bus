 ...
 * <p>
 * This immutable record contains identifiers only. Private and public key material remains in the external key resolver
 * and is acquired by the adapter for one gateway operation. The optional callback container follows the common Vendor
 * settings shape; the Vendor Source driver requires it to be present for this browser variant.
 * </p>
 *
 ...
 * @author Kimi Liu
 */
public record AlipaySourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, String verificationKeyId)
     ...
     * @throws ValidateException        if routing or credential type differs from the frozen definition
     */
    public AlipaySourceSettings {
        if (!AlipayDefinition.ID.equals(vendor) || !AlipayDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Alipay settings routing keys must select alipay/default");
        }
