 ...
 * </p>
 * <p>
 * This exported package provides Source configuration and Realm management metadata, not an executable client. Runtime
 * use must enter a Provider obtained from Roster and delegate to the non-exported adapter. Source completion accepts
 * only the stable profile {@code id} as Identity subject; token {@code uid} remains a token extension and profile name,
 * avatar, URL, location, gender, and email remain attributes. Client secrets, state, codes, access or refresh tokens,
 * complete profile bodies, and upstream error descriptions must not enter diagnostics, Context, logs, or failures.
 * </p>
 *
 * @author Kimi Liu
 */
