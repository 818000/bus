...
import org.miaixz.bus.auth.source.protocol.oidc.client.UserInfoClient;
 ...
 * <p>
 * Roster-visible operations use only standard OpenID Connect models. The incomplete LinkedIn token and Discovery
...
public class LinkedInSourceAdapter implements VendorAdapter {
    ...
    /**
     * Source identifier copied into every verified external identity.
     */
    private final String sourceId;

    /**
     * Selected immutable LinkedIn variant manifest.
     */
    private final VendorManifest.Variant variant;

    /**
     * Validated externally loaded LinkedIn configuration options.
     */
    private final LinkedInOptions options;
    ...
    public LinkedInSourceAdapter(final String spaceId, final String sourceId, final LinkedInManifest manifest,
        ...
        this.options = Assert.notNull(options, "LinkedIn options must not be null");
        this.services = Assert.notNull(services, "LinkedIn execution services must not be null");
        if (!LinkedInManifest.ID.equals(manifest.vendor())
                || !manifest.variant(LinkedInManifest.DEFAULT).equals(variant)
                || !LinkedInManifest.DEFAULT.equals(variant.variant()) || variant.protocol() != Protocol.OIDC
                || !LinkedInManifest.ID.equals(options.vendor())
                || !LinkedInManifest.DEFAULT.equals(options.variant())) {
            throw new ValidateException("LinkedIn adapter requires the linkedin/default OIDC manifest");
        }
        if (!services.policies().require(Protocol.OIDC).algorithms().contains(JwaAlgorithm.RS256.name())) {
            throw new ValidateException("LinkedIn RS256 is not permitted by the OIDC security rules");
        }
        ...
        final UserInfoClient userInfoClient = new UserInfoClient(openIdSettings, services, new UserInfoCodec());
        this.standardAdapter = new StandardAdapter(variant, options, Optional.of(redirectManager),
                List.of(
                        ...
                        new StandardAdapter.Binding<>(OpenIdClientScheme.USERINFO, userInfoClient::userInfo)));
        ...
        this.idTokenCodec = new IdTokenCodec(new JwtVerifier(jwsService, dormantJweService));
    }
    ...
    /**
     * Creates a implementation-neutral empty JSON object.
     *
     ...
     * register that parameter, so this Vendor manifest validates but does not transmit the model's nonce.
     * </p>
     *
     * @param request standard request to inspect
     * @return whether every transmitted member matches the frozen LinkedIn manifest
     */
    ...
    private CompletionStage<Outcome<Identity>> identity(
        ...
        if (resolution == null) {
            return completed(failed(ErrorCode._502, "LinkedIn client-secret loader returned no stage"));
        }
     ...
     */
    private CompletionStage<Outcome<Identity>> verifyWithRotation(
            final OpenIdTokenResponse token,
     ...
     */
    private CompletionStage<Outcome<Identity>> verifyIdToken(
            final OpenIdTokenResponse token,
            final String compact,
            final Jwk selected,
            final Context context,
        ...
        try {
            ...
            if (!claims.audience().contains(options.clientId())) {
            ...
            if (authorizedParty != null && !options.clientId().equals(authorizedParty)) {
     ...
     */
    private CompletionStage<Outcome<Identity>> userInfo(
            final OpenIdTokenResponse token,
            final IdTokenClaims claims,
            final Context context,
            final Timeout timeout) {
     ...
     */
    private Outcome<Identity> identity(
    ...
    private CompletionStage<Outcome<Void>> discovery(final Timeout timeout) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                try (Response response = FabricX.http(Protocol.OIDC, timeout, services.policies()).url(endpoint)
        ...
        if (!resolvedTargets.authorization().getOrNull().url().toString()
                ...
                || !strings(object, "claims_supported").containsAll(
                        List.of(
                                ...
                                "email_verified",
                                "locale"))) {
            throw new ValidateException("LinkedIn Discovery metadata differs from the frozen manifest");
        }
    }
     ...
     * @return implementation-neutral immutable JSON object
     ...
     *
     * @return ordered explicit scopes or frozen manifest defaults
     */
