...
import org.miaixz.bus.auth.worker.KeyRecord;
...
public final class KeyParser {
    ...
    public KeyMaterial parse(final KeyLoader.Request request, final KeyRecord record) {
        final KeyLoader.Request expected = Assert.notNull(request, "Key request must not be null");
        final KeyRecord loaded = Assert.notNull(record, "Loaded key record must not be null");
