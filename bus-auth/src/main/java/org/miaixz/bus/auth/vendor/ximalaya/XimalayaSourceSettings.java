 ...
 * <p>
 * Fixed endpoints and signature behavior remain definition-owned. The record retains the six common Vendor components
 ...
 * @author Kimi Liu
 */
public record XimalayaSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, String deviceId,
        String clientOsType, String packageId) implements VendorSettings {
     ...
     *                                  definition
     */
    public XimalayaSourceSettings {
        if (!XimalayaDefinition.ID.equals(vendor) || !XimalayaDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Ximalaya settings must select ximalaya/default");
        }
    ...
    private static void callback(final Optional<String> redirectUri) {
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Ximalaya settings require a registered redirect URI");
        }
    ...
    public String toString() {
        return "XimalayaSourceSettings[vendor=" + vendor + ", variant=" + variant
