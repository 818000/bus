...
import org.miaixz.bus.auth.source.protocol.ldap.SaslCredentials;
import org.miaixz.bus.core.lang.Assert;
import org.miaixz.bus.core.lang.Optional;
import org.miaixz.bus.core.lang.exception.ValidateException;

/**
 * Holds immutable deployment limits and advertised authentication mechanisms for one LDAPv3 directory server.
 * <p>
 * The options declare which Bind choices and the standard StartTLS extended operation are available. They contain
 * neither directory data nor connection authentication state; the external {@link DirectoryStore} owns both at runtime.
 * The LDAP server driver additionally requires the message limit not to exceed the shared LDAP security rule.
 * </p>
 *
 ...
 * @author Kimi Liu
 */
public record LdapServerOptions(boolean anonymousBindSupported, boolean simpleBindSupported, Set<String> saslMechanisms,
        boolean startTlsSupported, int maximumSearchEntries, int maximumSearchTimeSeconds, long maximumMessageBytes,
        int maximumBerDepth) implements Options<LdapServerOptions> {

    /**
     * Freezes mechanism names and verifies that the directory server has a usable Bind and positive resource limits.
     *
    ...
    public LdapServerOptions {
        Assert.notNull(saslMechanisms, "LDAP server SASL mechanisms must not be null");
        final LinkedHashSet<String> mechanisms = new LinkedHashSet<>(saslMechanisms.size());
        for (String mechanism : saslMechanisms) {
            final String current = Assert.notBlank(mechanism, "LDAP server SASL mechanism must not be blank");
        ...
        if (!anonymousBindSupported && !simpleBindSupported && saslMechanisms.isEmpty()) {
            throw new ValidateException("LDAP server must enable at least one Bind authentication choice");
        }
        if (maximumSearchEntries <= 0 || maximumSearchTimeSeconds <= 0 || maximumMessageBytes <= 0
                || maximumBerDepth <= 0) {
            throw new ValidateException("LDAP server search, message, and BER depth limits must be positive");
        }
    }
    ...
    /**
     * Returns this already immutable LDAP server configuration snapshot.
     *
