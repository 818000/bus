 ...
 * <p>
 * Official cloud hosts and endpoint paths remain definition-owned. This record retains only the Application ID, an
 ...
 * @param scopes      ordered delegated scopes, or empty to use definition defaults
 * @param tenant      Microsoft tenant GUID, verified domain, or registered audience alias
 * @author Kimi Liu
 */
public record MicrosoftSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     *                                  invalid
     */
    public MicrosoftSourceSettings {
        if (!MicrosoftDefinition.ID.equals(vendor)
                || !MicrosoftDefinition.GLOBAL.equals(variant) && !MicrosoftDefinition.CHINA.equals(variant)) {
            throw new ValidateException("Microsoft settings must select microsoft/global or microsoft/china");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Microsoft settings require a registered redirect URI");
        }
        ...
        if (MicrosoftDefinition.CHINA.equals(variant) && "consumers".equals(tenant)) {
    ...
    /**
     * Returns the tenant value consumed by definition-owned endpoint templates.
     *
    ...
    public String toString() {
        return "MicrosoftSourceSettings[vendor=" + vendor + ", variant=" + variant
