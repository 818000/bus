 ...
 * @param scopes          ordered requested scopes, or empty to use the selected definition default
 ...
 * @author Kimi Liu
 */
public record DingTalkSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, Optional<String> orgType,
        Optional<String> corpId, boolean exclusiveLogin, Optional<String> exclusiveCorpId) implements VendorSettings {
     ...
     *                                  variant
     */
    public DingTalkSourceSettings {
        if (!DingTalkDefinition.ID.equals(vendor)
                || !DingTalkDefinition.OAUTH2.equals(variant) && !DingTalkDefinition.ACCOUNT.equals(variant)) {
            throw new ValidateException("DingTalk settings must select dingtalk/oauth2 or dingtalk/account");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("DingTalk browser settings require a registered redirect URI");
        }
        ...
        if (DingTalkDefinition.OAUTH2.equals(variant)) {
    ...
    private static void account(
        ...
        if (orgType.isPresent() || corpId.isPresent() || exclusiveLogin || exclusiveCorpId.isPresent()) {
            throw new ValidateException("DingTalk account settings prohibit delegated organization selectors");
        }
    }
    ...
    public String toString() {
        return "DingTalkSourceSettings[vendor=" + vendor + ", variant=" + variant
