 ...
 * <p>
 * Fixed endpoints and mandatory S256 behavior remain manifest-owned. The application-secret reference preserves the
 ...
 * @param scopes      ordered approved VK scopes, or empty to select manifest defaults
 * @author Kimi Liu
 */
public record VkOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
    ...
    public VkOptions {
        if (!VkManifest.ID.equals(vendor) || !VkManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("VK options must select vk/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("VK options require a registered redirect URI");
        }
