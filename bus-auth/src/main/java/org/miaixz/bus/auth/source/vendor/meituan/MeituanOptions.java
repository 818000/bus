...
/**
 * Carries externally managed Meituan Waimai application configuration values.
 * <p>
 * Official endpoints, empty-scope behavior, and platform form mappings remain manifest- and adapter-owned. This record
 * retains only the canonical public {@code app_id}, an external {@code secret} reference, and the exact registered
 * callback value.
 * </p>
 *
 ...
 * @author Kimi Liu
 */
public record MeituanOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
     ...
     * @throws ValidateException        if routing, app id, credential, callback, or scopes differ from the manifest
     */
    public MeituanOptions {
        if (!MeituanManifest.ID.equals(vendor) || !MeituanManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("Meituan options must select meituan/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Meituan options require a registered redirect URI");
        }
        redirect(redirectUri.getOrNull());
        Assert.notNull(scopes, "Meituan scopes must not be null");
        if (!scopes.isEmpty()) {
            throw new ValidateException("Meituan options require an empty scope list");
        }
        scopes = List.of();
    }
