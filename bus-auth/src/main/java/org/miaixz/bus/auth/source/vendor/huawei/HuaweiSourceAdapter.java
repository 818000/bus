...
import org.miaixz.bus.auth.source.protocol.oidc.codec.AuthenticationRequestEncoder;
 ...
 * <p>
 * Roster operations retain standard Authentication Request, Discovery, JWK Set, and Revocation types. Huawei's numeric
 * token errors, {@code supportAlg}, form-carried profile token, and {@code NSP_STATUS} semantics remain private to the
 * Source completion chain and never appear as invented public protocol models.
 * </p>
 *
 * @author Kimi Liu
 */
public class HuaweiSourceAdapter implements VendorAdapter {
    ...
    /**
     * Source identifier written into verified external identities.
     */
    ...
    /**
     * Validated externally loaded Huawei configuration values.
     */
     ...
     *
     * @param spaceId  Source space used for browser correlation isolation
    ...
    public HuaweiSourceAdapter(final String spaceId, final String sourceId, final HuaweiManifest manifest,
        ...
        final var rule = services.policies().require(Protocol.OIDC);
        if (!rule.algorithms().contains(JwaAlgorithm.RS256.name()) || rule.minimumEntropyBits() > 400) {
        ...
        final OpenIdClient openIdClient = new OpenIdClient(oauthClient, new AuthenticationRequestEncoder(),
                new DiscoveryClient(openIdSettings, services, new OpenIdProviderMetadataCodec()), Optional.empty(),
                Optional.empty());
     ...
     */
    private Outcome<Identity> sendProfile(final PrivateToken token, final IdTokenClaims claims, final Timeout timeout) {
        byte[] body = null;
        try {
            if (timeout.expired()) {
            ...
            try (Response response = FabricX.http(Protocol.OIDC, timeout, services.policies()).url(endpoint)
                ...
                final Evidence evidence = new Evidence(Evidence.Type.FEDERATED, Evidence.Strength.SINGLE_FACTOR,
                        new Evidence.Claim(JwtClaims.SUBJECT, new JsonValue.StringValue(unionId), ISSUER,
                                timeout.clock().now()));
                return Outcome.succeeded(
                        new Identity(sourceId, unionId, new JsonValue.ObjectValue(attributes), List.of(evidence)));
            }
        } catch (RuntimeException cause) {
    ...
    private Outcome<OpenIdProviderMetadata> metadata(final OpenIdProviderMetadata metadata) {
        try {
            ...
            return rejected("Huawei Discovery metadata does not match the configured Source");
        }
    }
