...
import org.miaixz.bus.auth.protocol.ldap.codec.BerFrameReader;
import org.miaixz.bus.auth.protocol.ldap.codec.LdapMessageCodec;
...
public final class LdapServerSession {
     ...
     */
    private final Context invocation;
    /**
     ...
     */
    public LdapServerSession(final Context invocation, final SocketSession stream,
            final TransportPolicy transportPolicy, final TlsPolicy tlsPolicy, final LdapRequestHandler requests,
        ...
        if (transportPolicy.requireStartTls() && tlsPolicy == null) {
            throw new ValidateException("LDAP StartTLS requires a Fabric TLS policy");
        }
    ...
    /**
     * Executes one single-use Fabric call without blocking the caller thread.
     */
    private static <T> CompletionStage<T> invoke(final java.util.concurrent.Callable<T> operation) {
    ...
    private CompletionStage<Void> read() {
        return invoke(() -> stream.receive().execute()).thenCompose(read -> {
            final byte[] bytes = read.payload().bytes(readBytes);
            final List<byte[]> encoded = bytes.length == Normal._0 ? List.of() : frames.append(bytes);
     ...
     */
    private CompletionStage<Void> continueReading(final Message read) {
    ...
    private CompletionStage<Void> authorized(
            ...
            final java.util.function.BiFunction<Context, LdapMessage, CompletionStage<List<LdapMessage>>> operation) {
        if (!bound) {
            return write(
                    LdapMessage.of(
                            message.messageId(),
    ...
    private CompletionStage<Void> authorizedSingle(
            final LdapMessage message,
            final java.util.function.BiFunction<Context, LdapMessage, CompletionStage<LdapMessage>> operation) {
        if (!bound) {
            return write(
                    LdapMessage.of(
                            message.messageId(),
    ...
    private CompletionStage<Void> startTls(final LdapMessage message, final boolean hasPlaintextAfter) {
        ...
        return write(response).thenCompose(ignored -> invoke(() -> stream.upgradeTls(tlsPolicy).execute()))
    ...
    private CompletionStage<Void> write(final LdapMessage message) {
        return invoke(() -> stream.send(Payload.of(messages.encode(message))).execute());
    }
