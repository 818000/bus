...
import org.miaixz.bus.auth.protocol.oauth2.OAuth2.AuthorizationRequest;
import org.miaixz.bus.auth.protocol.oauth2.OAuth2.CodeChallengeMethod;
import org.miaixz.bus.auth.protocol.oauth2.OAuth2.GrantType;
...
public final class OAuth2Validator {
     ...
     * @throws ProtocolException if the resolver result is absent or identifies another client
     */
    public static RegisteredClient client(final Optional<RegisteredClient> resolved, final String clientId) {
     ...
     * @param secrets         non-null tenant-aware secret resolver
     * @return stage containing the authenticated client
     * @throws IllegalArgumentException if a required collaborator is {@code null}
     */
    public static CompletionStage<RegisteredClient> authenticate(
        ...
        final RegisteredClient registered = Assert.notNull(client, "RegisteredClient must be not null!");
        final SecretResolver resolver = Assert.notNull(secrets, "Secret resolver must be not null!");
        final char[] presented = presentedSecret == null ? new char[0] : presentedSecret.clone();
        final Object configured = registered.tokenEndpointAuthMethod();
        ...
        final CompletionStage<char[]> resolved = Assert.notNull(
                resolver.resolve(context, "oauth2-client", registered.id()),
     ...
     * @throws ProtocolException if the URI is unsafe or not registered for the client
     */
    public static URI redirect(final RegisteredClient client, final URI redirectUri) {
        final RegisteredClient registered = Assert.notNull(client, "RegisteredClient must be not null!");
        try {
     ...
     * @throws ProtocolException if a scope is not permitted, exceeds a bound, or violates the wire grammar
     */
     ...
     * @throws ProtocolException if the grant is absent or is not enabled
     */
     ...
     * @throws ProtocolException if any client, redirect, scope, PKCE, or request-source field is invalid
     */
     ...
     * @throws ProtocolException if the challenge or verifier is invalid or does not match
     */
     ...
     * @throws ProtocolException if either secret is absent or their contents differ
     */
     ...
     * @throws ProtocolException if both sources are present or either source is malformed
     */
     ...
     * @throws ProtocolException if the method or challenge grammar is invalid
     */
     ...
     *
     * @param verifier presented verifier
     * @throws ProtocolException if the verifier violates RFC 7636
     */
     ...
     * @param limits authentication input limits
     * @throws ProtocolException if the token exceeds a bound or violates RFC 6749
     */
     ...
     * @throws ProtocolException always, carrying {@code error}
     */
