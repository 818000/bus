 ...
 * <p>
 * Official endpoints remain definition-owned. This immutable value stores only a reference to the client secret and
 * never resolves, copies, or exposes the secret material.
 * </p>
 *
 ...
 * @author Kimi Liu
 */
public record KujialeSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     * @throws ValidateException        if routing, credential type, callback, or scopes differ from the frozen
     *                                  definition
     */
    public KujialeSourceSettings {
        if (!KujialeDefinition.ID.equals(vendor) || !KujialeDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Kujiale settings must select kujiale/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Kujiale settings require a registered redirect URI");
        }
        ...
        if (copy.isEmpty()) {
            throw new ValidateException("Kujiale settings require at least one scope");
        }
    ...
    public String toString() {
        return "KujialeSourceSettings[vendor=" + vendor + ", variant=" + variant
