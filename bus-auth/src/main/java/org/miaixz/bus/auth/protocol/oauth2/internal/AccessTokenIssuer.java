...
import org.miaixz.bus.auth.resolver.ResourceResolver;
...
public final class AccessTokenIssuer {
    ...
    /**
     * Frozen Provider settings governing enabled grants and token lifetimes.
     */
     ...
     */
    public AccessTokenIssuer(final String providerId, final OAuth2ProviderSettings settings,
            final ExecutionServices services, final ScopeValidator scopeValidator, final PkceValidator pkceValidator,
            final Augmenter augmenter) {
    ...
    public CompletionStage<Outcome<TokenEndpointResponse>> token(
        ...
        if (!settings.grantTypesSupported().contains(grantType)) {
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> authorizationCode(
            ...
            final Context context,
            final Timeout.Budget timeout) {
        final CompletionStage<ExpiringValue<AuthorizationCodeStore.Entry>> consumption;
        try {
            consumption = services.authorizationCodeStore().take(tokenKey(grant.code()));
        } catch (RuntimeException exception) {
            return completed(storeFailure("OAuth 2.x authorization code consumption failed"));
        }
        return consumption.handle((stored, thrown) -> new StoreResult<>(stored, thrown)).thenCompose(result -> {
            if (result.failure() != null) {
                return completed(storeFailure("OAuth 2.x authorization code consumption failed"));
            }
            final ExpiringValue<AuthorizationCodeStore.Entry> stored = result.value();
            ...
            final AuthorizationCodeStore.Entry entry = stored.value();
            ...
            return issue(
                    new Grant(client.id(), entry.subjectId(), entry.scope(), entry.resource(),
                            ...
                            settings.grantTypesSupported().contains(GrantType.REFRESH_TOKEN), Optional.empty(),
    ...
    private boolean validAuthorizationCode(
            ...
            final AuthorizationCodeStore.Entry entry) {
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> clientCredentials(
        ...
        if (!validScope(scope, client.scopes(), settings.scopesSupported())) {
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> tokenExchange(
        ...
        return activeToken(grant.subjectToken(), client.id(), timeout)
                .thenCompose(subjectOutcome -> switch (subjectOutcome) {
                    case Outcome.Succeeded<AccessTokenStore.Entry> success -> actor(
                            grant,
                            client.id(),
                    ...
                    case Outcome.Rejected<AccessTokenStore.Entry> rejected -> completed(
                            Outcome.rejected(rejected.failure()));
                    case Outcome.Failed<AccessTokenStore.Entry> failed -> completed(Outcome.failed(failed.failure()));
                });
    }
    ...
    private CompletionStage<Outcome<Optional<String>>> actor(
        ...
        return activeToken(actorToken, clientId, timeout).thenApply(outcome -> switch (outcome) {
            case Outcome.Succeeded<AccessTokenStore.Entry> success -> success.value().actorSubjectId().isPresent()
                    ? Outcome.rejected(
                            failure(
                                    ErrorCode._400,
                                    OAuth2ErrorCode.INVALID_REQUEST,
            ...
            case Outcome.Rejected<AccessTokenStore.Entry> rejected -> Outcome.rejected(rejected.failure());
            case Outcome.Failed<AccessTokenStore.Entry> failed -> Outcome.failed(failed.failure());
        });
    }
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> exchangeTarget(
            final TokenExchangeGrant grant,
            final ClientResolver.Client client,
            final AccessTokenStore.Entry subject,
        ...
        final CompletionStage<Outcome<ResourceResolver.Resource>> resolution;
        try {
            resolution = services.resourceResolver().resolve(
                    new ResourceResolver.Query(providerId, grant.audience(), grant.resource()),
                    context,
                    timeout);
        } catch (RuntimeException exception) {
        ...
        return resolution
                .handle(
                        (outcome, thrown) -> thrown == null && outcome != null ? outcome
                                : Outcome.<ResourceResolver.Resource>failed(
                                        failure(
                                                ErrorCode._500,
                                                OAuth2ErrorCode.SERVER_ERROR,
                                                "OAuth 2.x exchange target resolution failed")))
                .thenCompose(outcome -> switch (outcome) {
                    case Outcome.Succeeded<ResourceResolver.Resource> success -> {
                        final ResourceResolver.Resource resource = success.value();
                    ...
                    case Outcome.Rejected<ResourceResolver.Resource> rejected -> completed(
                            Outcome.rejected(
                                    failure(
                                            rejected.failure().error(),
                                            OAuth2ErrorCode.INVALID_TARGET,
                                            "OAuth 2.x exchange target was rejected")));
                    case Outcome.Failed<ResourceResolver.Resource> failed -> completed(
                            Outcome.failed(
                                    failure(
                                            failed.failure().error(),
                                            OAuth2ErrorCode.SERVER_ERROR,
    ...
    private CompletionStage<Outcome<AccessTokenStore.Entry>> activeToken(
        ...
        return lookup.handle((stored, thrown) -> {
            ...
            if (stored == null || !stored.expiresAt().isAfter(timeout.clock().now())
                ...
                return Outcome.<AccessTokenStore.Entry>rejected(
                        failure(
                                ErrorCode._400,
                                OAuth2ErrorCode.INVALID_REQUEST,
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> deviceCode(
        ...
        final CompletionStage<ExpiringValue<DeviceCodeStore.Entry>> lookup;
        try {
            lookup = services.deviceCodeStore().get(key);
        } catch (RuntimeException exception) {
        ...
        return lookup.handle((stored, thrown) -> new StoreResult<>(stored, thrown)).thenCompose(result -> {
            ...
            final ExpiringValue<DeviceCodeStore.Entry> stored = result.value();
            ...
            final DeviceCodeStore.Entry entry = stored.value();
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> pollPending(
            final DeviceCodeGrant grant,
            final String key,
            final ExpiringValue<DeviceCodeStore.Entry> stored,
            final DeviceCodeStore.Entry entry,
            final ClientResolver.Client client,
            final Context context,
            final Timeout.Budget timeout,
            final int attempt,
        ...
        final DeviceCodeStore.Entry update = new DeviceCodeStore.Entry(entry.providerId(), entry.clientId(),
                entry.userCode(), entry.scope(), DeviceCodeStore.Status.PENDING, interval, Optional.of(now),
                Optional.empty());
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> consumeApproved(
            ...
            final int attempt,
            final Instant now) {
        final DeviceCodeStore.Entry consumed = new DeviceCodeStore.Entry(entry.providerId(), entry.clientId(),
                entry.userCode(), entry.scope(), DeviceCodeStore.Status.CONSUMED, entry.interval(), Optional.of(now),
                entry.subjectId());
        return replaceDevice(key, stored, consumed, timeout).thenCompose(replaced -> {
            ...
            return issue(
                    new Grant(client.id(), entry.subjectId().getOrNull(), entry.scope(), List.of(),
                            GrantType.DEVICE_CODE, settings.grantTypesSupported().contains(GrantType.REFRESH_TOKEN),
                            Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty()),
                    context,
                    timeout);
        });
    ...
    private CompletionStage<Boolean> replaceDevice(
            final String key,
            final ExpiringValue<DeviceCodeStore.Entry> expected,
            final DeviceCodeStore.Entry update,
            final Timeout.Budget timeout) {
        ...
        try {
            return services.deviceCodeStore()
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> createAccess(
        ...
        final Instant expiresAt = timeout.clock().now().plus(settings.accessTokenLifetime());
        final ExpiringValue<AccessTokenStore.Entry> value = new ExpiringValue<>(
                new AccessTokenStore.Entry(providerId, grant.clientId(), grant.subjectId(), grant.scope(),
                        grant.audience(), grant.actorSubjectId(), grant.confirmation(), grant.openIdBinding()),
                expiresAt);
        final CompletionStage<Boolean> creation;
        try {
            creation = services.accessTokenStore().create(accessKey, value, settings.accessTokenLifetime().toMillis());
        } catch (RuntimeException exception) {
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> createRefresh(
        ...
        final Instant expiresAt = timeout.clock().now().plus(settings.refreshTokenLifetime());
        final ExpiringValue<RefreshTokenStore.Entry> value = new ExpiringValue<>(new RefreshTokenStore.Entry(providerId,
                grant.clientId(), grant.subjectId(), familyId, 0L, grant.scope(), grant.audience(),
                grant.confirmation(), grant.openIdBinding(), RefreshTokenStore.Status.ACTIVE), expiresAt);
        final CompletionStage<Boolean> creation;
        try {
            creation = services.refreshTokenStore()
                    .create(tokenKey(refreshToken), value, settings.refreshTokenLifetime().toMillis());
        } catch (RuntimeException exception) {
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> cleanupIssued(
        ...
        try {
            cleanup = services.accessTokenStore().delete(accessKey);
        } catch (RuntimeException exception) {
        ...
        if (refreshKey != null) {
            cleanup = cleanup.handle((ignored, thrown) -> Boolean.TRUE).thenCompose(ignored -> {
                try {
                    return services.refreshTokenStore().delete(refreshKey);
                } catch (RuntimeException exception) {
                    return CompletableFuture.completedFuture(false);
                }
            });
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> cleanupAccess(
            ...
            final String description) {
        try {
            return services.accessTokenStore().delete(accessKey).handle((ignored, thrown) -> storeFailure(description));
        } catch (RuntimeException exception) {
    ...
    private TokenEndpointResponse response(final String accessToken, final String refreshToken, final Grant grant) {
        ...
        if (grant.grantType() == GrantType.TOKEN_EXCHANGE) {
            return new TokenExchangeResponse(accessToken, grant.issuedTokenType().getOrNull(), TokenType.BEARER,
                    Optional.of(settings.accessTokenLifetime().toSeconds()), scope, Optional.ofNullable(refreshToken),
                    new JsonValue.ObjectValue(Map.of()));
        }
        return new TokenResponse(accessToken, TokenType.BEARER, Optional.of(settings.accessTokenLifetime().toSeconds()),
    ...
    public record Grant(String clientId, String subjectId, List<String> scope, List<String> audience,
            ...
            Optional<AuthorizationCodeStore.OpenIdBinding> openIdBinding) {
    ...
    /**
     * Couples a Store value with a normalized completion failure.
     *
     * @param value   returned Store value
     * @param failure completion failure, or {@code null}
     * @param <T>     Store value type
     * @author Kimi Liu
     */
    private record StoreResult<T>(T value, Throwable failure) {
