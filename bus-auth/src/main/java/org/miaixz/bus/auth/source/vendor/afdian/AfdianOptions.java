...
/**
 * Carries the externally managed Afdian client configuration values.
 *
 ...
 * @author Kimi Liu
 */
public record AfdianOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
        Optional<String> redirectUri, List<String> scopes) implements VendorOptions<AfdianOptions> {

    /**
     * Validates and freezes one Afdian Source configuration.
     *
     * @throws IllegalArgumentException if a required component is null or blank
     * @throws ValidateException        if routing, credential type, or callback policy differs from the frozen manifest
     */
    public AfdianOptions {
        if (!AfdianManifest.ID.equals(vendor) || !AfdianManifest.DEFAULT.equals(variant)) {
            throw new ValidateException("Afdian options routing keys must select afdian/default");
        }
