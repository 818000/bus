 ...
 * @author Kimi Liu
 */
public record QqOptions(Vendor.Id vendor, Vendor.Variant variant, String clientId, Credential.Reference credential,
     ...
     */
    public QqOptions {
        if (!QqManifest.ID.equals(vendor)
                || !QqManifest.OPEN.equals(variant) && !QqManifest.MINI_PROGRAM.equals(variant)) {
            throw new ValidateException("QQ options must select qq/open or qq/mini-program");
        }
        ...
        if (QqManifest.OPEN.equals(variant)) {
            open(redirectUri, scopes);
        } else if (redirectUri.isPresent() || !scopes.isEmpty() || preferUnionId) {
            throw new ValidateException("QQ mini-program options prohibit callback, scopes, and UnionID preference");
        }
    }
     ...
     * @param scopes      immutable explicit scope values, possibly empty for manifest defaults
    ...
    private static void open(final Optional<String> redirectUri, final List<String> scopes) {
        if (redirectUri.isEmpty()) {
            throw new ValidateException("QQ open options require a registered redirect URI");
        }
