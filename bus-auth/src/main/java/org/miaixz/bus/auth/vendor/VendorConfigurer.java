 ...
 * not create a Source, mutate the Registry, load credentials, start authorization, or implement project CRUD.
 * </p>
 *
 * @author Kimi Liu
 */
public class VendorConfigurer {
     ...
     */
    public VendorConfigurer(final OptionsBindings bindings, final CredentialWriter writer) {
     ...
     */
    private static void callback(final VariantManifest.Variant variant, final Optional<String> callback) {
     ...
     */
    private static List<String> scopes(final VariantManifest.Variant variant, final List<String> requested) {
    ...
    public Scheme.Form form(final Vendor.Id vendor, final Vendor.Variant variant) {
        return bindings.resolve(vendor, variant).manifest().form();
    }
    ...
    public CompletionStage<Outcome<VendorOptions<?>>> configure(
        ...
        try {
            ...
            parameters = parameters(binding.manifest().form(), checked.parameters());
        } catch (RuntimeException cause) {
