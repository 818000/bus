 ...
 * <p>
 * The service accepts only an already decoded Authorization-header bearer token, resolves its Source-isolated
...
public class UserInfoService {
    ...
    /**
     * Source identifier used to isolate opaque access-token digests.
     */
    private final String sourceId;

    /**
     * Frozen OpenID Provider claim-release options.
     */
    ...
    private CompletionStage<Outcome<UserInfoResponse>> validateAndResolve(
        ...
        if (stored == null || stored.value() == null || !stored.expiresAt().isAfter(now)
                || !sourceId.equals(stored.value().sourceId())) {
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._401,
        ...
        try {
            authorization = services.authorizationCache()
                    .find(AuthorizationCache.key(sourceId, entry.authorizationId()));
        } catch (RuntimeException exception) {
            return completed(
                    Outcome.failed(
                            failure(
    ...
    private CompletionStage<Outcome<UserInfoResponse>> resolveAuthorized(
        ...
        if (authorization == null || !authorization.expiresAt().isAfter(timeout.clock().now())
                ...
                || !sourceId.equals(authorization.value().sourceId())
        ...
        try {
            resolution = Outcome.mapStage(
                    () -> services.attributeLoader().load(
                            new AttributeLoader.Request(services.entry(), new Subject.Key(entry.subjectId())),
                            context,
                            timeout),
                    loaded -> services.attributeParser()
                            .parse(services.entry(), new Subject.Key(entry.subjectId()), loaded));
        } catch (RuntimeException exception) {
            return completed(
                    Outcome.failed(
                            failure(
    ...
    /**
     * Produces a Source-isolated irreversible lookup key for an opaque bearer token.
     *
