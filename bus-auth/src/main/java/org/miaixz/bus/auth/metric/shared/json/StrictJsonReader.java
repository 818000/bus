 ...
 * <p>
 * <strong>Bus dependencies:</strong> {@link JsonProvider} performs the sole object mapping, {@link Limits} supplies the
 * closed JSON bounds, {@link Symbol} and {@link Normal} supply framework punctuation and numeric constants, and
 * {@link ErrorCode} supplies the framework JSON and request-size errors.
 *
 * @author Kimi Liu
 */
public final class StrictJsonReader {
    ...
    /**
     * Creates an immutable strict reader from product runtime dependencies.
     *
     * @param provider product-supplied JSON provider
     * @param limits   authentication input limits
     */
    public StrictJsonReader(final JsonProvider provider, final Limits limits) {
        this.provider = Assert.notNull(provider, () -> new ValidateException("JSON provider must not be null"));
        final Limits current = Assert.notNull(limits, () -> new ValidateException("Limits must not be null"));
        this.maximumBytes = current.maxJsonBytes();
        this.maximumDepth = current.maxJsonDepth();
    }
