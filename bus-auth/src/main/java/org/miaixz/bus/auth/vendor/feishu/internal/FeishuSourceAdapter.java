...
import org.miaixz.bus.auth.vendor.feishu.FeishuSourceSettings;
...
public final class FeishuSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Feishu definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Feishu settings.
     */
    private final FeishuSourceSettings settings;
     ...
     * @param vendorDefinition  selected Feishu definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded Feishu settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if routing, protocol, definition, callback, or authorization is inconsistent
     */
    public FeishuSourceAdapter(final String namespaceId, final String sourceId, final FeishuDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final FeishuSourceSettings settings,
            final ExecutionServices services) {
        final FeishuDefinition selectedProfile = Assert.notNull(vendorDefinition, "Feishu definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Feishu Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Feishu definition must not be null");
        this.settings = Assert.notNull(settings, "Feishu settings must not be null");
        this.services = Assert.notNull(services, "Feishu execution services must not be null");
        if (!FeishuDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(FeishuDefinition.DEFAULT).equals(variantDefinition)
                || !FeishuDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !FeishuDefinition.ID.equals(settings.vendor())
                || !FeishuDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("Feishu adapter requires the feishu/default OAuth 2.0 definition");
        }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected("Feishu capability is not declared by the selected definition"));
    }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected Feishu definition.
     *
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        try {
            ...
            final var location = endpoint.url().newBuilder().query(OAuth2.Parameters.CLIENT_ID, settings.clientId())
            ...
            if (!settings.scopes().isEmpty()) {
                location.query(OAuth2.Parameters.SCOPE, String.join(Symbol.SPACE, settings.scopes()));
            }
    ...
    private Outcome<Access> token(
        ...
        try {
            ...
            fields.put(OAuth2.Parameters.CLIENT_ID, new JsonValue.StringValue(settings.clientId()));
            ...
            fields.put(OAuth2.Parameters.REDIRECT_URI, new JsonValue.StringValue(settings.redirectUri().getOrNull()));
            fields.put(
    ...
    private Outcome<Access> token(final HttpResponse response) {
        try {
            ...
            final List<String> effectiveScopes = scopeText == null ? settings.scopes()
                    : Scope.parse(scopeText).values();
    ...
    private boolean valid(final AuthorizationRequest request) {
        ...
        final boolean scopeMatches = settings.scopes().isEmpty() ? scope == null || scope.values().isEmpty()
                : scope != null && settings.scopes().equals(scope.values());
        ...
        return ResponseType.CODE.equals(request.responseType()) && settings.clientId().equals(request.clientId())
                && settings.redirectUri().equals(request.redirectUri()) && scopeMatches && request.state().isPresent()
