 ...
 * <p>
 * Fixed endpoints and merchant RPC behavior remain definition-owned. The record retains only the public application
 * key, an external application-secret reference, the exact registered callback, and the sole supported scope.
 * </p>
 *
 ...
 * @param scopes      ordered requested scope, or empty to use the definition default
 * @author Kimi Liu
 */
public record ElemeSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     * @throws ValidateException        if routing, credential, callback, or scope data violate the frozen definition
     */
    public ElemeSourceSettings {
        if (!ElemeDefinition.ID.equals(vendor) || !ElemeDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Eleme settings must select eleme/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Eleme settings require a registered redirect URI");
        }
    ...
    public String toString() {
        return "ElemeSourceSettings[vendor=" + vendor + ", variant=" + variant
