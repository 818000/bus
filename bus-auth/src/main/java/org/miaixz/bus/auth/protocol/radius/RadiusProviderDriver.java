 ...
 * @author Kimi Liu
 */
public final class RadiusProviderDriver implements SourceDriver<RadiusServerOptions> {
    ...
    public SourceWorker compile(
        ...
        final RadiusServerOptions options = require(source.getOptions());
        ...
        final RadiusRequestHandler handler = Assert.notNull(
                services.bindingParser().parse(
                        ...
                        services.bindingLoader().load(record, RadiusRequestHandler.class)),
        ...
        final AccountingService accounting = new AccountingService(source.getId(), options, handler, services,
                authenticator);
        return new CompiledProvider(new RadiusServerScheme().manifest(), access, accounting);
    }
