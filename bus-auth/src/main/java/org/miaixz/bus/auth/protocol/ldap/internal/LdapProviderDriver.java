 ...
 * @author Kimi Liu
 */
public final class LdapProviderDriver implements ProviderDriver<LdapProviderSettings> {
    ...
    /**
     * Returns the LDAP server profile bound to this driver.
     *
     * @return immutable LDAP Provider profile
     */
    @Override
    public LdapProviderProfile profile() {
        return new LdapProviderProfile();
    }

    /**
     * Validates typed settings, enforces the shared message baseline, resolves DirectoryStore, and assembles all
     * services.
     *
    ...
    public RuntimeProvider compile(
        ...
        final LdapProviderSettings settings = decode(source);
        ...
        final DirectoryStore store = Assert.notNull(
                services.bindingResolver().resolve(record, DirectoryStore.class),
                "LDAP external DirectoryStore binding must not be null");
        return new CompiledProvider(new LdapProviderProfile().manifest(),
                new BindService(source.getId(), settings, store), new UnbindService(source.getId(), settings, store),
                new SearchService(source.getId(), settings, store), new ModifyService(source.getId(), settings, store),
                new AddService(source.getId(), settings, store), new DeleteService(source.getId(), settings, store),
                new ModifyDNService(source.getId(), settings, store),
                new CompareService(source.getId(), settings, store),
                new AbandonService(source.getId(), settings, store),
                new ExtendedOperationService(source.getId(), settings, store));
    }
    ...
    private static final class CompiledProvider implements RuntimeProvider {
        ...
        public <Q, S> CompletionStage<Outcome<S>> invoke(
            ...
            final CompletionStage<? extends Outcome<?>> stage;
            if (capability == LdapProviderProfile.BIND)
                stage = bind.bind(message, context, timeout);
            else if (capability == LdapProviderProfile.UNBIND)
                stage = unbind.unbind(message, context, timeout);
            else if (capability == LdapProviderProfile.SEARCH)
                stage = search.search(message, context, timeout);
            else if (capability == LdapProviderProfile.MODIFY)
                stage = modify.modify(message, context, timeout);
            else if (capability == LdapProviderProfile.ADD)
                stage = add.add(message, context, timeout);
            else if (capability == LdapProviderProfile.DELETE)
                stage = delete.delete(message, context, timeout);
            else if (capability == LdapProviderProfile.MODIFY_DN)
                stage = modifyDn.modifyDN(message, context, timeout);
            else if (capability == LdapProviderProfile.COMPARE)
                stage = compare.compare(message, context, timeout);
            else if (capability == LdapProviderProfile.ABANDON)
                stage = abandon.abandon(message, context, timeout);
            else if (capability == LdapProviderProfile.EXTENDED)
