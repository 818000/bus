...
/**
 * Implements the provider-neutral WeCom realm surface under the stable WeChat Vendor.
 * <p>
 ...
 * @author Kimi Liu
 */
public final class WeChatEeRealmAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable WeChat enterprise Variant.
     */
    private final VariantManifest.Variant variant;

    /**
     * Validated WeChat enterprise deployment options containing only a credential reference.
     */
     ...
     * @param options  validated WeChat enterprise options
    ...
    public WeChatEeRealmAdapter(final String spaceId, final String sourceId, final WeChatManifest manifest,
        ...
        Assert.notBlank(spaceId, "WeChat enterprise space id must not be blank");
        Assert.notBlank(sourceId, "WeChat enterprise Source id must not be blank");
        final WeChatManifest selectedManifest = Assert.notNull(manifest, "WeChat enterprise manifest must not be null");
        this.variant = Assert.notNull(variant, "WeChat enterprise Variant must not be null");
        this.options = Assert.notNull(options, "WeChat enterprise options must not be null");
        this.services = Assert.notNull(services, "WeChat enterprise services must not be null");
        ...
        if (this.targets.token().isEmpty()
            ...
            throw new ValidateException("WeChat enterprise manifest has an invalid management target set");
        }
        if (!this.targets.management().get(Builder.ENTERPRISE_USERS)
            ...
            throw new ValidateException("WeChat enterprise user-list targets must resolve identically");
        }
    ...
    private static void requireKinds(final Set<Realm.Kind> kinds) {
        if (!SUPPORTED_KINDS.containsAll(kinds)) {
            throw new ValidateException("WeChat enterprise snapshot contains an unsupported kind");
        }
    }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(
                rejected(ErrorCode._400, "WeChat enterprise capability is not declared by the selected manifest"));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> mismatch() {
        return completed(
                rejected(ErrorCode._400, "WeChat enterprise request does not match the selected capability contract"));
    }
    ...
    /**
     * Routes one exact WeChat enterprise capability and request type.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "WeChat enterprise capability must not be null");
        Assert.notNull(context, "WeChat enterprise context must not be null");
        Assert.notNull(timeout, "WeChat enterprise timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return missing();
        }
        if (capability.equals(Realm.describe(WeChatManifest.ID)) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(WeChatManifest.enterpriseDescription())));
        }
        if (capability.equals(Realm.snapshot(WeChatManifest.ID)) && request instanceof Realm.Snapshot snapshot) {
            return narrow(snapshot(snapshot, context, timeout), capability.responseType());
        }
        if (capability.equals(Realm.retrieve(WeChatManifest.ID)) && request instanceof Realm.Retrieve retrieve) {
    ...
    private CompletionStage<Outcome<Realm.Page>> snapshot(
        ...
        try {
            ...
            return completed(rejected(ErrorCode._400, "WeChat enterprise snapshot request is invalid"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "WeChat enterprise snapshot has no remaining timeout"));
        }
    ...
    private Outcome<Realm.Page> snapshot(
        ...
        try {
            ...
            return failed(ErrorCode._500, "WeChat enterprise snapshot processing failed locally");
        }
    }
    ...
    private CompletionStage<Outcome<Realm.Retrieved>> retrieve(
        ...
        if (!SUPPORTED_KINDS.contains(request.key().kind())) {
            return completed(rejected(ErrorCode._400, "WeChat enterprise retrieve kind is unsupported"));
        }
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "WeChat enterprise retrieve has no remaining timeout"));
        }
        final Instant observedAt = FabricX.clock(services.fabric()).now();
    ...
    private Outcome<Realm.Retrieved> retrieve(
        ...
        try {
            return switch (key.kind()) {
                ...
                default -> rejected(ErrorCode._400, "WeChat enterprise retrieve kind is unsupported");
            };
        } catch (ValidateException ignored) {
            return failed(ErrorCode._502, "WeChat WeCom management API returned an invalid retrieval projection");
        } catch (RuntimeException ignored) {
            return failed(ErrorCode._500, "WeChat enterprise retrieval processing failed locally");
        }
    }
    ...
    private <T> CompletionStage<Outcome<T>> execute(
        ...
        return CompletableFuture.<Outcome<T>>supplyAsync(() -> {
            if (timeout.expired()) {
                return failed(ErrorCode._408, "WeChat enterprise operation has no remaining timeout");
            }
            try {
                return Assert.notNull(operation.apply(access), "WeChat enterprise operation returned no outcome");
            } catch (TimeoutException ignored) {
                return failed(ErrorCode._408, "WeChat enterprise operation timed out");
            } catch (RuntimeException ignored) {
    ...
    private Outcome<List<Department>> departmentList(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = getUnpaged(
                target(Builder.ENTERPRISE_ORGANIZATIONS),
                access,
                timeout,
                operation,
                Map.of());
    ...
    private Outcome<List<Group>> groupList(final Access access, final String operation, final Timeout timeout) {
        final Outcome<JsonValue.ObjectValue> fetched = getUnpaged(
                target(Builder.ENTERPRISE_GROUPS),
                access,
                timeout,
                operation,
                Map.of());
    ...
    private Outcome<List<Member>> memberList(
            ...
            final Timeout timeout) {
        final Outcome<JsonValue.ObjectValue> fetched = getUnpaged(
                target(Builder.ENTERPRISE_GROUP_MEMBERS),
                access,
                timeout,
                operation,
    ...
    private String fingerprint(final JsonValue value) {
        final byte[] encoded = services.jsonProvider().writeValue(value);
    ...
    private Endpoint target(final String name) {
        ...
        if (endpoint == null) {
            throw new ValidateException("WeChat enterprise manifest omits a required management target");
        }
        return endpoint;
    }
