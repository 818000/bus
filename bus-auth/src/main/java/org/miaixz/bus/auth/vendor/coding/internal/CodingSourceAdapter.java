...
import org.miaixz.bus.auth.vendor.coding.CodingSourceSettings;
...
public final class CodingSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable CODING definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded CODING settings.
     */
    private final CodingSourceSettings settings;

    /**
     * Caller-owned runtime, resolver, JSON, network, and execution dependencies.
     */
    ...
    /**
     * Creates one Source-bound CODING adapter from the frozen default definition.
     *
     * @param namespaceId       registration namespace used to isolate browser state and credentials
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected CODING definition
     * @param variantDefinition selected default variant definition
     * @param settings          decoded externally loaded CODING settings
     ...
     */
    public CodingSourceAdapter(final String namespaceId, final String sourceId, final CodingDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final CodingSourceSettings settings,
            final ExecutionServices services) {
        Assert.notNull(vendorDefinition, "CODING definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "CODING Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "CODING definition must not be null");
        this.settings = Assert.notNull(settings, "CODING settings must not be null");
        this.services = Assert.notNull(services, "CODING execution services must not be null");
        if (!CodingDefinition.ID.equals(vendorDefinition.type())
                || !vendorDefinition.variant(CodingDefinition.DEFAULT).equals(variantDefinition)
                || !CodingDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !CodingDefinition.ID.equals(settings.vendor())
                || !CodingDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("CODING adapter requires the coding/default OAuth 2.0 definition");
        }
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        if (timeout.expired() || initiation.nonce().isPresent() || initiation.codeChallenge().isPresent()) {
            return completed(failed("CODING authorization security material violates the frozen definition"));
        }
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        if (resolution == null) {
            return completed(failed("CODING client-secret resolver returned no stage"));
        }
        return resolution
                .handle(
    ...
    private Outcome<Access> decodeToken(final HttpResponse response) {
        ...
        try {
            ...
            if (!TokenType.BEARER.value().equalsIgnoreCase(tokenType) || !settings.team().equals(team)
    ...
    private CallbackWire callback(final Callback.Inbound callback) {
        ...
        if (success && (!settings.team().equals(team) || !separated(scope, Symbol.COMMA).equals(requestedScopes()))) {
    ...
    /**
     * Returns explicit registered scopes or the immutable definition default.
     *
