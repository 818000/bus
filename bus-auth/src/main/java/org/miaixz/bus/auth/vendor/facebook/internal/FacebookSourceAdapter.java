...
import org.miaixz.bus.auth.vendor.facebook.FacebookSourceSettings;
...
public final class FacebookSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable Facebook definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Facebook settings.
     */
    private final FacebookSourceSettings settings;
    ...
    /**
     * Creates one Source-bound Facebook Login adapter from the frozen v26.0 definition.
     *
     * @param namespaceId       registration namespace used to isolate state and credential resolution
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected Facebook definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded Facebook settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if routing, protocol, definition, callback, or standard authorization is
     *                                  inconsistent
     */
    public FacebookSourceAdapter(final String namespaceId, final String sourceId,
            final FacebookDefinition vendorDefinition, final VendorDefinition.Definition variantDefinition,
            final FacebookSourceSettings settings, final ExecutionServices services) {
        final FacebookDefinition selectedProfile = Assert
                .notNull(vendorDefinition, "Facebook definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Facebook Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Facebook definition must not be null");
        this.settings = Assert.notNull(settings, "Facebook settings must not be null");
        this.services = Assert.notNull(services, "Facebook execution services must not be null");
        if (!FacebookDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(FacebookDefinition.DEFAULT).equals(variantDefinition)
                || !FacebookDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !FacebookDefinition.ID.equals(settings.vendor())
                || !FacebookDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("Facebook adapter requires the facebook/default OAuth 2.0 definition");
        }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected("Facebook capability is not declared by the selected definition"));
    }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected Facebook definition.
     *
    ...
    private Outcome<Access> token(final String code, final SecretLease secret, final Timeout.Budget timeout) {
        ...
        try {
            ...
            try (HttpResponse response = Fabric.http(services.fabricContext()).url(endpoint.url().toString())
                    .method(Http.Method.GET).query(OAuth2.Parameters.CLIENT_ID, settings.clientId())
                    .query(OAuth2.Parameters.REDIRECT_URI, settings.redirectUri().getOrNull())
    ...
    /**
     * Returns explicit permissions or immutable definition defaults.
     *
