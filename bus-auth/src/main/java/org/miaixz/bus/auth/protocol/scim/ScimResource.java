...
public record ScimResource(ResourceType type, String id, String externalId, Set<String> schemas,
     ...
     * @throws ValidateException if a component violates the SCIM resource or bounded JSON constraints
     */
    public ScimResource {
        ...
        Assert.isTrue(
                attributes.keySet().stream().noneMatch(ScimResource::reserved),
     ...
     * @throws ValidateException if the identifier or attributes are invalid
     */
     ...
     * @throws ValidateException if a key/value, depth, or collection bound is invalid
     */
    private static Map<String, Object> object(final Map<String, ?> source, final int depth) {
        ...
        input.forEach((key, value) -> {
            ...
            Assert.isTrue(
                    result.keySet().stream().noneMatch(existing -> existing.equalsIgnoreCase(name)),
                    () -> new ValidateException("SCIM attribute is duplicated"));
            result.put(name, copy(value, depth + Normal._1));
        });
     ...
     * @throws ValidateException if the value type, depth, or collection bound is invalid
     */
     ...
     * @throws ValidateException if a supplied value is blank or oversized
     */
    ...
    /**
     * Tests whether a case-insensitive SCIM attribute name is reserved by the resource envelope.
     *
     * @param name attribute name
     * @return whether the name is reserved
     */
    private static boolean reserved(final String name) {
        return RESERVED.stream().anyMatch(value -> value.equalsIgnoreCase(name));
    }
    ...
    public Object attribute(final String name) {
        final String requested = Assert
                .notBlank(name, () -> new ValidateException("SCIM attribute name must not be blank"));
        final String stored = attributes.keySet().stream().filter(key -> key.equalsIgnoreCase(requested)).findFirst()
                .orElse(null);
        return copy(stored == null ? null : attributes.get(stored), Normal._1);
    }
     ...
     * @throws ValidateException if the replacement attributes are invalid
     */
    ...
    public record Meta(ResourceType resourceType, Instant created, Instant lastModified, URI location, String version) {
         ...
         * @throws ValidateException if metadata is incomplete, inconsistent, or contains an invalid strong tag
         */
        public Meta {
            resourceType = Assert
            ...
            Assert.isTrue(
                    ...
                    () -> new ValidateException("SCIM metadata version must be a strong entity tag"));
            ScimEtag.normalize(version, false);
        }
    }
