 ...
 * consumption, key derivation, and rollback are delegated to {@link RedirectState}; protocol parsing and Roster routing
 * remain outside both classes.
 * </p>
 *
 * @author Kimi Liu
 */
public class RedirectManager {
    ...
    public RedirectManager(final String spaceId, final String sourceId, final VendorManifest.Variant variant,
        ...
        this.pkceGenerator = pkceEnabled ? new PkceGenerator(rule) : null;
        this.randomBytes = (Math.max(256, rule.minimumEntropyBits()) + Byte.SIZE - 1) / Byte.SIZE;
    ...
    public CompletionStage<Outcome<SourceWorkflow.Stage>> initiate(
        ...
        if (!sourceId.equals(request.sourceId()) || vendorOptions.redirectUri().isEmpty()
            ...
            return completed(rejected("Vendor redirect callback target does not match the configured Source"));
        }
        if (timeout.expired()) {
     ...
     */
    public CompletionStage<Outcome<Identity>> complete(
     ...
     */
    private CompletionStage<Outcome<Identity>> finish(
        ...
        final CompletionStage<Outcome<Identity>> stage;
        try {
        ...
        return stage.handle((outcome, cause) -> {
            ...
            if (cause != null || outcome == null) {
                return RedirectManager.<Identity>failed("Vendor callback completion failed");
            }
            return switch (outcome) {
                case Outcome.Succeeded<Identity> success -> success.value() == null
    ...
    /**
     * Generates an opaque value sized by the applicable security rule.
     *
    ...
    public interface CompleteOperation {
         ...
         * @return verified external identity outcome
         */
        CompletionStage<Outcome<Identity>> complete(Completion completion, Context context, Timeout timeout);
