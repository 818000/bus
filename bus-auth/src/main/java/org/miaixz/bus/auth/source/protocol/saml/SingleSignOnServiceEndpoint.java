...
/**
 * Models a SAML Metadata {@code SingleSignOnService} endpoint.
 *
 ...
 * @author Kimi Liu
 */
public record SingleSignOnServiceEndpoint(SamlBinding binding, String location, Optional<String> responseLocation,
        List<byte[]> extensions, Map<QName, String> attributes) {

    /**
     * Validates EndpointType URI fields and takes deep immutable ownership of extensions.
     *
     * @throws IllegalArgumentException if a component, item, key, or value is {@code null}
     * @throws ValidateException        if a URI or extension value is invalid
     */
    public SingleSignOnServiceEndpoint {
        Assert.notNull(binding, "SAML SingleSignOnService Binding must not be null");
        absolute(location, "SAML SingleSignOnService Location");
        responseLocation = optionalUri(responseLocation, "SAML SingleSignOnService ResponseLocation");
        extensions = elements(extensions, "SAML SingleSignOnService extension");
        attributes = attributes(attributes, "SAML SingleSignOnService");
    }
    ...
    public List<byte[]> extensions() {
        return elements(extensions, "SAML SingleSignOnService extension");
    }
