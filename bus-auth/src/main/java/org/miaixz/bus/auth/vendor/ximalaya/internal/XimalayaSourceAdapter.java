...
import org.miaixz.bus.auth.vendor.ximalaya.XimalayaSourceSettings;
...
public final class XimalayaSourceAdapter implements VendorAdapter {
    ...
    /**
     * Exact immutable Ximalaya definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded Ximalaya settings.
     */
    private final XimalayaSourceSettings settings;
    ...
    /**
     * Creates one Source-bound Ximalaya adapter from the frozen default definition.
     *
     * @param namespaceId       registration namespace isolating browser state and credentials
     * @param sourceId          registered Source identifier
     * @param vendorDefinition  selected Ximalaya definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded Ximalaya settings
     ...
     */
    public XimalayaSourceAdapter(final String namespaceId, final String sourceId,
            final XimalayaDefinition vendorDefinition, final VendorDefinition.Definition variantDefinition,
            final XimalayaSourceSettings settings, final ExecutionServices services) {
        final XimalayaDefinition selected = Assert.notNull(vendorDefinition, "Ximalaya definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "Ximalaya Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "Ximalaya definition must not be null");
        this.settings = Assert.notNull(settings, "Ximalaya settings must not be null");
        this.services = Assert.notNull(services, "Ximalaya execution services must not be null");
        if (!XimalayaDefinition.ID.equals(selected.type())
                || !selected.variant(XimalayaDefinition.DEFAULT).equals(variantDefinition)
                || !XimalayaDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !XimalayaDefinition.ID.equals(settings.vendor())
                || !XimalayaDefinition.DEFAULT.equals(settings.variant())) {
            throw new ValidateException("Ximalaya adapter requires the ximalaya/default OAuth 2.0 definition");
        }
        this.redirectManager = RedirectManager.create(namespaceId, sourceId, variantDefinition, settings, services);
        final VendorTargets.Resolved resolvedTargets = variantDefinition.targets().resolve(settings);
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        if (timeout.expired() || initiation.nonce().isPresent() || initiation.codeChallenge().isPresent()) {
            return completed(failed(ErrorCode._500, "Ximalaya browser material violates the frozen definition"));
        }
        try {
    ...
    private UnoUrl authorize(final AuthorizationRequest request) {
        ...
        extensions
                .put(XimalayaDefinition.Parameters.CLIENT_OS_TYPE, new JsonValue.StringValue(settings.clientOsType()));
        extensions.put(XimalayaDefinition.Parameters.DEVICE_ID, new JsonValue.StringValue(settings.deviceId()));
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        if (response.scope().isPresent() || response.issuer().isPresent() || response.extensions().values().size() != 1
                ...
                || !settings.deviceId().equals(actualDevice.value())) {
    ...
    private Outcome<Access> token(final String code, final SecretLease secret, final Timeout.Budget timeout) {
        ...
        try {
            ...
            final List<Parameter> parameters = List.of(
                    ...
                    new Parameter("device_id", settings.deviceId()),
    ...
    private Outcome<Access> token(final HttpResponse response) {
        ...
        try {
            ...
            if (device != null && !settings.deviceId().equals(device)) {
    ...
    private Outcome<ExternalIdentity> profile(
        ...
        query.put("app_key", settings.clientId());
        query.put("client_os_type", settings.clientOsType());
        query.put("device_id", settings.deviceId());
        query.put("pack_id", settings.packageId());
    ...
    private CompletionStage<Outcome<SecretLease>> resolve(final Context context, final Timeout.Budget timeout) {
        try {
            ...
            if (stage == null) {
                return completed(failed(ErrorCode._502, "Ximalaya application-secret resolver returned no stage"));
            }
            return stage.handle(
    ...
    private AuthorizationResponseDecoder.Decoded decode(final Callback.Inbound callback) {
        ...
        for (Callback.Parameter parameter : inbound.parameters()) {
            ...
            switch (parameter.name()) {
                ...
                case XimalayaDefinition.Parameters.DEVICE_ID -> device = unique(device);
