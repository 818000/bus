...
/**
 * Implements the provider-neutral GitLab Enterprise realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public final class GitLabRealmAdapter implements VendorAdapter {
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "GitLab enterprise capability must not be null");
        Assert.notNull(context, "GitLab enterprise context must not be null");
        Assert.notNull(timeout, "GitLab enterprise timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected(ErrorCode._400, "GitLab enterprise capability is not declared"));
        }
        if (capability.equals(Realm.describe(GitLabManifest.ID)) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(GitLabManifest.enterpriseDescription())));
        }
        if (capability.equals(Realm.snapshot(GitLabManifest.ID)) && request instanceof Realm.Snapshot value) {
            return narrow(snapshot(value, context, timeout), capability.responseType());
        }
        if (capability.equals(Realm.retrieve(GitLabManifest.ID)) && request instanceof Realm.Retrieve value) {
            return narrow(retrieve(value, context, timeout), capability.responseType());
        }
        return completed(rejected(ErrorCode._400, "GitLab enterprise request does not match the capability contract"));
    }
    ...
    private Outcome<Group> root(final SecretLease secret, final Timeout timeout, final String operation) {
        final Outcome<JsonValue.ObjectValue> fetched = get(
                target(Builder.ENTERPRISE_ORGANIZATIONS).url(),
                secret,
                timeout,
                operation);
        if (!(fetched instanceof Outcome.Succeeded<JsonValue.ObjectValue> success)) {
    ...
    private Outcome<WirePage<Group>> descendants(
        ...
        final Url request = url(
                Builder.ENTERPRISE_ORGANIZATION,
                List.of(rootId, "descendant_groups"),
                query(page, limit));
    ...
    private Outcome<WirePage<Member>> memberPage(
        ...
        return collection(
                url(Builder.ENTERPRISE_ORGANIZATION_USERS, segments, query(page, limit)),
                page,
                secret,
                timeout,
                operation,
