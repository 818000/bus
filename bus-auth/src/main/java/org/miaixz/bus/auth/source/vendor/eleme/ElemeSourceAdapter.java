...
public class ElemeSourceAdapter implements VendorAdapter {
     ...
     */
    private static Outcome<Identity> merchantError(final JsonValue.ObjectValue error) {
     ...
     */
    private Outcome<Identity> merchant(final String accessToken, final SecretLease secret, final Timeout timeout) {
        byte[] body = null;
        ...
        try {
            ...
            appKeyJson = JsonKit.writeValue(new JsonValue.StringValue(options.clientId()));
            timestampJson = JsonKit.writeValue(new JsonValue.NumberValue(BigDecimal.valueOf(timestamp)));
            ...
            body = JsonKit.writeValue(new JsonValue.ObjectValue(members));
     ...
     */
    private Outcome<Identity> merchant(final Response response, final String requestId, final Timeout timeout) {
     ...
     */
    private Outcome<Identity> merchant(
    ...
    /**
     * Maps one strict successful merchant result to a implementation-neutral external identity.
     *
     * @param result  strict Eleme merchant result
     * @param timeout shared clock used for evidence verification time
     * @return verified external identity
     */
    private Outcome<Identity> identity(final JsonValue.ObjectValue result, final Timeout timeout) {
        ...
        final Evidence evidence = new Evidence(Evidence.Type.FEDERATED, Evidence.Strength.SINGLE_FACTOR,
                new Evidence.Claim("eleme_user_id", new JsonValue.StringValue(merchant.subject()), AUTHORITY,
                        timeout.clock().now()));
        return Outcome.succeeded(new Identity(sourceId, merchant.subject(), merchant.attributes(), List.of(evidence)));
    }
