...
import org.miaixz.bus.auth.vendor.dingtalk.DingTalkSourceSettings;
...
public final class DingTalkSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable DingTalk variant definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded DingTalk settings.
     */
    private final DingTalkSourceSettings settings;

    /**
     * Caller-owned resolver, JSON, network, crypto-policy, clock, and execution dependencies.
     */
     ...
     * @param vendorDefinition  selected DingTalk definition
     ...
     */
    public DingTalkSourceAdapter(final String namespaceId, final String sourceId,
            final DingTalkDefinition vendorDefinition, final VendorDefinition.Definition variantDefinition,
            final DingTalkSourceSettings settings, final ExecutionServices services) {
        final DingTalkDefinition selectedProfile = Assert
                .notNull(vendorDefinition, "DingTalk definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "DingTalk Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "DingTalk definition must not be null");
        this.settings = Assert.notNull(settings, "DingTalk settings must not be null");
        this.services = Assert.notNull(services, "DingTalk execution services must not be null");
        if (!DingTalkDefinition.ID.equals(selectedProfile.type())
                || !selectedProfile.variant(settings.variant()).equals(variantDefinition)
                || !settings.variant().equals(variantDefinition.variant())
                || !DingTalkDefinition.ID.equals(settings.vendor())
                || DingTalkDefinition.OAUTH2.equals(settings.variant())
                        && variantDefinition.protocol() != Protocol.OAUTH2
                || DingTalkDefinition.ACCOUNT.equals(settings.variant())
                        && variantDefinition.protocol() != Protocol.VENDOR_AUTH) {
            throw new ValidateException("DingTalk adapter profile, variant, and settings must match");
        }
        this.redirectManager = RedirectManager.create(namespaceId, sourceId, variantDefinition, settings, services);
        this.authorizationEncoder = DingTalkDefinition.OAUTH2.equals(settings.variant())
                ? new AuthorizationRequestEncoder(
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        if (DingTalkDefinition.OAUTH2.equals(settings.variant()) && capability.equals(OAuth2SourceProfile.AUTHORIZATION)
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        if (timeout.expired() || initiation.nonce().isPresent() || initiation.codeChallenge().isPresent()) {
            return completed(failed("DingTalk authorization security material violates the selected definition"));
        }
        try {
            final String endpoint = variantDefinition.targets().resolve(settings).authorization().getOrNull().url()
                    .toString();
            final List<Parameter> parameters = DingTalkDefinition.OAUTH2.equals(settings.variant())
    ...
    private AuthorizationRequest authorization(final AuthorizationRequest source, final Optional<String> state) {
        if (source.responseType() != ResponseType.CODE || !settings.clientId().equals(source.clientId())
                || !settings.redirectUri().equals(source.redirectUri()) || source.scope().isEmpty()
        ...
        return new AuthorizationRequest(ResponseType.CODE, settings.clientId(), settings.redirectUri(),
    ...
    private List<Parameter> delegatedParameters(final String state) {
        ...
        parameters.add(new Parameter(OAuth2.Parameters.REDIRECT_URI, settings.redirectUri().getOrNull()));
        ...
        settings.orgType().ifPresent(value -> parameters.add(new Parameter("org_type", value)));
        settings.corpId().ifPresent(value -> parameters.add(new Parameter("corpId", value)));
        if (settings.exclusiveLogin()) {
            parameters.add(new Parameter("exclusiveLogin", Normal.TRUE));
            parameters.add(new Parameter("exclusiveCorpId", settings.exclusiveCorpId().getOrNull()));
        }
    ...
    private List<Parameter> accountParameters(final String state) {
        return List.of(
                new Parameter("appid", settings.clientId()),
    ...
    private JsonValue.ObjectValue extensions() {
        ...
        settings.orgType().ifPresent(value -> values.put("org_type", new JsonValue.StringValue(value)));
        settings.corpId().ifPresent(value -> values.put("corpId", new JsonValue.StringValue(value)));
        if (settings.exclusiveLogin()) {
            values.put("exclusiveLogin", new JsonValue.BooleanValue(true));
            values.put("exclusiveCorpId", new JsonValue.StringValue(settings.exclusiveCorpId().getOrNull()));
        }
    ...
    private CompletionStage<Outcome<ExternalIdentity>> identity(
        ...
        if (resolution == null) {
            return completed(failed("DingTalk credential resolver returned no stage"));
        }
        return resolution
                .handle(
                ...
                .thenCompose(outcome -> switch (outcome) {
                    case Outcome.Succeeded<SecretLease> success -> DingTalkDefinition.OAUTH2.equals(settings.variant())
    ...
    private CompletionStage<Outcome<ExternalIdentity>> token(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (secret) {
                ...
                fields.put("clientId", new JsonValue.StringValue(settings.clientId()));
    ...
    private CompletionStage<Outcome<ExternalIdentity>> profile(final Access access, final Timeout.Budget timeout) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                return DingTalkSourceAdapter.<ExternalIdentity>failed("DingTalk profile request failed");
            }
        }, services.executor());
    }
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (secret) {
                ...
                final String endpoint = variantDefinition.targets().resolve(settings).userInfo().getOrNull().url()
                        .toString();
                try (HttpResponse response = Fabric.http(services.fabricContext()).url(endpoint)
                        .method(Http.Method.POST).query("accessKey", settings.clientId()).query("timestamp", timestamp)
                ...
                return DingTalkSourceAdapter.<ExternalIdentity>failed("DingTalk account request failed");
            } finally {
                clear(key);
                clear(message);
    ...
    private CallbackWire callback(final Callback.Inbound callback) {
        ...
        String state = null;
        String error = null;
        final String successName = DingTalkDefinition.OAUTH2.equals(settings.variant()) ? "authCode" : "code";
