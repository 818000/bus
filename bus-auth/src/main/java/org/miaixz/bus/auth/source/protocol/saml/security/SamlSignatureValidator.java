...
public class SamlSignatureValidator {
     ...
     * @param ids     codec-proven unique ID index
     */
    ...
    public CompletionStage<Outcome<RedirectBindingCodec.Decoded<AuthnRequest>>> validateAuthnRequest(
        ...
        Assert.notNull(options, "SAML signature options must not be null");
     ...
     * @param ids                 unique document ID index
    ...
    private CompletionStage<Outcome<PublicKey>> certificate(
        ...
        final CertificateLoader.Request request = new CertificateLoader.Request(services.entry(), issuer,
        ...
        final CompletionStage<Outcome<CertificateMaterial>> resolution = Outcome.mapStage(
                ...
                loaded -> services.certificateParser().parse(services.entry(), request, loaded));
        if (resolution == null)
    ...
    /**
     * Checks an algorithm against the shared SAML security rule and the SHA-1 prohibition.
     *
     * @param algorithm exact standard algorithm URI
     * @return whether the security rule permits the algorithm
     */
    private boolean allowed(final String algorithm) {
        return !sha1(algorithm) && policies.require(Protocol.SAML).algorithms().contains(algorithm);
    }
