 ...
 * only under Source-isolated irreversible digests.
 * </p>
 *
 * @author Kimi Liu
 */
public class AccessTokenIssuer {
    ...
    /**
     * Source identifier used to isolate every opaque credential digest.
     */
    private final String sourceId;

    /**
     * Frozen authorization-server options governing enabled grants and token lifetimes.
     */
    ...
    /**
     * Source-isolated opaque token generation and irreversible key derivation.
     */
    private final TokenMaterial tokenMaterial;

    /**
     * Creates an initial token issuer for one compiled OAuth authorization server.
     *
            ...
            return completed(
                    Outcome.rejected(
                            failure(
                                    ErrorCode._400,
                                    OAuth2ErrorCode.UNSUPPORTED_GRANT_TYPE,
                                    "OAuth 2.x token grant is disabled by the authorization server")));
        }
    ...
    private boolean validAuthorizationCode(
        ...
        if (!sourceId.equals(entry.sourceId()) || !clientId.equals(entry.clientId())) {
            return false;
        }
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> exchangeTarget(
        ...
        try {
            final ResourceLoader.Request request = new ResourceLoader.Request(services.entry(), sourceId,
                    grant.audience(), grant.resource());
            resolution = Outcome.mapStage(
                    () -> services.resourceLoader().load(request, context, timeout),
                    loaded -> services.resourceParser().parse(services.entry(), request, loaded));
        } catch (RuntimeException exception) {
     ...
     * @param serverScopes   authorization-server-supported scopes
    ...
    private List<String> effectiveScope(
            ...
            final Set<String> serverScopes,
            final Set<String> resourceScopes) {
        if (requested.isPresent()) {
            final List<String> values = requested.getOrNull().values();
            if (!validScope(values, Set.copyOf(subjectScopes), clientScopes, serverScopes)
        ...
        for (String value : subjectScopes) {
            if (clientScopes.contains(value) && serverScopes.contains(value)
    ...
    /**
     * Resolves one opaque access token and verifies Source, client, and expiration bindings.
     *
    ...
    private CompletionStage<Outcome<AccessTokenCache.Entry>> activeToken(
        ...
        return lookup.handle((stored, thrown) -> new CacheResult<>(stored, thrown)).thenCompose(result -> {
            ...
            if (stored == null || !stored.expiresAt().isAfter(timeout.clock().now())
                    || !sourceId.equals(stored.value().sourceId()) || !clientId.equals(stored.value().clientId())) {
                return completedValue(
            ...
            return authorization.handle((state, thrown) -> {
                ...
                if (state == null || !state.expiresAt().isAfter(timeout.clock().now())
                        ...
                        || !sourceId.equals(state.value().sourceId()) || !clientId.equals(state.value().clientId())) {
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> deviceCode(
        ...
        return lookup.handle((stored, thrown) -> new CacheResult<>(stored, thrown)).thenCompose(result -> {
            ...
            if (!sourceId.equals(entry.sourceId()) || !client.id().equals(entry.clientId())) {
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> pollPending(
        ...
        final DeviceCodeCache.Entry update = new DeviceCodeCache.Entry(entry.sourceId(), entry.clientId(),
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> consumeApproved(
            ...
            final int attempt,
            final Instant now) {
        final DeviceCodeCache.Entry consumed = new DeviceCodeCache.Entry(entry.sourceId(), entry.clientId(),
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> createAuthorization(
        ...
        final AuthorizationCache.Entry entry = new AuthorizationCache.Entry(sourceId, grant.clientId(),
                AuthorizationCache.Status.ACTIVE);
        final CompletionStage<Boolean> creation;
        try {
            creation = services.authorizationCache()
                    .issue(AuthorizationCache.key(sourceId, authorizationId), new ExpiringValue<>(entry, expiresAt));
        } catch (RuntimeException exception) {
                    ...
                    return createAccess(authorized, context, timeout, 1).thenCompose(outcome -> {
                        ...
                        try {
                            return services.authorizationCache()
                                    .delete(AuthorizationCache.key(sourceId, authorizationId))
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> createAccess(
        ...
        final ExpiringValue<AccessTokenCache.Entry> value = new ExpiringValue<>(new AccessTokenCache.Entry(sourceId,
    ...
    private CompletionStage<Outcome<TokenEndpointResponse>> createRefresh(
        ...
        final ExpiringValue<RefreshTokenCache.Entry> value = new ExpiringValue<>(
                new RefreshTokenCache.Entry(sourceId, grant.clientId(), grant.subjectId(), familyId, 0L, grant.scope(),
                        grant.audience(), grant.confirmation(), grant.openIdBinding(), RefreshTokenCache.Status.ACTIVE),
                expiresAt);
        final CompletionStage<Boolean> creation;
        try {
