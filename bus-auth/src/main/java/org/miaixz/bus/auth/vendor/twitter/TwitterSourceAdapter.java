...
public final class TwitterSourceAdapter implements VendorAdapter {
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        if (timeout.expired()) {
            return completed(failed(ErrorCode._408, "Twitter authorization has no remaining time budget"));
        }
        try {
    ...
    private Outcome<Access> token(
        ...
        try {
            if (timeout.expired()) {
                return failed(ErrorCode._408, "Twitter token request has no remaining time budget");
            }
