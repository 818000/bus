 ...
 * @author Kimi Liu
 */
public final class OAuth2ProviderDriver implements SourceDriver<OAuth2ServerOptions> {
    ...
    public SourceWorker compile(
        ...
        final OAuth2ServerOptions options = require(source.getOptions());
        ...
        final AuthorizationCodeIssuer codeIssuer = new AuthorizationCodeIssuer(source.getId(), options, services,
                redirectUriValidator, scopeValidator);
        final AccessTokenIssuer accessTokenIssuer = new AccessTokenIssuer(source.getId(), options, services,
                scopeValidator, pkceValidator, AccessTokenIssuer.Augmenter.passThrough());
        final RefreshTokenRotator refreshTokenRotator = new RefreshTokenRotator(source.getId(), options, services,
                scopeValidator, accessTokenIssuer);
        return new CompiledProvider(manifest(options), new AuthorizationService(codeIssuer),
                new TokenService(accessTokenIssuer, refreshTokenRotator),
                new IntrospectionService(source.getId(), options, services),
                new RevocationService(source.getId(), services),
                new DeviceAuthorizationService(source.getId(), options, services, scopeValidator),
                new AuthorizationServerMetadataService(options));
    }
