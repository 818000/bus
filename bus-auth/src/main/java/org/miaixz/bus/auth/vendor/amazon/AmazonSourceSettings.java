 ...
 * Fixed Amazon endpoints remain owned by {@link AmazonDefinition}.
 * </p>
 *
 ...
 * @param scopes      ordered requested scopes, or empty to use definition defaults
 * @param pkce        whether this registration uses optional S256 PKCE
 * @author Kimi Liu
 */
public record AmazonSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes, boolean pkce)
     ...
     */
    public AmazonSourceSettings {
        if (!AmazonDefinition.ID.equals(vendor) || !AmazonDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Login with Amazon settings must select amazon/default");
        }
    ...
    public String toString() {
        return "AmazonSourceSettings[vendor=" + vendor + ", variant=" + variant
