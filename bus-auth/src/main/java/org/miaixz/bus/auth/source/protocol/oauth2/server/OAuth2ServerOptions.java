...
import org.miaixz.bus.auth.source.protocol.oauth2.grant.GrantPolicy;
...
/**
 * Holds immutable deployment and security policy for one OAuth 2.x authorization server.
 *
 ...
 * @param dpopSupported                       whether the authorization server can issue DPoP-constrained access tokens
...
public record OAuth2ServerOptions(String issuer, Optional<Endpoint> authorizationEndpoint,
    ...
    public OAuth2ServerOptions {
        ...
        if (List.of(
            ...
            throw new ValidateException("OAuth 2.x authorization server options require at least one endpoint");
        }
        ...
        if (dpopSupported) {
            throw new ValidateException(
                    "OAuth 2.x authorization server options cannot enable DPoP without a complete token transport contract");
        }
    ...
    private static Set<GrantType> grants(final Set<GrantType> values) {
        ...
        for (GrantType value : values) {
            if (!IMPLEMENTED_GRANTS.contains(Assert.notNull(value, "OAuth 2.x grant type must not be null"))) {
                throw new ValidateException(
                        "OAuth 2.x authorization server enables a grant not implemented by this version");
            }
        }
    ...
    private static Set<ClientAuthenticationMethod> authentication(final Set<ClientAuthenticationMethod> values) {
        ...
        for (ClientAuthenticationMethod value : values) {
            if (!IMPLEMENTED_AUTHENTICATION
                ...
                throw new ValidateException(
                        "OAuth 2.x authorization server enables client authentication not implemented by this version");
            }
        }
        return Set.copyOf(values);
    }
    ...
    /**
     * Returns this already immutable OAuth 2.x server configuration snapshot.
     *
