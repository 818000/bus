...
/**
 * Executes the RFC 4511 Delete operation under the connection authorization state owned by the store.
 *
 * @author Kimi Liu
 */
public class DeleteService {
    ...
    /**
     * Creates a Delete service for one compiled LDAP server.
     *
     ...
     */
    public DeleteService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Delete Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Delete LDAP server options must not be null");
        this.store = Assert.notNull(store, "LDAP Delete directory store must not be null");
    }

    /**
     * Correlates one external Delete response and preserves failures.
     *
     * @param messageId request identifier
     * @param outcome   external store outcome
     * @param thrown    asynchronous store failure
     * @return complete Delete outcome
     */
    private static Outcome<LdapMessage> map(
            final int messageId,
            final Outcome<DeleteResponse> outcome,
            final Throwable thrown) {
        if (thrown != null || outcome == null) {
            return Outcome.failed(failure("LDAP Delete directory store failed"));
        }
        return switch (outcome) {
            case Outcome.Succeeded<DeleteResponse> succeeded -> succeeded.value() == null
                    ? Outcome.failed(failure("LDAP Delete directory store returned no response"))
    ...
    /**
     * Creates one local formal Delete failure response.
     *
     * @param messageId request identifier
    ...
    /**
     * Executes one standard Delete request and correlates its formal response.
     *
    ...
    public CompletionStage<Outcome<LdapMessage>> delete(
        ...
        try {
            stage = store.delete(sourceId, context.network().connection().getOrThrow().id(), request, context, timeout);
        } catch (RuntimeException exception) {
            return completed(Outcome.failed(failure("LDAP Delete directory store failed")));
        }
        if (stage == null) {
            return completed(Outcome.failed(failure("LDAP Delete directory store returned no stage")));
        }
