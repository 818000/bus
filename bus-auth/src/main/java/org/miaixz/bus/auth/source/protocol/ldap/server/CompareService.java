...
public class CompareService {
    ...
    /**
     * Frozen LDAP server options retained for the uniform LDAP service contract.
     */
    ...
    /**
     * Creates a Compare service for one compiled LDAP server.
     *
     ...
     */
    public CompareService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Compare Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Compare LDAP server options must not be null");
    ...
    public CompletionStage<Outcome<LdapMessage>> compare(
        ...
        try {
            stage = store
                    .compare(sourceId, context.network().connection().getOrThrow().id(), request, context, timeout);
