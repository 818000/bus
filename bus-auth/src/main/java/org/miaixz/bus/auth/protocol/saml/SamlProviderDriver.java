 ...
 * <p>
 * Compilation performs all options and baseline validation before creating services. The runtime receives only standard
 * SAML models and exposes only endpoint-backed capabilities. Successful response and metadata models contain the
 * complete XML Signatures required by the compiled server-role Source policy before they cross the Registry boundary;
 * transport adapters only serialize those already secured models.
 * </p>
 *
 * @author Kimi Liu
 */
public final class SamlProviderDriver implements SourceDriver<SamlServerOptions> {
     ...
     *
     * @return immutable SAML Provider scheme
     */
    @Override
    ...
    public SourceWorker compile(
        ...
        final SamlServerOptions options = require(source.getOptions());
        if (!services.securityBaseline().require(Protocol.SAML).algorithms().contains(options.signatureAlgorithm())) {
            throw new ValidateException("SAML Provider signature algorithm is outside the security baseline");
        }
        if (options.clockSkew().compareTo(services.securityBaseline().require(Protocol.SAML).maximumClockSkew()) > 0) {
        ...
        final SingleLogoutService logout = new SingleLogoutService(source.getId(), options, services, errorMapper);
        ...
        return new CompiledProvider(manifest(options), new SamlIdentityProvider(signOn, logout, metadata), signer);
    }
    ...
    private static final class CompiledProvider implements SourceWorker {
         ...
         * @param signer   XML signer bound to the same Provider options and key inventory
         */
    ...
    private static final class XmlSigner {
         ...
         */
        private XmlSigner(final SamlServerOptions options, final ExecutionServices services,
        ...
        private CompletionStage<Outcome<KeyMaterial>> resolve(final Context context, final Timeout.Budget timeout) {
            ...
            final KeyLoader.Request query = new KeyLoader.Request(options.entityId(),
                    ...
                    timeout.clock().now());
            try {
                final CompletionStage<Outcome<KeyMaterial>> stage = org.miaixz.bus.auth.runtime.LoadResult.parse(
