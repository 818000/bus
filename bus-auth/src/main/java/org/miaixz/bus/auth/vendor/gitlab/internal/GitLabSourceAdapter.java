...
import org.miaixz.bus.auth.vendor.gitlab.GitLabSourceSettings;
...
public final class GitLabSourceAdapter implements VendorAdapter {
    ...
    /**
     * Selected immutable GitLab definition.
     */
    private final VendorDefinition.Definition variantDefinition;

    /**
     * Validated externally loaded GitLab settings.
     */
    private final GitLabSourceSettings settings;
     ...
     * @param vendorDefinition  selected GitLab definition
     * @param variantDefinition exact selected default definition
     * @param settings          decoded externally loaded GitLab settings
     * @param services          caller-owned execution services
     * @throws IllegalArgumentException if an identifier is blank or a collaborator is {@code null}
     * @throws ValidateException        if routing, protocol, definition, callback, or capabilities are inconsistent
     */
    public GitLabSourceAdapter(final String namespaceId, final String sourceId, final GitLabDefinition vendorDefinition,
            final VendorDefinition.Definition variantDefinition, final GitLabSourceSettings settings,
            final ExecutionServices services) {
        final GitLabDefinition selected = Assert.notNull(vendorDefinition, "GitLab definition must not be null");
        this.sourceId = Assert.notBlank(sourceId, "GitLab Source id must not be blank");
        this.variantDefinition = Assert.notNull(variantDefinition, "GitLab definition must not be null");
        this.settings = Assert.notNull(settings, "GitLab settings must not be null");
        this.services = Assert.notNull(services, "GitLab execution services must not be null");
        if (!GitLabDefinition.ID.equals(selected.type())
                || !selected.variant(GitLabDefinition.DEFAULT).equals(variantDefinition)
                || !GitLabDefinition.DEFAULT.equals(variantDefinition.variant())
                || variantDefinition.protocol() != Protocol.OAUTH2 || !GitLabDefinition.ID.equals(settings.vendor())
                || !GitLabDefinition.DEFAULT.equals(settings.variant()) || settings.redirectUri().isEmpty()) {
            throw new ValidateException("GitLab adapter requires the gitlab/default OAuth 2.0 definition");
        }
        ...
        this.standardAdapter = new StandardAdapter(variantDefinition, settings, Optional.of(redirectManager),
                List.of(
                        ...
                        new StandardAdapter.Binding<>(OAuth2SourceProfile.REVOCATION, this::revoke)));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(rejected("GitLab capability is not declared by the selected definition"));
    }
    ...
    /**
     * Returns the exact capability manifest frozen by the selected GitLab definition.
     *
    ...
    private CompletionStage<Outcome<RedirectManager.Prepared>> prepare(
        ...
        try {
            ...
            final String location = endpoint.url().newBuilder().query(OAuth2.Parameters.CLIENT_ID, settings.clientId())
                    ...
                    .query(OAuth2.Parameters.SCOPE, String.join(Symbol.SPACE, settings.scopes()))
    ...
    private Outcome<TokenResponse> validate(final TokenResponse response) {
        try {
            ...
            return failed(ErrorCode._502, "GitLab token success response violates the registered definition");
        }
    }
