...
public class ModifyService {
    ...
    /**
     * Creates a Modify service for one compiled LDAP server.
     *
     ...
     */
    public ModifyService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Modify Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Modify LDAP server options must not be null");
    ...
    public CompletionStage<Outcome<LdapMessage>> modify(
        ...
        try {
            stage = store.modify(sourceId, context.network().connection().getOrThrow().id(), request, context, timeout);
