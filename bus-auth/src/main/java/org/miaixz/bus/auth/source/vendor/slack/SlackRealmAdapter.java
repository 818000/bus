...
/**
 * Implements the implementation-neutral Slack administrator SCIM realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public class SlackRealmAdapter implements VendorAdapter {
    ...
    private static final List<String> MANAGEMENT_TARGETS = List
            .of(Builder.REALM_USERS, Builder.REALM_USER, Builder.REALM_GROUPS, Builder.REALM_GROUP);
    ...
    public SlackRealmAdapter(final String spaceId, final String sourceId, final SlackManifest manifest,
            final VendorManifest.Variant variant, final SlackOptions options, final SourceServices services) {
    ...
    /**
     * Routes one exact Slack SCIM Realm capability.
     *
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Slack Realm capability must not be null");
        Assert.notNull(context, "Slack Realm context must not be null");
        Assert.notNull(timeout, "Slack Realm timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected(ErrorCode._400, "Slack Realm capability is not declared"));
        }
        if (capability.equals(Realm.DESCRIBE) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(SlackManifest.realmDescription())));
        }
        ...
        return completed(rejected(ErrorCode._400, "Slack Realm request does not match the capability contract"));
    }
    ...
    private Outcome<Realm.Page> users(
        ...
        final Outcome<ScimPage<User>> fetched = page(
                target(Builder.REALM_USERS).url(),
                secret,
                count,
                start,
                timeout,
    ...
    private Outcome<Realm.Page> groups(
        ...
        final Outcome<ScimPage<Group>> fetched = page(
                target(Builder.REALM_GROUPS).url(),
                secret,
                count,
                start,
                timeout,
    ...
    private Outcome<Realm.Retrieved> retrieve(
            ...
            final Instant observedAt,
            final Timeout timeout) {
        final String targetName = key.kind() == Realm.Kind.USER ? Builder.REALM_USER : Builder.REALM_GROUP;
    ...
    private Outcome<JsonValue.ObjectValue> get(
        ...
        try {
            response = FabricX.http(Protocol.SCIM, timeout, services.policies()).url(url.toString())
