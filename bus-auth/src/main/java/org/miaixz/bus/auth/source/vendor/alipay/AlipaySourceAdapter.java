...
public class AlipaySourceAdapter implements VendorAdapter {
    ...
    /**
     * Returns the immutable capability manifest frozen by the selected Alipay manifest.
     *
     ...
     */
    private Outcome<Identity> profile(
        ...
        return Outcome.succeeded(
                new Identity(sourceId, profileUser, new JsonValue.ObjectValue(attributes), List.of(evidence)));
    }
    ...
    private CompletionStage<Outcome<JsonValue.ObjectValue>> gateway(
        ...
        final KeyLoader.Request query = new KeyLoader.Request(services.entry(), AUTHORITY,
    ...
    private CompletionStage<Outcome<JsonValue.ObjectValue>> send(
        ...
        fields.put("app_id", options.clientId());
                    ...
                    .handle((resolved, cause) -> {
                        try {
                            ...
                            return switch (resolved) {
                                case Outcome.Succeeded<KeyMaterial> success -> verified(
                                        method,
                                        body,
                                        success.value(),
                                        now);
    ...
    private Outcome<JsonValue.ObjectValue> verified(
            ...
            final KeyMaterial key,
            final Instant now) {
        if (!options.verificationKeyId().equals(key.keyId())
                || !Algorithm.SHA256WITHRSA.getValue().equals(key.algorithm())
        ...
        try {
            final JsonValue parsed = JsonKit.readValue(body, Normal._64, true);
            ...
            raw = JsonKit.extractValue(body, member, Normal._64, true);
