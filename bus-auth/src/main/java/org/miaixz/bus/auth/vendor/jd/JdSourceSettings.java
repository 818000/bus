...
/**
 * Carries externally managed JD application registration values.
 * <p>
 * Fixed authorization, token, refresh, and Zeus router endpoints remain definition-owned. The application secret
 * remains an external reference and is never resolved or exposed by this immutable settings value.
 * </p>
 *
 ...
 * @author Kimi Liu
 */
public record JdSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
     ...
     *                                  definition
     */
    public JdSourceSettings {
        if (!JdDefinition.ID.equals(vendor) || !JdDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("JD settings must select jd/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("JD settings require a registered redirect URI");
        }
    ...
    public String toString() {
        return "JdSourceSettings[vendor=" + vendor + ", variant=" + variant
