...
public class ModifyDNService {
    ...
    /**
     * Creates a Modify DN service for one compiled LDAP server.
     *
     ...
     */
    public ModifyDNService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Modify DN Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Modify DN LDAP server options must not be null");
    ...
    public CompletionStage<Outcome<LdapMessage>> modifyDN(
        ...
        try {
            stage = store
                    .modifyDN(sourceId, context.network().connection().getOrThrow().id(), request, context, timeout);
