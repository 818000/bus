...
/**
 * Applies LDAP server limits and validates the complete RFC 4511 Search response sequence.
 *
 * @author Kimi Liu
 */
public class SearchService {
    ...
    /**
     * Frozen LDAP server search limits.
     */
    ...
    /**
     * Creates a Search service for one compiled LDAP server.
     *
     ...
     */
    public SearchService(final String sourceId, final LdapServerOptions options, final DirectoryStore store) {
        this.sourceId = Assert.notBlank(sourceId, "LDAP Search Source id must not be blank");
        this.options = Assert.notNull(options, "LDAP Search LDAP server options must not be null");
    ...
    public CompletionStage<Outcome<List<LdapMessage>>> search(
        ...
        try {
            stage = store.search(sourceId, context.network().connection().getOrThrow().id(), bounded, context, timeout);
    ...
    /**
     * Rebuilds the standard Search request with LDAP server search limits applied.
     *
     * @param request decoded Search request
     * @return request whose size and time limits cannot exceed LDAP server options
     */
