...
public class LdapClient implements AutoCloseable {
     ...
     */
    public LdapClient(final Executor executor, final LdapClientOptions options, final BerCodec frameCodec,
            final LdapMessageEncoder encoder, final LdapMessageDecoder decoder) {
    ...
    private void startTls(final FabricX.Socket current, final Timeout timeout) {
        ...
        send(
                current,
                new LdapMessage(requestId, new ExtendedRequest(Ldap.START_TLS_OID, Optional.empty()), List.of()),
                timeout);
