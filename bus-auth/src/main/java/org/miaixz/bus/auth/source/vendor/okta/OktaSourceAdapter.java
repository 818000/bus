...
public class OktaSourceAdapter implements VendorAdapter {
    ...
    /**
     * Validated externally loaded Okta configuration options.
     */
     ...
     * @throws ValidateException        if profile, manifest, options, or security rules do not match the frozen variant
     */
    public OktaSourceAdapter(final String spaceId, final String sourceId, final OktaManifest manifest,
        ...
        if (!services.policies().require(Protocol.OIDC).algorithms().contains(JwaAlgorithm.RS256.name())) {
            throw new ValidateException("Okta RS256 is not permitted by the OIDC security rules");
        }
    ...
    private Outcome<Identity> identity(
        ...
        final Evidence evidence = new Evidence(Evidence.Type.FEDERATED, Evidence.Strength.SINGLE_FACTOR,
                new Evidence.Claim(JwtClaims.SUBJECT, new JsonValue.StringValue(claims.subject()), issuer,
                        timeout.clock().now()));
        return Outcome.succeeded(new Identity(sourceId, claims.subject(), userInfo.claims(), List.of(evidence)));
    }
