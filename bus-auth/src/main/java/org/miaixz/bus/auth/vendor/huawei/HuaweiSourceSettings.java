 ...
 * the definition and adapter. This record never resolves or exposes the referenced Client Secret.
 * </p>
 *
 ...
 * @author Kimi Liu
 */
public record HuaweiSourceSettings(Vendor.Id vendor, Vendor.Variant variant, String clientId,
        Credential.Reference credential, Optional<String> redirectUri, List<String> scopes) implements VendorSettings {

    /**
     * Stable identifier of Huawei's sole web Authorization Code variant.
     */
    /**
     ...
     *                                  profile
     */
    public HuaweiSourceSettings {
        if (!HuaweiDefinition.ID.equals(vendor) || !HuaweiDefinition.DEFAULT.equals(variant)) {
            throw new ValidateException("Huawei settings must select huawei/default");
        }
        ...
        if (redirectUri.isEmpty()) {
            throw new ValidateException("Huawei settings require a registered redirect URI");
        }
        ...
        for (String scope : scopes) {
            ...
            if (!approvedScope(checked) || copy.contains(checked)) {
                throw new ValidateException("Huawei scope is unsupported or duplicated by the frozen definition");
            }
            copy.add(checked);
        }
        if (copy.isEmpty()) {
    ...
    public String toString() {
        return "HuaweiSourceSettings[vendor=" + vendor + ", variant=" + variant
