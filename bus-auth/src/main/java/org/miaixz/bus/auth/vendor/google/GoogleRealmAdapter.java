...
/**
 * Implements the provider-neutral Google Workspace realm administration surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public final class GoogleRealmAdapter implements VendorAdapter {
    ...
    /**
     * Converts one minimal Google user projection to a provider-neutral resource.
     *
    ...
    /**
     * Converts one minimal Google organization projection to a provider-neutral resource.
     *
    ...
    /**
     * Converts one minimal Google group projection to a provider-neutral resource.
     *
    ...
    /**
     * Converts one minimal Google administrator role to a provider-neutral resource.
     *
    ...
    private static <T> CompletionStage<Outcome<T>> missing() {
        return completed(
                rejected(ErrorCode._400, "Google enterprise capability is not declared by the selected manifest"));
    }
    ...
    private static <T> CompletionStage<Outcome<T>> mismatch() {
        return completed(
                rejected(ErrorCode._400, "Google enterprise request does not match the selected capability contract"));
    }
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "Google enterprise capability must not be null");
        Assert.notNull(context, "Google enterprise context must not be null");
        Assert.notNull(timeout, "Google enterprise timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return missing();
        }
        if (capability.equals(Realm.describe(GoogleManifest.ID)) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(GoogleManifest.enterpriseDescription())));
        }
        if (capability.equals(Realm.snapshot(GoogleManifest.ID)) && request instanceof Realm.Snapshot snapshot) {
            return narrow(snapshot(snapshot, context, timeout), capability.responseType());
        }
        if (capability.equals(Realm.retrieve(GoogleManifest.ID)) && request instanceof Realm.Retrieve retrieve) {
    ...
    private Outcome<Realm.Page> users(
        ...
        final Outcome<WirePage<User>> fetched = wirePage(
                url(Builder.ENTERPRISE_USERS, null, query),
                access,
                timeout,
                "snapshot",
                "users",
    ...
    private Outcome<Realm.Page> groups(
        ...
        final Outcome<WirePage<Group>> fetched = wirePage(
                url(Builder.ENTERPRISE_GROUPS, null, query),
                access,
                timeout,
                "snapshot",
                "groups",
    ...
    private Outcome<Realm.Page> groupMembers(
        ...
        final Outcome<WirePage<Member>> fetched = wirePage(
                url(Builder.ENTERPRISE_GROUP_MEMBERS, List.of(groupId, "members"), query),
                access,
                timeout,
                "snapshot",
                "members",
    ...
    private Outcome<Realm.Page> roles(
        ...
        final Outcome<WirePage<Role>> fetched = wirePage(
                url(Builder.ENTERPRISE_ROLES, List.of(options.customer(), "roles"), query),
                access,
                timeout,
                "snapshot",
                "items",
    ...
    private Outcome<Realm.Page> roleAssignments(
        ...
        final Outcome<WirePage<RoleAssignment>> fetched = wirePage(
                url(Builder.ENTERPRISE_ROLE_ASSIGNMENTS, List.of(options.customer(), "roleassignments"), query),
                access,
                timeout,
                "snapshot",
                "items",
    ...
    private Outcome<Realm.Retrieved> retrieveUser(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = get(
                url(Builder.ENTERPRISE_USER, List.of(id), Map.of("projection", "basic")),
                access,
                timeout,
                "retrieve",
                true);
    ...
    private Outcome<Realm.Retrieved> retrieveOrganization(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = get(
                url(
                        Builder.ENTERPRISE_ORGANIZATION,
                        List.of(options.customer(), "orgunits", selected.path()),
                        Map.of()),
                access,
                timeout,
                "retrieve",
    ...
    private Outcome<Realm.Retrieved> retrieveGroup(
            ...
            final Timeout timeout) {
        final Outcome<JsonValue.ObjectValue> fetched = get(
                url(Builder.ENTERPRISE_GROUP, List.of(id), Map.of()),
                access,
                timeout,
                "retrieve",
                true);
    ...
    private Outcome<Realm.Retrieved> retrieveRole(
            ...
            final Timeout timeout) {
        final Outcome<JsonValue.ObjectValue> fetched = get(
                url(Builder.ENTERPRISE_ROLE, List.of(options.customer(), "roles", id), Map.of()),
                access,
                timeout,
                "retrieve",
                true);
    ...
    private Outcome<List<Organization>> organizationList(
        ...
        final Outcome<JsonValue.ObjectValue> fetched = getUnpaged(
                url(Builder.ENTERPRISE_ORGANIZATIONS, List.of(options.customer(), "orgunits"), Map.of("type", "all")),
                access,
                timeout,
                operation);
        if (!(fetched instanceof Outcome.Succeeded<JsonValue.ObjectValue> success)) {
    ...
    private Outcome<Optional<String>> groupParent(
        ...
        while (true) {
            final Map<String, String> query = pageQuery(Builder.MAXIMUM_ENTERPRISE_PAGE_SIZE, token);
            query.put("customer", options.customer());
            final Outcome<WirePage<Group>> fetched = wirePage(
                    url(Builder.ENTERPRISE_GROUPS, null, query),
                    access,
    ...
    private String organizationFingerprint(final List<Organization> organizations) {
        ...
        final byte[] encoded = services.jsonProvider().writeValue(new JsonValue.ArrayValue(values));
