...
/**
 * Implements the provider-neutral GitHub Enterprise realm surface.
 * <p>
 ...
 * @author Kimi Liu
 */
public final class GitHubRealmAdapter implements VendorAdapter {
    ...
    private static final List<String> MANAGEMENT_TARGETS = List.of(
            ...
            Builder.ENTERPRISE_ORGANIZATION_ASSIGNMENTS);
    ...
    public <Q, S> CompletionStage<Outcome<S>> invoke(
        ...
        Assert.notNull(capability, "GitHub enterprise capability must not be null");
        Assert.notNull(context, "GitHub enterprise context must not be null");
        Assert.notNull(timeout, "GitHub enterprise timeout must not be null");
        if (!manifest().capabilities().contains(capability)) {
            return completed(rejected(ErrorCode._400, "GitHub enterprise capability is not declared"));
        }
        if (capability.equals(Realm.describe(GitHubManifest.ID)) && request instanceof Realm.Describe) {
            return completed(Outcome.succeeded(capability.responseType().cast(GitHubManifest.enterpriseDescription())));
        }
        if (capability.equals(Realm.snapshot(GitHubManifest.ID)) && request instanceof Realm.Snapshot value) {
            return narrow(snapshot(value, context, timeout), capability.responseType());
        }
        if (capability.equals(Realm.retrieve(GitHubManifest.ID)) && request instanceof Realm.Retrieve value) {
            return narrow(retrieve(value, context, timeout), capability.responseType());
        }
        return completed(rejected(ErrorCode._400, "GitHub enterprise request does not match the capability contract"));
    }
    ...
    private Outcome<Realm.Retrieved> retrieve(
        ...
        final String targetName = key.kind() == Realm.Kind.USER ? Builder.ENTERPRISE_USER
                : Builder.ENTERPRISE_ORGANIZATION;
            ...
            for (Group group : success.value().items()) {
                if (key.externalId().equals(group.id())) {
                    final Outcome<JsonValue.ObjectValue> direct = get(
                            url(Builder.ENTERPRISE_GROUP, List.of(group.slug()), Map.of()),
                            secret,
                            timeout,
                            "retrieve");
    ...
    private <T> Outcome<WirePage<T>> page(
        ...
        final Url request = url(
                phase == Phase.TEAM_ORGANIZATIONS ? Builder.ENTERPRISE_ORGANIZATION_ASSIGNMENTS
                        : phase == Phase.TEAM_MEMBERS ? Builder.ENTERPRISE_GROUP_MEMBERS : Builder.ENTERPRISE_GROUPS,
                segments,
    ...
    private int pagination(final String value, final Phase phase, final String parent, final int perPage) {
        ...
        final Url base = target(Builder.ENTERPRISE_GROUPS).url();
